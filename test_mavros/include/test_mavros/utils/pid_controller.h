...
public:
  ...
  /* *-* PID Setup *-* */
  /**
   * @brief Sets up the PID values for computation of the linear velocities
   * effort
   */
  void setup_linvel_pid(double p_gain, double i_gain, double d_gain,
   ...
   */
  void setup_yawrate_pid(double p_gain, double i_gain, double d_gain,
                         double i_max, double i_min,
                         const ros::NodeHandle &node);
   ...
   */
  Eigen::Vector3d compute_linvel_effort(Eigen::Vector3d goal,
   ...
   */
  double compute_yawrate_effort(double goal, double current,
                                ros::Time last_time);
