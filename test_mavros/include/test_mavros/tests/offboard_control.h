...
public:
  OffboardControl()
      : nh_sp(test.nh),
        local_pos_sp_pub(nh_sp.advertise<geometry_msgs::PoseStamped>(
            "/mavros/setpoint_position/local", 10)),
        vel_sp_pub(nh_sp.advertise<geometry_msgs::TwistStamped>(
            "/mavros/setpoint_velocity/cmd_vel", 10)),
        local_pos_sub(nh_sp.subscribe("/mavros/local_position/local", 10,
                                      &OffboardControl::local_pos_cb, this)),
  ...
  void init() {
    ...
    if (use_pid) {
      /**
       * @note some of these are based on values defaulted @
       * https://bitbucket.org/enddl22/ardrone_side_project/ tweaks to them so
       * to get a better velocity response are welcomed!
       */
      ...
      pid.setup_linvel_pid(linvel_p_gain, linvel_i_gain, linvel_d_gain,
                           linvel_i_max, linvel_i_min, nh_sp);
      pid.setup_yawrate_pid(yawrate_p_gain, yawrate_i_gain, yawrate_d_gain,
                            yawrate_i_max, yawrate_i_min, nh_sp);
    }
    ...
    if (mode_ == "position")
      ...
      ROS_ERROR_NAMED("sitl_test",
                      "Control mode: wrong/unexistant control mode name %s",
                      mode_.c_str());
      return;
    }
    ...
    if (shape_ == "square")
      ...
      ROS_ERROR_NAMED("sitl_test",
                      "Path shape: wrong/unexistant path shape name %s",
                      shape_.c_str());
      return;
    }
  }
  ...
  void spin(int argc, char *argv[]) {
    ...
    if (mode == POSITION) {
      ...
      ROS_ERROR_NAMED("sitl_test", "Control mode: acceleration control mode "
                                   "not supported in PX4 current Firmware.");
      /**
...
private:
  ...
  Eigen::Vector3d pos_setpoint(int tr_x, int tr_y, int tr_z) {
    /** @todo Give possibility to user define amplitude of movement (square
     * corners coordinates)*/
  ...
  Eigen::Vector3d circle_shape(int angle) {
    /** @todo Give possibility to user define amplitude of movement (circle
     * radius)*/
    double r = 5.0f; // 5 meters radius

    return Eigen::Vector3d(r * cos(angles::from_degrees(angle)),
                           r * sin(angles::from_degrees(angle)), 1.0f);
  }
  ...
  Eigen::Vector3d eight_shape(int angle) {
    /** @todo Give possibility to user define amplitude of movement (vertical
     * tangent size)*/
    double a = 5.0f; // vertical tangent with 5 meters size

    return Eigen::Vector3d(a * cos(angles::from_degrees(angle)),
                           a * sin(angles::from_degrees(angle)) *
                               cos(angles::from_degrees(angle)),
                           1.0f);
  }
  ...
  Eigen::Vector3d ellipse_shape(int angle) {
    /** @todo Give possibility to user define amplitude of movement (tangent
     * sizes)*/
    double a = 5.0f; // major axis
    double b = 2.0f; // minor axis

    // rotation around y-axis
    return Eigen::Vector3d(a * cos(angles::from_degrees(angle)), 0.0f,
  ...
  void circle_path_motion(ros::Rate loop_rate, control_mode mode) {
    ...
    while (ros::ok()) {
      ...
      // starting point
      if (mode == POSITION) {
        tf::pointEigenToMsg(Eigen::Vector3d(5.0f, 0.0f, 1.0f),
                            ps.pose.position);
        local_pos_sp_pub.publish(ps);
      } else if (mode == VELOCITY) {
        if (use_pid)
          tf::vectorEigenToMsg(
              pid.compute_linvel_effort(Eigen::Vector3d(5.0f, 0.0f, 1.0f),
                                        current, last_time),
              vs.twist.linear);
        else
          tf::vectorEigenToMsg(Eigen::Vector3d(5.0f - current.x(), -current.y(),
                                               1.0f - current.z()),
                               vs.twist.linear);
      ...
      for (int theta = 0; theta <= 360; theta++) {
        ...
        if (mode == POSITION) {
          ...
          if (use_pid)
            tf::vectorEigenToMsg(pid.compute_linvel_effort(circle_shape(theta),
                                                           current, last_time),
                                 vs.twist.linear);
          else
            tf::vectorEigenToMsg(circle_shape(theta) - current,
                                 vs.twist.linear);
          vel_sp_pub.publish(vs);
        } else if (mode == ACCELERATION) {
          // TODO
  ...
  void eight_path_motion(ros::Rate loop_rate, control_mode mode) {
    ...
    while (ros::ok()) {
      ...
      // starting point
      if (mode == POSITION) {
        tf::pointEigenToMsg(Eigen::Vector3d(0.0f, 0.0f, 1.0f),
                            ps.pose.position);
        local_pos_sp_pub.publish(ps);
      } else if (mode == VELOCITY) {
        if (use_pid)
          tf::vectorEigenToMsg(
              pid.compute_linvel_effort(Eigen::Vector3d(0.0f, 0.0f, 1.0f),
                                        current, last_time),
              vs.twist.linear);
        else
          tf::vectorEigenToMsg(
              Eigen::Vector3d(-current.x(), -current.y(), 1.0f - current.z()),
              vs.twist.linear);
        vel_sp_pub.publish(vs);
      } else if (mode == ACCELERATION) {
        // TODO
      ...
      for (int theta = -180; theta <= 180; theta++) {
        ...
        if (mode == POSITION) {
          ...
          if (use_pid)
            tf::vectorEigenToMsg(pid.compute_linvel_effort(eight_shape(theta),
                                                           current, last_time),
                                 vs.twist.linear);
          else
  ...
  void ellipse_path_motion(ros::Rate loop_rate, control_mode mode) {
    ...
    while (ros::ok()) {
      ...
      // starting point
      if (mode == POSITION) {
        tf::pointEigenToMsg(Eigen::Vector3d(0.0f, 0.0f, 2.5f),
                            ps.pose.position);
        local_pos_sp_pub.publish(ps);
      } else if (mode == VELOCITY) {
        if (use_pid)
          tf::vectorEigenToMsg(
              pid.compute_linvel_effort(Eigen::Vector3d(0.0f, 0.0f, 2.5f),
                                        current, last_time),
              vs.twist.linear);
        else
          tf::vectorEigenToMsg(
              Eigen::Vector3d(-current.x(), -current.y(), 2.5f - current.z()),
              vs.twist.linear);
        vel_sp_pub.publish(vs);
      } else if (mode == ACCELERATION) {
        // TODO
      ...
      for (int theta = 0; theta <= 360; theta++) {
        ...
        if (mode == POSITION) {
          ...
          if (use_pid)
            tf::vectorEigenToMsg(pid.compute_linvel_effort(ellipse_shape(theta),
                                                           current, last_time),
                                 vs.twist.linear);
          else
            tf::vectorEigenToMsg(ellipse_shape(theta) - current,
                                 vs.twist.linear);
          vel_sp_pub.publish(vs);
        } else if (mode == ACCELERATION) {
          // TODO
  ...
  void wait_and_move(geometry_msgs::PoseStamped target) {
    ...
    while (ros::ok() && !stop) {
      ...
      if (mode == POSITION) {
        ...
        if (use_pid)
          tf::vectorEigenToMsg(
              pid.compute_linvel_effort(dest, current, last_time),
              vs.twist.linear);
        else
