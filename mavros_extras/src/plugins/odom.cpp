...
public:
  ...
  explicit OdometryPlugin(plugin::UASPtr uas_)
      : Plugin(uas_, "odometry"), fcu_odom_parent_id_des("map"),
        fcu_odom_child_id_des("base_link") {
    enable_node_watch_parameters();

    // frame params:
    node_declare_and_watch_parameter("fcu.odom_parent_id_des", "map",
                                     [&](const rclcpp::Parameter &p) {
                                       fcu_odom_parent_id_des = p.as_string();
                                     });
    node_declare_and_watch_parameter("fcu.odom_child_id_des", "map",
                                     [&](const rclcpp::Parameter &p) {
    ...
    odom_sub = node->create_subscription<nav_msgs::msg::Odometry>(
        "~/out", 1, std::bind(&OdometryPlugin::odom_cb, this, _1));
  }

  Subscriptions get_subscriptions() override {
    return {make_handler(&OdometryPlugin::handle_odom)};
  }
...
private:
   ...
   * @param[in,out] &tf_source2target The affine transform from the source to
   * target
   */
  void lookup_static_transform(const std::string &target,
    ...
    try {
      ...
      tf_source2target = tf2::transformToEigen(
          uas->tf2_buffer.lookupTransform(target, source, rclcpp::Time(0)));
    } catch (tf2::TransformException &ex) {
      RCLCPP_ERROR_THROTTLE(get_logger(), *get_clock(), 1, "ODOM: Ex: %s",
                            ex.what());
      return;
    }
  }
   ...
   *
   * Callback for mavlink ODOMETRY messages sent from the FCU. According to the
   * mavlink specification, all quantities are for the child frame (fcu_frd),
   * expressed in the parent frame (local_origin_ned). To be compliant with ROS
   * REP 147 for the published nav_msgs/Odometry, the data will be appropriately
   * transformed and published. Frames for the publish message should be
   * specified in specified in the rosparams "odometry/fcu/odom_*_id_des" (set
   * in px4_config.yaml).
   *
   * @param msg	Received Mavlink msg
   * @param odom_msg	ODOMETRY msg
   */
  void handle_odom(const mavlink::mavlink_message_t *msg [[maybe_unused]],
                   mavlink::common::msg::ODOMETRY &odom_msg,
                   plugin::filter::SystemAndOk filter [[maybe_unused]]) {
    /**
     * Required rotations to transform the FCU's odometry msg tto desired parent
     * and child frame
     */
    Eigen::Affine3d tf_parent2parent_des;
    Eigen::Affine3d tf_child2child_des;

    lookup_static_transform(fcu_odom_parent_id_des, "map_ned",
                            tf_parent2parent_des);
    lookup_static_transform(fcu_odom_child_id_des, "base_link_frd",
                            tf_child2child_des);
    ...
    ftf::mavlink_urt_to_covariance_matrix(odom_msg.velocity_covariance,
                                          cov_vel);
    ...
    Matrix6d r_pose =
        Matrix6d::Zero(); //!< Pose 6-D Covariance matrix. WRT frame_id
    Matrix6d r_vel = Matrix6d::Zero(); //!< Velocity 6-D Covariance matrix. WRT
                                       //!< child_frame_id

    auto odom = nav_msgs::msg::Odometry();

    odom.header =
        uas->synchronized_header(fcu_odom_parent_id_des, odom_msg.time_usec);
     ...
     */
    position =
        Eigen::Vector3d(tf_parent2parent_des.linear() *
                        Eigen::Vector3d(odom_msg.x, odom_msg.y, odom_msg.z));
    odom.pose.pose.position = tf2::toMsg(position);

    /**
     * Orientation parsing. Quaternion has to be the rotation from desired child
     * frame to desired parent frame
     */
    Eigen::Quaterniond q_child2parent(ftf::mavlink_to_quaternion(odom_msg.q));
    Eigen::Affine3d tf_childDes2parentDes =
        tf_parent2parent_des * q_child2parent * tf_child2child_des.inverse();
     ...
     */
    lin_vel =
        Eigen::Vector3d(tf_child2child_des.linear() *
                        Eigen::Vector3d(odom_msg.vx, odom_msg.vy, odom_msg.vz));
    ang_vel =
        Eigen::Vector3d(tf_child2child_des.linear() *
                        Eigen::Vector3d(odom_msg.rollspeed, odom_msg.pitchspeed,
                                        odom_msg.yawspeed));
    ...
    Eigen::Map<Matrix6d>(odom.pose.covariance.data(), cov_pose.rows(),
                         cov_pose.cols()) = cov_pose;
    ...
    Eigen::Map<Matrix6d>(odom.twist.covariance.data(), cov_vel.rows(),
                         cov_vel.cols()) = cov_vel;
   ...
   *
   * Callback to odometry that should go to FCU. The frame_ids in the odom
   * message have to fit the frames that are are added to the tf tree. The
   * odometry message gets rotated such that the parent frame is "odom_ned" and
   * the child frame is "base_link_frd".
   *
   * Message specification: https://mavlink.io/en/messages/common.html#ODOMETRY
  ...
  void odom_cb(const nav_msgs::msg::Odometry::SharedPtr odom) {
    ...
    lookup_static_transform("odom_ned", odom->header.frame_id,
                            tf_parent2parent_des);
    lookup_static_transform("base_link_frd", odom->child_frame_id,
                            tf_child2child_des);
    ...
    Matrix6d r_vel =
        Matrix6d::Zero(); //!< Velocity 6-D Covariance. WRT child_frame_id
     ...
     */
    position = Eigen::Vector3d(tf_parent2parent_des.linear() *
                               ftf::to_eigen(odom->pose.pose.position));

    /**
     * Orientation parsing.
     */
    Eigen::Quaterniond q_child2parent(
     ...
     */
    lin_vel = Eigen::Vector3d(tf_child2child_des.linear() *
                              ftf::to_eigen(odom->twist.twist.linear));
    ang_vel = Eigen::Vector3d(tf_child2child_des.linear() *
    ...
    RCLCPP_DEBUG_STREAM(get_logger(), "ODOM: output: pose covariance matrix:"
                                          << std::endl
                                          << cov_pose_map);
    RCLCPP_DEBUG_STREAM(get_logger(),
                        "ODOM: output: velocity covariance matrix:"
                            << std::endl
                            << cov_vel_map);
