 ...
 *
 * This plugin is intended to publish the location of a landing area captured
 * from a downward facing camera to the FCU and/or receive landing target
 * tracking data coming from the FCU.
 */
...
public:
  explicit LandingTargetPlugin(plugin::UASPtr uas_)
      : Plugin(uas_, "landing_target"), tf_rate(50.0), tf_send(true),
        tf_listen(false), frame_id("landing_target_1"),
        tf_frame_id("landing_target_1"), target_size_x(1.0), target_size_y(1.0),
        fov_x(2.0071286398), fov_y(2.0071286398), focal_length(2.8),
        image_width(640), image_height(480), mav_frame("LOCAL_NED"),
    ...
    node_declare_and_watch_parameter(
        "listen_lt", false, [&](const rclcpp::Parameter &p) {
          ...
          if (listen_lt) {
            land_target_sub =
                node->create_subscription<mavros_msgs::msg::LandingTarget>(
                    "~/raw", 10,
                    std::bind(&LandingTargetPlugin::landtarget_cb, this, _1));
          }
        });
    ...
    node_declare_and_watch_parameter(
        "mav_frame", "LOCAL_NED", [&](const rclcpp::Parameter &p) {
          ...
          // MAV_FRAME index based on given frame name (If unknown, defaults to
          // GENERIC)
        });
    ...
    node_declare_and_watch_parameter(
        "land_target_type", "VISION_FIDUCIAL", [&](const rclcpp::Parameter &p) {
          ...
          // LANDING_TARGET_TYPE index based on given type name (If unknown,
          // defaults to LIGHT_BEACON)
        });
    ...
    node_declare_and_watch_parameter(
        "target_size.y", 1.0,
        [&](const rclcpp::Parameter &p) { target_size_y = p.as_double(); });

    // image size
    node_declare_and_watch_parameter("image.width", 640,
    ...
    node_declare_and_watch_parameter(
        "image.height", 480,
        [&](const rclcpp::Parameter &p) { image_height = p.as_int(); });

    // camera field-of-view -> should be precised using the calibrated camera
    // intrinsics
    node_declare_and_watch_parameter(
    ...
    // tf subsection
    node_declare_and_watch_parameter("tf.rate_limit", 50.0,
                                     [&](const rclcpp::Parameter &p) {
                                       // no dynamic update here yet. need to
                                       // modify the thread in setpoint_mixin to
                                       // handle new rates
                                       tf_rate = p.as_double();
                                     });
    ...
    node_declare_and_watch_parameter(
        "tf.listen", false, [&](const rclcpp::Parameter &p) {
          ...
          RCLCPP_INFO_STREAM(get_logger(),
                             "LT: Listen to landing_target transform "
    ...
    land_target_pub = node->create_publisher<geometry_msgs::msg::PoseStamped>(
        "~/pose_in", sensor_qos);
    lt_marker_pub = node->create_publisher<geometry_msgs::msg::Vector3Stamped>(
        "~/lt_marker", sensor_qos);

    pose_sub = node->create_subscription<geometry_msgs::msg::PoseStamped>(
        "~/pose", 10, std::bind(&LandingTargetPlugin::pose_cb, this, _1));
  }

  Subscriptions get_subscriptions() override {
    return {make_handler(&LandingTargetPlugin::handle_landing_target)};
  }
...
private:
  ...
  rclcpp::Subscription<mavros_msgs::msg::LandingTarget>::SharedPtr
      land_target_sub;
  ...
  void landing_target(uint64_t time_usec, uint8_t target_num, uint8_t frame,
                      Eigen::Vector2f angle, float distance,
                      Eigen::Vector2f size, Eigen::Vector3d pos,
                      Eigen::Quaterniond q, uint8_t type,
   ...
   *      θ_y = π + angle_rad
   */
  void inline cartesian_to_displacement(const Eigen::Vector3d &pos,
   ...
   */
  void send_landing_target(const rclcpp::Time &stamp,
    ...
    // the norm of the position vector is considered the distance to the landing
    // target
    ...
    if (land_target_type.find("VISION")) {
      /**
       * @brief: the camera angular offsets can be computed by knowing the
       * position of the target center relative to the camera center, the
       * field-of-view of the camera and the image resolution being considered.
       * The target size is computed by the angle of view formula (similar to
       * angular diameter).
       */
       ...
       * δ = 2 * atan(d / (2 * D))
       * where,	d = actual diameter; D = distance to the object (or focal length
       * of a camera)
       */
    ...
    if (last_transform_stamp == stamp) {
      RCLCPP_DEBUG_THROTTLE(get_logger(), *get_clock(), 10,
                            "LT: Same transform as last one, dropped.");
      return;
    }
    ...
    landing_target(stamp.nanoseconds() / 1000, id,
                   utils::enum_value(frame), // by default, in LOCAL_NED
                   angle, distance, size_rad, pos, q, utils::enum_value(type),
   ...
   */
  void
  handle_landing_target([[maybe_unused]] const mavlink::mavlink_message_t *msg,
    ...
    auto position = ftf::transform_frame_ned_enu(
        Eigen::Vector3d(land_target.x, land_target.y, land_target.z));
    ...
    RCLCPP_DEBUG_STREAM_THROTTLE(get_logger(), *get_clock(), 10,
                                 "landing_target:\n"
                                     << land_target.to_yaml());
    ...
    if (tf_send) {
      ...
      transform.header.frame_id =
          "landing_target_" + std::to_string(land_target.target_num);
  ...
  void landtarget_cb(const mavros_msgs::msg::LandingTarget::SharedPtr req) {
    ...
    landing_target(rclcpp::Time(req->header.stamp).nanoseconds() / 1000,
                   req->target_num,
                   req->frame, // by default, in LOCAL_NED
                   Eigen::Vector2f(req->angle[0], req->angle[1]), req->distance,
                   Eigen::Vector2f(req->size[0], req->size[1]), position,
                   orientation, req->type,
                   1); // position is valid from the first received msg
  }
};
} // namespace extra_plugins
