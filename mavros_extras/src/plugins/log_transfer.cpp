...
public:
  explicit LogTransferPlugin(plugin::UASPtr uas_)
    ...
    log_entry_pub = node->create_publisher<mavros_msgs::msg::LogEntry>(
        "~/raw/log_entry", 1000);
    log_data_pub = node->create_publisher<mavros_msgs::msg::LogData>(
        "~/raw/log_data", 1000);

    log_request_list_srv =
        node->create_service<mavros_msgs::srv::LogRequestList>(
            "~/raw/log_request_list",
            std::bind(&LogTransferPlugin::log_request_list_cb, this, _1, _2));
    log_request_data_srv =
        node->create_service<mavros_msgs::srv::LogRequestData>(
            "~/raw/log_request_data",
            std::bind(&LogTransferPlugin::log_request_data_cb, this, _1, _2));
    log_request_end_srv = node->create_service<mavros_msgs::srv::LogRequestEnd>(
        "~/raw/log_request_end",
        std::bind(&LogTransferPlugin::log_request_end_cb, this, _1, _2));
    log_request_erase_srv = node->create_service<std_srvs::srv::Trigger>(
        "~/raw/log_request_erase",
        std::bind(&LogTransferPlugin::log_request_erase_cb, this, _1, _2));
  }
...
private:
  ...
  rclcpp::Service<mavros_msgs::srv::LogRequestList>::SharedPtr
      log_request_list_srv;
  rclcpp::Service<mavros_msgs::srv::LogRequestData>::SharedPtr
      log_request_data_srv;
  rclcpp::Service<mavros_msgs::srv::LogRequestEnd>::SharedPtr
      log_request_end_srv;
  rclcpp::Service<std_srvs::srv::Trigger>::SharedPtr log_request_erase_srv;

  void handle_log_entry(const mavlink::mavlink_message_t *mmsg [[maybe_unused]],
  ...
  void handle_log_data(const mavlink::mavlink_message_t *mmsg [[maybe_unused]],
    ...
    msg.data.insert(msg.data.cbegin(), ld.data.cbegin(),
                    ld.data.cbegin() + count);
  ...
  void
  log_request_end_cb(mavros_msgs::srv::LogRequestEnd::Request::SharedPtr req
                     [[maybe_unused]],
  ...
  void log_request_erase_cb(std_srvs::srv::Trigger::Request::SharedPtr req
                            [[maybe_unused]],
