...
public:
  MountStatusDiag(const std::string &name)
      : diagnostic_updater::DiagnosticTask(name),
        _last_orientation_update(0, 0), _debounce_s(NAN), _roll_deg(NAN),
        _pitch_deg(NAN), _yaw_deg(NAN), _setpoint_roll_deg(NAN),
        _setpoint_pitch_deg(NAN), _setpoint_yaw_deg(NAN),
        _err_threshold_deg(NAN), _error_detected(false), _mode(255) {}
  ...
  void set_setpoint(float roll_deg, float pitch_deg, float yaw_deg,
                    uint8_t mode) {
    std::lock_guard<std::mutex> lock(mutex);
  ...
  void run(diagnostic_updater::DiagnosticStatusWrapper &stat) {
    ...
    if (_mode !=
      ...
      // Can only directly compare the MAV_CMD_DO_MOUNT_CONTROL angles with the
      // MOUNT_ORIENTATION angles when in MAVLINK_TARGETING mode
    ...
    if (stale) {
      stat.summary(DiagnosticStatus::STALE,
               ...
               (now - _error_started >
                rclcpp::Duration(std::chrono::duration<double>(_debounce_s)))) {
...
public:
  explicit MountControlPlugin(plugin::UASPtr uas_)
      : Plugin(uas_, "mount_control"), mount_diag("Mount") {
    enable_node_watch_parameters();

    node_declare_and_watch_parameter("negate_measured_roll", false,
                                     [&](const rclcpp::Parameter &p) {
                                       negate_measured_roll = p.as_bool();
                                     });
    node_declare_and_watch_parameter("negate_measured_pitch", false,
                                     [&](const rclcpp::Parameter &p) {
                                       negate_measured_pitch = p.as_bool();
                                     });
    node_declare_and_watch_parameter(
        "negate_measured_yaw", false,
        [&](const rclcpp::Parameter &p) { negate_measured_yaw = p.as_bool(); });

    node_declare_and_watch_parameter("debounce_s", 4.0,
                                     [&](const rclcpp::Parameter &p) {
    ...
    command_sub = node->create_subscription<mavros_msgs::msg::MountControl>(
        "~/command", 10, std::bind(&MountControlPlugin::command_cb, this, _1));

    mount_orientation_pub =
        node->create_publisher<geometry_msgs::msg::Quaternion>("~/orientation",
                                                               10);
    mount_status_pub =
        node->create_publisher<geometry_msgs::msg::Vector3Stamped>("~/status",
                                                                   10);

    configure_srv = node->create_service<mavros_msgs::srv::MountConfigure>(
        "~/configure",
        std::bind(&MountControlPlugin::mount_configure_cb, this, _1, _2));
  }

  Subscriptions get_subscriptions() override {
    return {make_handler(&MountControlPlugin::handle_mount_orientation),
...
private:
   ...
   * https://mavlink.io/en/messages/common.html#MOUNT_ORIENTATION
  ...
  void handle_mount_orientation(const mavlink::mavlink_message_t *msg
    ...
    // some gimbals send negated/inverted angle measurements, correct that to
    // obey the MAVLink frame convention
    ...
    auto q = ftf::quaternion_from_rpy(
        Eigen::Vector3d(mo.roll, mo.pitch, mo.yaw) * M_PI / 180.0);
   ...
   */
  void handle_mount_status(const mavlink::mavlink_message_t *mmsg
                           [[maybe_unused]],
    ...
    auto vec = Eigen::Vector3d(ms.pointing_b, ms.pointing_a, ms.pointing_c) *
               M_PI / 18000.0;
    ...
    // pointing_X is cdeg
    auto q = ftf::quaternion_from_rpy(
        Eigen::Vector3d(ms.pointing_b, ms.pointing_a, ms.pointing_c) * M_PI /
        18000.0);
    geometry_msgs::msg::Quaternion quaternion_msg = tf2::toMsg(q);
    mount_orientation_pub->publish(quaternion_msg);
  }
  ...
  void command_cb(const mavros_msgs::msg::MountControl::SharedPtr req) {
    ...
    mount_diag.set_setpoint(req->roll * 0.01f, req->pitch * 0.01f,
                            req->yaw * 0.01f, req->mode);
  }
