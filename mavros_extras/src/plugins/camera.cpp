...
public:
  explicit CameraPlugin(plugin::UASPtr uas_) : Plugin(uas_, "camera") {
    camera_image_captured_pub =
        node->create_publisher<mavros_msgs::msg::CameraImageCaptured>(
            "~/image_captured", 10);
  }

  Subscriptions get_subscriptions() override {
    return {make_handler(&CameraPlugin::handle_camera_image_captured)};
  }

private:
  rclcpp::Publisher<mavros_msgs::msg::CameraImageCaptured>::SharedPtr
      camera_image_captured_pub;
  ...
  void handle_camera_image_captured(
    ...
    ic.geo.altitude = mo.alt / 1E3 +
                      uas->data.geoid_to_ellipsoid_height(&ic.geo); // in meters
