...
//! Type matching
//! mavlink::common::msg::TRAJECTORY::TRAJECTORY_REPRESENTATION_WAYPOINTS fields
...
public:
  explicit TrajectoryPlugin(plugin::UASPtr uas_) : Plugin(uas_, "trajectory") {
    trajectory_generated_sub =
        node->create_subscription<mavros_msgs::msg::Trajectory>(
            "~/generated", 10,
            std::bind(&TrajectoryPlugin::trajectory_cb, this, _1));
    path_sub = node->create_subscription<nav_msgs::msg::Path>(
        "~/path", 10, std::bind(&TrajectoryPlugin::path_cb, this, _1));
    trajectory_desired_pub =
        node->create_publisher<mavros_msgs::msg::Trajectory>("~/desired", 10);
  }

  Subscriptions get_subscriptions() override {
    return {make_handler(&TrajectoryPlugin::handle_trajectory)};
  }

private:
  rclcpp::Subscription<mavros_msgs::msg::Trajectory>::SharedPtr
      trajectory_generated_sub;
  rclcpp::Subscription<nav_msgs::msg::Path>::SharedPtr path_sub;

  rclcpp::Publisher<mavros_msgs::msg::Trajectory>::SharedPtr
      trajectory_desired_pub;
  ...
  //         f"""  const geometry_msgs::msg::{vec_type} & {vec_name}, const
  //         size_t i)\n""" f"""{{\n""" f"""  auto {vec_name}_ned =
  //         ftf::transform_frame_enu_ned("""
  ...
  //
  //
  // outl_fill_points_ned_vector('x', 'y', 'z', 'position', 'Point', range(0,
  // 3)) outl_fill_points_ned_vector('x', 'y', 'z', 'velocity', 'Vector3',
  // range(3, 6)) outl_fill_points_ned_vector('x', 'y', 'z', 'acceleration',
  // 'Vector3', range(6, 9))
  // ]]]
  ...
  void fill_points_yaw_speed(MavPoints &yv, const double yaw_speed,
                             const size_t i) {
    yv[i] = yaw_speed;
  }

  void fill_points_yaw_q(MavPoints &y,
  ...
  void fill_points_delta(MavPoints &y, const double time_horizon,
                         const size_t i) {
    y[i] = time_horizon;
  }
  ...
  void fill_msg_position(
      ...
      const size_t i) {
    auto position_enu = ftf::transform_frame_ned_enu(
        Eigen::Vector3d(t.pos_x[i], t.pos_y[i], t.pos_z[i]));
  ...
  void fill_msg_velocity(
      ...
      const mavlink::common::msg::TRAJECTORY_REPRESENTATION_WAYPOINTS &t,
      const size_t i) {
    auto velocity_enu = ftf::transform_frame_ned_enu(
        Eigen::Vector3d(t.vel_x[i], t.vel_y[i], t.vel_z[i]));
  ...
  void fill_msg_acceleration(
      ...
      const mavlink::common::msg::TRAJECTORY_REPRESENTATION_WAYPOINTS &t,
      const size_t i) {
    auto acceleration_enu = ftf::transform_frame_ned_enu(
        Eigen::Vector3d(t.acc_x[i], t.acc_y[i], t.acc_z[i]));
  ...
  void trajectory_cb(const mavros_msgs::msg::Trajectory::SharedPtr req) {
    ...
    if (req->type ==
        mavros_msgs::msg::Trajectory::MAV_TRAJECTORY_REPRESENTATION_WAYPOINTS) {
      mavlink::common::msg::TRAJECTORY_REPRESENTATION_WAYPOINTS trajectory{};

      auto fill_point_rep_waypoints =
          [&](mavlink::common::msg::TRAJECTORY_REPRESENTATION_WAYPOINTS &t,
              const RosPoints &rp, const size_t i) {
            ...
            fill_points_acceleration(t.acc_x, t.acc_y, t.acc_z,
                                     rp.acceleration_or_force, i);
      ...
      // for i in range(5):
      //      cog.outl(
      //          f"fill_point_rep_waypoints(trajectory, req->point_{i+1},
      //          {i});"
      //      )
      // ]]]
  ...
  void handle_trajectory(
    ...
    tr_desired.header =
        uas->synchronized_header("local_origin", trajectory.time_usec);
