...
public:
  explicit ESCTelemetryPlugin(plugin::UASPtr uas_)
    ...
    esc_telemetry_pub = node->create_publisher<mavros_msgs::msg::ESCTelemetry>(
        "~/telemetry", 10);
...
private:
  ...
  rclcpp::Publisher<mavros_msgs::msg::ESCTelemetry>::SharedPtr
      esc_telemetry_pub;
  mavros_msgs::msg::ESCTelemetry _esc_telemetry;

  template <typename msgT>
  void handle_esc_telemetry(const mavlink::mavlink_message_t *msg
                            [[maybe_unused]],
                            msgT &et, size_t offset = 0) {
