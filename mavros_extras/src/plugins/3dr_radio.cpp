...
public:
  inline bool operator()(plugin::filter::UASPtr uas,
                         const mavlink::mavlink_message_t *cmsg,
                         const plugin::filter::Framing framing) override {
    if (cmsg->sysid != '3' || cmsg->compid != 'D') {
      RCLCPP_WARN_THROTTLE(uas->get_logger(), *uas->get_clock(), 30,
                           "RADIO_STATUS not from 3DR modem?");
    }
...
public:
  explicit TDRRadioPlugin(plugin::UASPtr uas_)
      : Plugin(uas_, "tdr_radio"), has_radio_status(false), diag_added(false),
        low_rssi(0) {
    enable_node_watch_parameters();

    node_declare_and_watch_parameter(
        "low_rssi", 40,
        [&](const rclcpp::Parameter &p) { low_rssi = p.as_int(); });

    auto sensor_qos = rclcpp::SensorDataQoS();

    status_pub = node->create_publisher<mavros_msgs::msg::RadioStatus>(
        "radio_status", sensor_qos);
...
private:
  ...
  void handle_radio(const mavlink::mavlink_message_t *msg [[maybe_unused]],
  ...
  void handle_message(const mavlink::mavlink_message_t *mmsg [[maybe_unused]],
                      msgT &rst) {
