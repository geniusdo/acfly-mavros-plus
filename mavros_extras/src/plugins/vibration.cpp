 ...
 * @plugin vibration
 *
 * This plugin is intended to publish MAV vibration levels and accelerometer
 * clipping from FCU.
 */
class VibrationPlugin : public plugin::Plugin {
public:
  explicit VibrationPlugin(plugin::UASPtr uas_) : Plugin(uas_, "vibration") {
    ...
    vibration_pub = node->create_publisher<mavros_msgs::msg::Vibration>(
        "~/raw/vibration", 10);
  }
...
private:
  ...
  void handle_vibration(const mavlink::mavlink_message_t *msg [[maybe_unused]],
