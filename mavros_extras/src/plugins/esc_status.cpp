...
public:
  explicit ESCStatusPlugin(plugin::UASPtr uas_)
      : Plugin(uas_, "esc_status"), _max_esc_count(0), _max_esc_info_index(0),
        _max_esc_status_index(0) {
    esc_info_pub =
        node->create_publisher<mavros_msgs::msg::ESCInfo>("~/info", 10);
    esc_status_pub =
        node->create_publisher<mavros_msgs::msg::ESCStatus>("~/status", 10);
...
private:
  ...
  void handle_esc_info(const mavlink::mavlink_message_t *msg [[maybe_unused]],
    ...
    for (int i = 0;
         i < std::min<ssize_t>(batch_size, ssize_t(_max_esc_count) - esc_index);
         i++) {
      _esc_info.esc_info[esc_index + i].header = _esc_info.header;
      _esc_info.esc_info[esc_index + i].failure_flags =
          esc_info.failure_flags[i];
      _esc_info.esc_info[esc_index + i].error_count = esc_info.error_count[i];
      _esc_info.esc_info[esc_index + i].temperature =
          esc_info.temperature[i] * 1E2; //!< [degC]
    }
  ...
  void handle_esc_status(const mavlink::mavlink_message_t *msg [[maybe_unused]],
