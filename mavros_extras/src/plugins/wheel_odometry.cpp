 ...
 *
 * This plugin allows computing and publishing wheel odometry coming from FCU
 * wheel encoders. Can use either wheel's RPM or WHEEL_DISTANCE messages (the
...
public:
  ...
  explicit WheelOdometryPlugin(plugin::UASPtr uas_)
      : Plugin(uas_, "wheel_odometry"), odom_mode(OM::NONE), count(0),
        raw_send(false), twist_send(false), tf_send(false),
        yaw_initialized(false), rpose(Eigen::Vector3d::Zero()),
    ...
    node_declare_and_watch_parameter(
        "send_raw", false, [&](const rclcpp::Parameter &p) {
          ...
          if (raw_send) {
            rpm_pub =
                node->create_publisher<mavros_msgs::msg::WheelOdomStamped>(
                    "~/rpm", 10);
            dist_pub =
                node->create_publisher<mavros_msgs::msg::WheelOdomStamped>(
                    "~/distance", 10);
    ...
    node_declare_and_watch_parameter("use_rpm", false,
    ...
    node_declare_and_watch_parameter(
        "send_twist", false,
        [&](const rclcpp::Parameter &p) { twist_send = p.as_bool(); });
    ...
    node_declare_and_watch_parameter(
        "tf.frame_id", "odom",
        [&](const rclcpp::Parameter &p) { tf_frame_id = p.as_string(); });

    node_declare_and_watch_parameter(
        "tf.child_frame_id", "base_link",
        [&](const rclcpp::Parameter &p) { tf_child_frame_id = p.as_string(); });
    ...
    for (int wi = 0; wi < count; wi++) {
      ...
      node_declare_and_watch_parameter(utils::format("wheel%i.x", wi), 0.0,
                                       [wi, this](const rclcpp::Parameter &p) {
                                         wheel_offset[wi][0] = p.as_double();
                                       });
      node_declare_and_watch_parameter(utils::format("wheel%i.y", wi), 0.0,
                                       [wi, this](const rclcpp::Parameter &p) {
                                         wheel_offset[wi][1] = p.as_double();
                                       });
      node_declare_and_watch_parameter(utils::format("wheel%i.radius", wi),
                                       0.05,
                                       [wi, this](const rclcpp::Parameter &p) {
    ...
    if (odom_mode != OM::NONE) {
      if (twist_send) {
        twist_pub = node->create_publisher<
            geometry_msgs::msg::TwistWithCovarianceStamped>("~/velocity", 10);
      } else {
        odom_pub =
  ...
  Subscriptions get_subscriptions() override {
    return {make_handler(&WheelOdometryPlugin::handle_rpm),
...
private:
  ...
  // ‘Eigen::Quaternion<double>’ and ‘Eigen::Quaternion<double>’)
   ...
   * Odometry is computed from the very start but no pose info is published
   * until we have initial orientation (yaw). Once we get it, the robot's
   * current pose is updated with it and starts to be published. Twist info
   * doesn't depend on initial orientation so is published from the very start.
  ...
  void publish_odometry(rclcpp::Time time) {
    ...
    if (!yaw_initialized &&
        !quaterniond_eq(attq, Eigen::Quaterniond::Identity())) {
      ...
      RCLCPP_INFO(get_logger(), "WO: Initial yaw (deg): %f",
                  yaw / M_PI * 180.0);
   ...
   * The wheels are assumed to be parallel to the robot's x-direction (forward)
   * and with the same x-offset. No slip is assumed (Instantaneous Center of
   * Curvature (ICC) along the axis connecting the wheels). All computations are
   * performed for ROS frame conventions. The approach is the extended and more
   * accurate version of standard one described in the book
   ...
   * and that the robot's origin can be specified anywhere instead of the
   * middle-point between the wheels.
   * @param distance	distance traveled by each wheel since last odometry
   * update
  ...
  void update_odometry_diffdrive(std::vector<double> distance, double dt) {
    ...
    // Distance traveled by the projection of origin onto the axis connecting
    // the wheels (Op)
    ...
    R_yaw << -sy, -cy, 0, cy, -sy, 0, 0, 0, 0;
    // dYaw/dPose
    Eigen::Vector3d yaw_pose(0, 0, 1);
    // Jacobian by previous pose
    Eigen::Matrix3d J_pose =
        Eigen::Matrix3d::Identity() + R_yaw * dpose * yaw_pose.transpose();
    ...
    v_meas << L_L0, L_L1, a * theta_L0, a * theta_L1, theta_L0, theta_L1;
    // dTheta/dMeasurement
    ...
    M_theta << px, -qx, 0, qx, px, 0, 0, 0, 0;
    // Jacobian by measurement
    Eigen::Matrix<double, 3, 2> J_meas =
        R * (M * v_meas + M_theta * v * theta_meas.transpose());

    // Measurement cov
    double L0_cov = vel_cov * dt * dt;
    double L1_cov = vel_cov * dt * dt;
    Eigen::Matrix2d meas_cov;
    meas_cov << L0_cov, 0, 0, L1_cov;

    // Update pose cov
    rpose_cov = J_pose * rpose_cov * J_pose.transpose() +
                J_meas * meas_cov * J_meas.transpose();
  }

  /**
   * @brief Update odometry (currently, only 2-wheels differential configuration
   * implemented). Odometry is computed for robot's origin (IMU).
   * @param distance	distance traveled by each wheel since last odometry
   * update
   * @param dt		time elapse since last odometry update (s)
   */
   ...
   * @param rpm		whether measurement contains RPM-s or cumulative wheel
   * distances
   * @param time		measurement's internal time stamp (for accurate
   * dt computations)
   * @param time_pub	measurement's time stamp for publish
   */
  void process_measurement(std::vector<double> measurement, bool rpm,
                           rclcpp::Time time, rclcpp::Time time_pub) {
    // Initial measurement
    if (time_prev == rclcpp::Time(0)) {
      count_meas = measurement.size();
      measurement_prev.resize(count_meas);
      count = std::min(count,
                       count_meas); // don't try to use more wheels than we have
    } else if (time == time_prev) {
      // Same time stamp (messages are generated by FCU more often than the
      // wheel state updated)
      return;
    } else if (measurement.size() != static_cast<size_t>(count_meas)) {
      // # of wheels differs from the initial value
      RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 10,
                           "WO: Number of wheels in measurement (%lu) differs "
                           "from the initial value (%i).",
                           measurement.size(), count_meas);
      return;
    } else {
      // Compute odometry
      double dt =
          (time - time_prev).seconds(); // Time since previous measurement (s)
      ...
      // This generalizes odometry computations for 1- and 2-wheels
      // configurations.
      if (count == 1) {
    ...
    // Time step
    time_prev = time;
    std::copy_n(measurement.begin(), measurement.size(),
   ...
   * @param rpm	RPM msg
   */
  void handle_rpm(const mavlink::mavlink_message_t *msg [[maybe_unused]],
   ...
   * https://mavlink.io/en/messages/common.html#WHEEL_DISTANCE
   * @param msg	Received Mavlink msg
  ...
  void handle_wheel_distance(const mavlink::mavlink_message_t *msg
    ...
    if (raw_send) {
      ...
      std::copy_n(wheel_dist.distance.begin(), wheel_dist.count,
                  wheel_dist_msg.data.begin());
