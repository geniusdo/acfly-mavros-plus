...
class DistanceSensorItem
    : public std::enable_shared_from_this<DistanceSensorItem> {
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

  explicit DistanceSensorItem(DistanceSensorPlugin *owner_,
                              std::string topic_name_, YAML::Node config);
...
private:
  ...
  Eigen::Quaternionf
      quaternion; //!< Orientation in vehicle body frame for ROTATION_CUSTOM
...
public:
  explicit DistanceSensorPlugin(plugin::UASPtr uas_)
    ...
    node_declare_and_watch_parameter(
        ...
        [&](const rclcpp::Parameter &p) { base_frame_id = p.as_string(); });
    node_declare_and_watch_parameter(
        "config", "", [&](const rclcpp::Parameter &p) {
          ...
          try {
            ...
            RCLCPP_ERROR_STREAM(lg,
                                "DS: Failed to parse config: " << ex.what());
            return;
          }
          ...
          for (auto it = root_node.begin(); it != root_node.end(); ++it) {
            ...
            RCLCPP_INFO_STREAM(
                lg, "DS: " << key_s << ": Loading config: " << it->second);

            try {
              auto item =
                  std::make_shared<DistanceSensorItem>(this, key_s, it->second);
              sensor_map[item->sensor_id] = item;
            } catch (const std::exception &ex) {
              RCLCPP_ERROR_STREAM(lg, "DS: " << key_s
...
private:
  ...
  /* -*- low-level send -*- */
  void distance_sensor(uint32_t time_boot_ms, uint32_t min_distance,
                       uint32_t max_distance, uint32_t current_distance,
                       uint8_t type, uint8_t id, uint8_t orientation,
                       uint8_t covariance, float horizontal_fov,
                       float vertical_fov, std::array<float, 4> quaternion,
  ...
  void handle_distance_sensor(const mavlink::mavlink_message_t *msg
    ...
    if (it == sensor_map.end()) {
      RCLCPP_ERROR(
          lg, "DS: no mapping for sensor id: %d, type: %d, orientation: %d",
    ...
    if (sensor->is_subscriber) {
      RCLCPP_ERROR(lg,
                   "DS: %s (id %d) is subscriber, but i got sensor data for "
                   "that id from FCU",
    ...
    if (sensor->orientation >= 0 &&
        dist_sen.orientation != sensor->orientation) {
      RCLCPP_ERROR(
          lg,
          "DS: %s: received sensor data has different orientation (%s) than in "
          "config (%s)!",
          sensor->topic_name.c_str(),
          utils::to_string_enum<MAV_SENSOR_ORIENTATION>(dist_sen.orientation)
              .c_str(),
          utils::to_string_enum<MAV_SENSOR_ORIENTATION>(sensor->orientation)
              .c_str());
      return;
    }
    ...
    range.header =
        uas->synchronized_header(sensor->frame_id, dist_sen.time_boot_ms);
    ...
    default:
      RCLCPP_ERROR(
          lg, "DS: %s: Wrong/undefined type of sensor (type: %d). Dropping!...",
    ...
    if (sensor->send_tf) {
      /* variables init */
      Eigen::Quaterniond q;
      if (dist_sen.orientation ==
          enum_value(MAV_SENSOR_ORIENTATION::ROTATION_CUSTOM)) {
        q = ftf::mavlink_to_quaternion(dist_sen.quaternion);
      } else {
        q = utils::sensor_orientation_matching(
      ...
      transform.header =
          uas->synchronized_header(base_frame_id, dist_sen.time_boot_ms);
...
DistanceSensorItem::DistanceSensorItem(DistanceSensorPlugin *owner_,
                                       std::string topic_name_,
                                       YAML::Node config)
    : is_subscriber(false), send_tf(false), sensor_id(0), field_of_view(0),
      position(0.0, 0.0, 0.0), orientation(-1), covariance(0),
      horizontal_fov_ratio(1.0), vertical_fov_ratio(1.0),
      quaternion(0.f, 0.f, 0.f, 0.f), topic_name(topic_name_),
      owner(owner_), data{}, data_index(0) {
  ...
  if (!is_subscriber) {
    ...
    if (orientation == enum_value(MSO::ROTATION_CUSTOM) &&
      ...
      throw std::invalid_argument(
          "`custom_orientation` required for orientation=CUSTOM");
    }
    ...
    if (auto con = config["custom_orientation"];
      ...
      quaternion =
          Eigen::Quaternionf(ftf::quaternion_from_rpy(rpy * DEG_TO_RAD));
    }
  }
  ...
  if (!is_subscriber) {
    ...
    sub = owner->node->create_subscription<Range>(
...
void DistanceSensorItem::range_cb(const Range::SharedPtr msg) {
  ...
  owner->distance_sensor(get_time_boot_ms(msg->header.stamp),
                         msg->min_range / 1E-2, msg->max_range / 1E-2,
                         msg->range / 1E-2, type, sensor_id, orientation,
                         covariance_, msg->field_of_view * horizontal_fov_ratio,
                         msg->field_of_view * vertical_fov_ratio, q, 0);
}
