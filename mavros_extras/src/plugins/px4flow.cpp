...
public:
  explicit PX4FlowPlugin(plugin::UASPtr uas_)
      : Plugin(uas_, "px4flow"), ranger_fov(0.0), ranger_min_range(0.3),
    ...
    node_declare_and_watch_parameter(
        ...
        [&](const rclcpp::Parameter &p) { ranger_fov = p.as_double(); });

    node_declare_and_watch_parameter(
        "ranger_min_range", 0.3,
        [&](const rclcpp::Parameter &p) { ranger_min_range = p.as_double(); });

    node_declare_and_watch_parameter(
        "ranger_max_range", 5.0,
        [&](const rclcpp::Parameter &p) { ranger_max_range = p.as_double(); });

    flow_rad_pub = node->create_publisher<mavros_msgs::msg::OpticalFlowRad>(
        "~/raw/optical_flow_rad", 10);
    range_pub = node->create_publisher<sensor_msgs::msg::Range>(
        "~/ground_distance", 10);
    temp_pub = node->create_publisher<sensor_msgs::msg::Temperature>(
        "~/temperature", 10);

    flow_rad_sub = node->create_subscription<mavros_msgs::msg::OpticalFlowRad>(
        "~/raw/send", 1, std::bind(&PX4FlowPlugin::send_cb, this, _1));
  }

  Subscriptions get_subscriptions() override {
    return {make_handler(&PX4FlowPlugin::handle_optical_flow_rad)};
  }
...
private:
  ...
  void handle_optical_flow_rad(const mavlink::mavlink_message_t *msg
    ...
    /**
     * Raw message with axes mapped to ROS conventions and temp in degrees
     * celsius.
     *
     * The optical flow camera is essentially an angular sensor, so conversion
     * is like gyroscope. (aircraft -> baselink)
     */
    auto int_xy = ftf::transform_frame_aircraft_baselink(
        Eigen::Vector3d(flow_rad.integrated_x, flow_rad.integrated_y, 0.0));
    auto int_gyro = ftf::transform_frame_aircraft_baselink(
        Eigen::Vector3d(flow_rad.integrated_xgyro, flow_rad.integrated_ygyro,
    ...
    flow_rad_msg.temperature =
        flow_rad.temperature / 100.0f; // in degrees celsius
  ...
  void send_cb(const mavros_msgs::msg::OpticalFlowRad::SharedPtr msg) {
    ...
    auto int_xy = ftf::transform_frame_baselink_aircraft(
        Eigen::Vector3d(msg->integrated_x, msg->integrated_y, 0.0));
    auto int_gyro = ftf::transform_frame_baselink_aircraft(Eigen::Vector3d(
    ...
    flow_rad_msg.integrated_zgyro = int_gyro.z();
    flow_rad_msg.temperature =
        msg->temperature * 100.0f; // temperature in centi-degrees Celsius
