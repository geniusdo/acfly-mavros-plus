...
public:
  explicit VisionSpeedEstimatePlugin(plugin::UASPtr uas_)
    ...
    vision_twist_cov_sub = node->create_subscription<
        geometry_msgs::msg::TwistWithCovarianceStamped>(
        "~/speed_twist_cov", 10,
        std::bind(&VisionSpeedEstimatePlugin::twist_cov_cb, this, _1));
    vision_twist_sub =
    ...
    vision_vector_sub =
        node->create_subscription<geometry_msgs::msg::Vector3Stamped>(
            "~/speed_vector", 10,
            std::bind(&VisionSpeedEstimatePlugin::vector_cb, this, _1));
  }
...
private:
  ...
  rclcpp::Subscription<geometry_msgs::msg::TwistWithCovarianceStamped>::
      SharedPtr vision_twist_cov_sub;
   ...
   * @param usec	Timestamp (microseconds, synced to UNIX time or since
   * system boot) (us)
  ...
  void twist_cb(const geometry_msgs::msg::TwistStamped::SharedPtr req) {
    ...
    convert_vision_speed(req->header.stamp, ftf::to_eigen(req->twist.linear),
                         cov);
  }
   ...
   */
  void twist_cov_cb(
      const geometry_msgs::msg::TwistWithCovarianceStamped::SharedPtr req) {
    ...
    convert_vision_speed(req->header.stamp,
                         ftf::to_eigen(req->twist.twist.linear), cov3d);
  }
