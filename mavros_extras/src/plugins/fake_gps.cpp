 ...
 * vision) to FCU - processed in HIL mode or out of it if parameter
 * MAV_USEHILGPS is set on PX4 Pro Autopilot Firmware; Ardupilot Firmware
 * already supports it without a flag set.
 */
...
public:
  ...
  explicit FakeGPSPlugin(plugin::UASPtr uas_)
      : Plugin(uas_, "fake_gps"),
        ...
        use_mocap(true), use_vision(false), use_hil_gps(true),
        mocap_transform(true), mocap_withcovariance(false), tf_listen(false),
        eph(2.0), epv(2.0), horiz_accuracy(0.0f), vert_accuracy(0.0f),
        speed_accuracy(0.0f), gps_id(0), satellites_visible(5),
        fix_type(GPS_FIX_TYPE::NO_GPS), tf_rate(10.0),
    ...
    node_declare_and_watch_parameter(
        "gps_id", 0, [&](const rclcpp::Parameter &p) { gps_id = p.as_int(); });
    node_declare_and_watch_parameter(
        "fix_type", utils::enum_value(GPS_FIX_TYPE::NO_GPS),
        [&](const rclcpp::Parameter &p) {
          fix_type = static_cast<GPS_FIX_TYPE>(p.as_int());
        });
    node_declare_and_watch_parameter("gps_rate", 5.0,
                                     [&](const rclcpp::Parameter &p) {
    ...
    node_declare_and_watch_parameter(
        "eph", 2.0, [&](const rclcpp::Parameter &p) { eph = p.as_double(); });
    node_declare_and_watch_parameter(
        "epv", 2.0, [&](const rclcpp::Parameter &p) { epv = p.as_double(); });
    node_declare_and_watch_parameter(
        "horiz_accuracy", 0.0,
        [&](const rclcpp::Parameter &p) { horiz_accuracy = p.as_double(); });
    node_declare_and_watch_parameter(
        "vert_accuracy", 0.0,
        [&](const rclcpp::Parameter &p) { vert_accuracy = p.as_double(); });
    node_declare_and_watch_parameter(
        "speed_accuracy", 0.0,
        [&](const rclcpp::Parameter &p) { speed_accuracy = p.as_double(); });
    node_declare_and_watch_parameter(
        "satellites_visible", 5,
        [&](const rclcpp::Parameter &p) { satellites_visible = p.as_int(); });

    // default origin/starting point: Zürich geodetic coordinates
    node_declare_and_watch_parameter(
        "geo_origin.lat", 47.3667,
        [&](const rclcpp::Parameter &p) { map_origin.x() = p.as_double(); });
    node_declare_and_watch_parameter(
        "geo_origin.lon", 8.5500,
        [&](const rclcpp::Parameter &p) { map_origin.y() = p.as_double(); });
    node_declare_and_watch_parameter(
        "geo_origin.alt", 408.0,
        [&](const rclcpp::Parameter &p) { map_origin.z() = p.as_double(); });
    ...
    node_declare_and_watch_parameter(
        ...
        "use_mocap", true,
        [&](const rclcpp::Parameter &p) { use_mocap = p.as_bool(); });
    node_declare_and_watch_parameter(
        // listen to MoCap source (TransformStamped if true; PoseStamped if
        // false)
        "mocap_transform", true,
        [&](const rclcpp::Parameter &p) { mocap_transform = p.as_bool(); });
    node_declare_and_watch_parameter(
    ...
    node_declare_and_watch_parameter(
        ...
        "use_vision", false,
        [&](const rclcpp::Parameter &p) { use_vision = p.as_bool(); });
    node_declare_and_watch_parameter("use_hil_gps", false,
                                     [&](const rclcpp::Parameter &p) {
                                       // send HIL_GPS MAVLink messages if true,
                                       // send GPS_INPUT mavlink messages if
                                       // false
    ...
    node_declare_and_watch_parameter(
        "tf.rate_limit", 10.0,
        [&](const rclcpp::Parameter &p) { tf_rate = p.as_double(); });
    node_declare_and_watch_parameter(
        "tf.listen", false,
        [&](const rclcpp::Parameter &p) { tf_listen = p.as_bool(); });
    ...
    if (use_mocap) {
      ...
      } else if (mocap_withcovariance) { // MoCap data in
                                         // PoseWithCovarianceStamped msg
        mocap_pose_cov_sub = node->create_subscription<
            geometry_msgs::msg::PoseWithCovarianceStamped>(
            "~/mocap/pose_cov", 10,
            std::bind(&FakeGPSPlugin::mocap_pose_cov_cb, this, _1));
      ...
      vision_pose_sub =
          node->create_subscription<geometry_msgs::msg::PoseStamped>(
              "~/vision", 10, std::bind(&FakeGPSPlugin::vision_cb, this, _1));
    } else if (tf_listen) { // Pose acquired from TF Listener
      RCLCPP_INFO_STREAM(get_logger(), "Listen to transform "
                                           << tf_frame_id << " -> "
                                           << tf_child_frame_id);
...
private:
  ...
  bool mocap_withcovariance; //!< ~mocap/pose uses PoseWithCovarianceStamped
                             //!< Message
   ...
   */
  void send_fake_gps(const rclcpp::Time &stamp,
                     const Eigen::Vector3d &ecef_offset) {
    auto now_ = node->now();

    // Throttle incoming messages
    if ((now_ - last_pos_time).to_chrono<std::chrono::nanoseconds>() <
        gps_rate_period) {
      return;
    }
    last_pos_time = now_;
    ...
    Eigen::Vector3d vel =
        (old_ecef - current_ecef) / (stamp.seconds() - old_stamp); // [m/s]
    ...
    if (use_hil_gps) {
      /**
       * @note: <a
       * href="https://mavlink.io/en/messages/common.html#HIL_GPS">HIL_GPS
       * MAVLink message</a> is supported by both Ardupilot and PX4 Firmware.
      ...
      hil_gps.alt = (geodetic.z() +
                     GeographicLib::Geoid::ELLIPSOIDTOGEOID *
                         (*uas->data.egm96_5)(geodetic.x(), geodetic.y())) *
                    1e3;                          // [meters * 1e3]
      ...
      /**
       * @note: <a
       * href="https://mavlink.io/en/messages/common.html#GPS_INPUT">GPS_INPUT
       * MAVLink message</a> is currently only supported by Ardupilot firmware
       */
      ...
      if (eph == 0.0f) {
        gps_input.ignore_flags |=
            utils::enum_value(GPS_INPUT_IGNORE_FLAGS::FLAG_HDOP);
      }
      if (epv == 0.0f) {
        gps_input.ignore_flags |=
            utils::enum_value(GPS_INPUT_IGNORE_FLAGS::FLAG_VDOP);
      }
      ...
      gps_input.speed_accuracy =
          speed_accuracy; // [m/s] TODO how can this be dynamicaly calculated
                          // ???   // NOLINT
      gps_input.horiz_accuracy =
          horiz_accuracy; // [m] will either use the static parameter value, or
                          // the dynamic covariance from function
                          // mocap_pose_cov_cb() bellow  // NOLINT
      gps_input.vert_accuracy =
          vert_accuracy; // [m] will either use the static parameter value, or
                         // the dynamic covariance from function
                         // mocap_pose_cov_cb() bellow  // NOLINT
  ...
  /* -*- callbacks -*- */
  void
  mocap_tf_cb(const geometry_msgs::msg::TransformStamped::SharedPtr trans) {
    Eigen::Affine3d pos_enu;
    tf2::fromMsg(trans->transform, pos_enu);

    send_fake_gps(trans->header.stamp,
                  ftf::transform_frame_enu_ecef(
                      Eigen::Vector3d(pos_enu.translation()), map_origin));
  }

  void mocap_pose_cov_cb(
      const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr req) {
    Eigen::Affine3d pos_enu;
    tf2::fromMsg(req->pose.pose, pos_enu);
    horiz_accuracy = (req->pose.covariance[0] + req->pose.covariance[7]) / 2.0f;
    vert_accuracy = req->pose.covariance[14];

    send_fake_gps(req->header.stamp,
                  ftf::transform_frame_enu_ecef(
                      Eigen::Vector3d(pos_enu.translation()), map_origin));
  }
