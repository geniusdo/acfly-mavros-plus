  ...
  explicit MocapPoseEstimatePlugin(plugin::UASPtr uas_)
    ...
    mocap_tf_sub =
        node->create_subscription<geometry_msgs::msg::TransformStamped>(
            "~/tf", 1,
            std::bind(&MocapPoseEstimatePlugin::mocap_tf_cb, this, _1));
    ...
    mocap_pose_sub = node->create_subscription<geometry_msgs::msg::PoseStamped>(
        "~/pose", 1,
        std::bind(&MocapPoseEstimatePlugin::mocap_pose_cb, this, _1));
  }
...
private:
  ...
  /* -*- low-level send -*- */
  void mocap_pose_send(uint64_t usec, Eigen::Quaterniond &q,
                       Eigen::Vector3d &v) {
  ...
  mocap_tf_cb(const geometry_msgs::msg::TransformStamped::SharedPtr trans) {
    ...
    mocap_pose_send(get_time_usec(trans->header.stamp), q, position);
  }
};
} // namespace extra_plugins
} // namespace mavros
