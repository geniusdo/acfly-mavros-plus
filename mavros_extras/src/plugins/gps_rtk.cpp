...
public:
  explicit GpsRtkPlugin(plugin::UASPtr uas_) : Plugin(uas_, "gps_rtk") {
    gps_rtk_sub = node->create_subscription<mavros_msgs::msg::RTCM>(
        "~/send_rtcm", 10, std::bind(&GpsRtkPlugin::rtcm_cb, this, _1));

    // TODO(vooon): set QoS for latched topic
    rtk_baseline_pub = node->create_publisher<mavros_msgs::msg::RTKBaseline>(
        "~/rtk_baseline", 1);
  }

  Subscriptions get_subscriptions() override {
    return {make_handler(&GpsRtkPlugin::handle_baseline_msg)};
  }
...
private:
   ...
   * It converts the message to the MAVLink GPS_RTCM_DATA message for GPS
   * injection. Message specification:
  ...
  void rtcm_cb(const mavros_msgs::msg::RTCM::SharedPtr msg) {
    ...
    if (msg->data.size() > 4 * max_frag_len) {
      RCLCPP_ERROR(get_logger(), "gps_rtk: RTCM message received is bigger "
                                 "than the maximal possible size.");
      return;
    }
    ...
    if (msg->data.size() <= max_frag_len) {
      ...
      std::fill(rtcm_data.data.begin() + rtcm_data.len, rtcm_data.data.end(),
                0);
      uas->send_message(rtcm_data);
    } else {
      for (uint8_t fragment_id = 0; fragment_id < 4 && data_it < end_it;
           fragment_id++) {
        uint8_t len =
            std::min((size_t)std::distance(data_it, end_it), max_frag_len);
  ...
  void handle_baseline_msg(const mavlink::mavlink_message_t *msg
    ...
    rtk_baseline_.header =
        uas->synchronized_header("", rtk_bsln.time_last_baseline_ms * 1000);
