 ...
 * @plugin adsb
 *
 * Publish/subscribe Automatic dependent surveillance-broadcast data to/from a
 * vehicle.
 */
class ADSBPlugin : public plugin::Plugin {
public:
  explicit ADSBPlugin(plugin::UASPtr uas_) : Plugin(uas_, "adsb") {
    adsb_pub =
        node->create_publisher<mavros_msgs::msg::ADSBVehicle>("~/vehicle", 10);
    adsb_sub = node->create_subscription<mavros_msgs::msg::ADSBVehicle>(
        "~/send", 10,
...
private:
  ...
  void handle_adsb(const mavlink::mavlink_message_t *msg [[maybe_unused]],
    ...
    //     else:
    //         cog.outl(f"""adsb_msg.{ros} = {to_ros.format(mm='adsb',
    //         **locals())};""")
    // ]]]
    ...
    RCLCPP_DEBUG_STREAM(
        get_logger(),
        "ADSB: recv type: "
            << utils::to_string_enum<ADSB_ALTITUDE_TYPE>(adsb.altitude_type)
            << " emitter: "
            << utils::to_string_enum<ADSB_EMITTER_TYPE>(adsb.emitter_type)
