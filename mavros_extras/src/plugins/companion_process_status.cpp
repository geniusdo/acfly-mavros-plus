...
public:
  explicit CompanionProcessStatusPlugin(plugin::UASPtr uas_)
    ...
    status_sub =
        node->create_subscription<mavros_msgs::msg::CompanionProcessStatus>(
            "~/status", 10,
            std::bind(&CompanionProcessStatusPlugin::status_cb, this,
                      std::placeholders::_1));
  }
...
private:
  rclcpp::Subscription<mavros_msgs::msg::CompanionProcessStatus>::SharedPtr
      status_sub;
   ...
   */
  void
  status_cb(const mavros_msgs::msg::CompanionProcessStatus::SharedPtr req) {
    ...
    RCLCPP_DEBUG_STREAM(
        get_logger(),
        "companion process component id: "
            << utils::to_string_enum<MAV_COMPONENT>(req->component)
            << " companion process status: "
            << utils::to_string_enum<MAV_STATE>(heartbeat.system_status)
            << std::endl
            << heartbeat.to_yaml());
