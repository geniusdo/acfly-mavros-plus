...
public:
  explicit VisionPoseEstimatePlugin(plugin::UASPtr uas_)
      : Plugin(uas_, "vision_pose"), tf_rate(10.0) {
    ...
    node_declare_and_watch_parameter(
        "tf/listen", false, [&](const rclcpp::Parameter &p) {
          ...
          if (tf_listen) {
            RCLCPP_INFO_STREAM(get_logger(), "Listen to vision transform"
                                                 << tf_frame_id << " -> "
                                                 << tf_child_frame_id);
            ...
            vision_sub =
                node->create_subscription<geometry_msgs::msg::PoseStamped>(
                    "~/pose", 10,
                    std::bind(&VisionPoseEstimatePlugin::vision_cb, this, _1));
            vision_cov_sub = node->create_subscription<
                geometry_msgs::msg::PoseWithCovarianceStamped>(
                "~/pose_cov", 10,
                std::bind(&VisionPoseEstimatePlugin::vision_cov_cb, this, _1));
          }
        });
