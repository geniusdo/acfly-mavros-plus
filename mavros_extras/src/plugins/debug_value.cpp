...
public:
  explicit DebugValuePlugin(plugin::UASPtr uas_) : Plugin(uas_, "debug_value") {
    // subscribers
    debug_sub = node->create_subscription<DV>(
        "~/send", 10, std::bind(&DebugValuePlugin::debug_cb, this, _1));
    ...
    named_value_float_pub =
        node->create_publisher<DV>("~/named_value_float", 10);
    named_value_int_pub = node->create_publisher<DV>("~/named_value_int", 10);
  }

  Subscriptions get_subscriptions() override {
    return {make_handler(&DebugValuePlugin::handle_debug),
...
private:
  ...
  void debug_logger(const std::string &type, const DV &dv) {
    ...
    RCLCPP_DEBUG_STREAM(get_logger(), type << "\t" << dv.header.stamp.sec << "."
                                           << dv.header.stamp.nanosec << "\t"
                                           << name << "\t[" << dv.index
                                           << "]\tvalue:" << ss.str());
  }
   ...
   */
  void handle_debug(const mavlink::mavlink_message_t *msg [[maybe_unused]],
    ...
    //
    //     cog.outl(f"""DV {p};""")
    //     cog.outl(f"""{p}.header.stamp =
    //     uas->synchronise_stamp({val}.{time_f});""") cog.outl(f"""{p}.type =
    //     DV::{type_};""") cog.outl(f"""{p}.index = {index};""") if name:
   ...
   */
  void handle_debug_vector(const mavlink::mavlink_message_t *msg
                           [[maybe_unused]],
  ...
  /**
   * @todo: add handler for DEBUG_ARRAY
   * (https://github.com/mavlink/mavlink/pull/734)
   */
  ...
  void debug_cb(const mavros_msgs::msg::DebugValue::SharedPtr req) {
    ...
    default:
      RCLCPP_ERROR(get_logger(), "Wrong debug type (%d). Droping!...",
                   req->type);
      return;
    }
  }
