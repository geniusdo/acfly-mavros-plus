 ...
 *
 * Publishes obstacle distance array to the FCU, in order to assist in an
...
public:
  explicit ObstacleDistancePlugin(plugin::UASPtr uas_)
    ...
    obstacle_sub = node->create_subscription<sensor_msgs::msg::LaserScan>(
        "~/send", 10,
  ...
  void obstacle_cb(const sensor_msgs::msg::LaserScan::SharedPtr req) {
    ...
    if (req->ranges.size() <= obstacle.distances.size()) {
      ...
      for (size_t i = 0; i < req->ranges.size(); i++) {
        ...
        if (std::isnan(distance_cm) || distance_cm >= UINT16_MAX ||
            distance_cm < 0) {
      ...
      std::fill(obstacle.distances.begin() + req->ranges.size(),
                obstacle.distances.end(),
                UINT16_MAX); //!< fill the rest of the array values as "Unknown"

      const float increment_deg = req->angle_increment * RAD_TO_DEG;
      obstacle.increment = static_cast<uint8_t>(
          increment_deg + 0.5f); //!< Round to nearest integer.
      ...
      for (size_t i = 0; i < obstacle.distances.size(); i++) {
        ...
        for (size_t j = 0; j < scale_factor; j++) {
          ...
          if (!std::isnan(distance_cm) && distance_cm < UINT16_MAX &&
              distance_cm > 0) {
            obstacle.distances[i] = std::min(
                obstacle.distances[i], static_cast<uint16_t>(distance_cm));
          }
        }
      }
      const float increment_deg =
          req->angle_increment * RAD_TO_DEG * scale_factor;
      obstacle.increment = static_cast<uint8_t>(
          increment_deg + 0.5f); //!< Round to nearest integer.
      obstacle.increment_f = increment_deg;
    }
    ...
    obstacle.sensor_type =
        utils::enum_value(MAV_DISTANCE_SENSOR::LASER); //!< defaults is laser
    ...
    // Assume angle_increment is positive and incoming message is in a FRD/NED
    // frame
