 ...
 * triggered by the FCU. Sequence ID from the message and the image sequence
 * from camera can be corellated to get the exact shutter trigger time.
 */
class CamIMUSyncPlugin : public plugin::Plugin {
public:
  explicit CamIMUSyncPlugin(plugin::UASPtr uas_)
      : Plugin(uas_, "cam_imu_sync") {
    cam_imu_pub = node->create_publisher<mavros_msgs::msg::CamIMUStamp>(
        "~/cam_imu_stamp", 10);
  }
...
private:
  ...
  void handle_cam_trig(const mavlink::mavlink_message_t *msg [[maybe_unused]],
