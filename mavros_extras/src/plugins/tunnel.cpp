...
public:
  explicit TunnelPlugin(plugin::UASPtr uas_) : Plugin(uas_, "tunnel") {
    sub_ = node->create_subscription<mavros_msgs::msg::Tunnel>(
        "~/in", 10, std::bind(&TunnelPlugin::ros_callback, this, _1));
...
private:
  ...
  void mav_callback(const mavlink::mavlink_message_t *msg [[maybe_unused]],
  ...
  static To copy_tunnel(const From &from) noexcept(false) {
    static const auto max_payload_length =
        mavlink::common::msg::TUNNEL().payload.max_size();
    ...
    std::copy(from.payload.begin(), from.payload.begin() + from.payload_length,
              to.payload.begin());
