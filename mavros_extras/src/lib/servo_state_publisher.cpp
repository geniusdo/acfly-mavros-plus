...
ServoDescription::ServoDescription(urdf::Model &model, std::string joint_name_,
                                   YAML::Node config)
  ...
  if (!joint->limits) {
    throw std::runtime_error("URDF for joint " + joint_name +
                             " must provide <limit>");
  }
...
ServoStatePublisher::ServoStatePublisher(const rclcpp::NodeOptions &options,
  ...
  robot_description_sub = this->create_subscription<std_msgs::msg::String>(
      ...
      std::bind(&ServoStatePublisher::robot_description_cb, this, _1));

  // Create topics
  rc_out_sub = this->create_subscription<mavros_msgs::msg::RCOut>(
      "rc_out", sensor_qos,
...
void ServoStatePublisher::robot_description_cb(
  ...
  if (!model.initString(msg->data)) {
    throw std::runtime_error(
        "Unable to initialize urdf::Model from robot description");
  }
  ...
  YAML::Node root_node;
  {
    ...
    this->get_parameter("config", configYaml);
  ...
  for (auto it = root_node.begin(); it != root_node.end(); ++it) {
    ...
    RCLCPP_INFO_STREAM(get_logger(),
                       "SSP: " << joint_name
                               << ": Loading joint: " << it->second);

    try {
      auto joint = servos.emplace_back(model, joint_name, it->second);
      RCLCPP_INFO(get_logger(), "SSP: joint '%s' (RC%zu) loaded",
                  joint_name.c_str(), joint.rc_channel);
    } catch (const std::exception &ex) {
      RCLCPP_ERROR_STREAM(get_logger(),
                          "SSP: " << joint_name
                                  << ": Failed to load mapping: " << ex.what());
    }
  }
}

void ServoStatePublisher::rc_out_cb(
