...
public:
  ...
  explicit ServoDescription(std::string joint_name_ = {})
      : joint_name(joint_name_), joint_lower(-M_PI / 4), joint_upper(M_PI / 4),
        rc_channel(0), rc_min(1000), rc_max(2000), rc_trim(1500), rc_dz(0),
        rc_rev(false) {}

  ServoDescription(urdf::Model &model, std::string joint_name_,
                   YAML::Node config);
  ...
  inline float normalize(uint16_t pwm) {
    ...
    if (pwm > (rc_trim + rc_dz)) {
      chan = (pwm - rc_trim - rc_dz) /
             static_cast<float>(rc_max - rc_trim - rc_dz);
    } else if (pwm < (rc_trim - rc_dz)) {
      chan = (pwm - rc_trim + rc_dz) /
             static_cast<float>(rc_trim - rc_min - rc_dz);
    } else {
      chan = 0.0;
    }
  ...
  inline float calculate_position(uint16_t pwm) {
    ...
    float position =
        (channel + 1.0) * (joint_upper - joint_lower) / (1.0 + 1.0) +
        joint_lower;
