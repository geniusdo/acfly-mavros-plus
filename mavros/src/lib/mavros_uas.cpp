  ...
  mavlink_status({})
{
  ...
  set_parameters_handle_ptr = this->add_on_set_parameters_callback(
      std::bind(&UAS::on_set_parameters_cb, this, std::placeholders::_1));
  ...
  // NOTE(vooon): we couldn't add_plugin() in constructor because it needs
  // shared_from_this()
  startup_delay_timer = this->create_wall_timer(10ms, [this]() {
    ...
    exec_spin_thd =
        thread_ptr(new std::thread([this]() {
                     utils::set_this_thread_name(
                         "uas-exec/%d.%d", source_system, source_component);
                     auto lg = this->get_logger();

                     RCLCPP_INFO(lg, "UAS Executor started, threads: %zu",
    ...
    // setup diag
    diagnostic_updater.setHardwareID(
        utils::format("uas://%s", uas_url.c_str()));
    ...
    if (fcu_protocol == "v1.0") {
      ...
      RCLCPP_WARN(get_logger(),
                  "Unknown FCU protocol: \"%s\", should be: \"v1.0\" or "
                  "\"v2.0\". Used default v2.0.",
                  fcu_protocol.c_str());
      set_protocol_version(mavconn::Protocol::V20);
    }
    ...
    add_connection_change_handler(
        std::bind(&UAS::log_connect_change, this, std::placeholders::_1));
    ...
    RCLCPP_INFO(get_logger(), "Built-in SIMD instructions: %s",
                Eigen::SimdInstructionSetsInUse());
    RCLCPP_INFO(get_logger(), "Built-in MAVLink package version: %s",
                mavlink::version);
    RCLCPP_INFO(get_logger(), "Known MAVLink dialects:%s", ss.str().c_str());
    RCLCPP_INFO(get_logger(),
                "MAVROS UAS via %s started. MY ID %u.%u, TARGET ID %u.%u",
                uas_url.c_str(), source_system, source_component, target_system,
                target_component);
  });
}
...
static bool pattern_match(const std::string &pattern,
                          const std::string &pl_name) {
  int cmp = fnmatch(pattern.c_str(), pl_name.c_str(), FNM_CASEFOLD);
  rcpputils::require_true(cmp == 0 || cmp == FNM_NOMATCH,
 ...
 * @note Issue #257.
 */
bool UAS::is_plugin_allowed(const std::string &pl_name) {
...
plugin::Plugin::SharedPtr
UAS::create_plugin_instance(const std::string &pl_name) {
  auto plugin_factory = plugin_factory_loader.createSharedInstance(pl_name);

  return plugin_factory->create_plugin_instance(
      std::static_pointer_cast<UAS>(shared_from_this()));
}
  ...
  try {
    ...
    for (auto &info : plugin->get_subscriptions()) {
      ...
      if (is_mavlink_message_t(type_hash_)) {
        ...
        log_msgname =
            utils::format("%s (%u) <%zu>", msgname, msgid, type_hash_);
      }
      ...
      if (it == plugin_subscriptions.end()) {
        ...
        if (!append_allowed) {
          ...
          for (auto &e : it->second) {
            ...
            if (!is_mavlink_message_t(t2) && t2 != type_hash_) {
              RCLCPP_ERROR_STREAM(
                  lg, log_msgname << " routed to different message type (hash: "
                                  << t2 << ")");
        ...
        if (append_allowed) {
          ...
          RCLCPP_ERROR_STREAM(lg, log_msgname
                                      << " handler dropped because this ID are "
                                         "used for another message type");
        }
      }
    }
    ...
    RCLCPP_ERROR_STREAM(lg, "Plugin " << pl_name
                                      << " load exception: " << ex.what());
  }
}

rcl_interfaces::msg::SetParametersResult
UAS::on_set_parameters_cb(const std::vector<rclcpp::Parameter> &parameters) {
  auto lg = get_logger();
...
void UAS::connect_to_router() {
  auto qos = rclcpp::QoS(1000).best_effort().durability_volatile();

  this->sink = this->create_publisher<mavros_msgs::msg::Mavlink>(
      utils::format("%s/%s", this->uas_url.c_str(), "mavlink_sink"), qos);
...
void UAS::send_message(const mavlink::Message &obj, const uint8_t src_compid) {
  ...
  obj.serialize(map);
  mavlink::mavlink_finalize_message_buffer(&msg, source_system, src_compid,
                                           &mavlink_status, mi.min_length,
                                           mi.length, mi.crc_extra);
...
void UAS::log_connect_change(bool connected) {
  ...
  if (connected) {
    RCLCPP_INFO(get_logger(), "CON: Got HEARTBEAT, connected. FCU: %s",
                ap.c_str());
  } else {
