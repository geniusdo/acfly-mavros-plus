...
static const cmode_map arduplane_cmode_map{
    {{0, "MANUAL"},   {1, "CIRCLE"},        {2, "STABILIZE"},   {3, "TRAINING"},
     ...
     {14, "LAND"},                                              // not in list
     {15, "GUIDED"},  {16, "INITIALISING"}, {17, "QSTABILIZE"}, // QuadPlane
 ...
 * APMrover2/defines.h
 */
static const cmode_map apmrover2_cmode_map{{{0, "MANUAL"},
                                            {1, "ACRO"},
                                            {3, "STEERING"},
                                            {4, "HOLD"},
                                            {5, "LOITER"},
 ...
 * ArduSub/defines.h
 */
static const cmode_map ardusub_cmode_map{{{0, "STABILIZE"},
                                          {1, "ACRO"},
                                          {2, "ALT_HOLD"},
                                          {3, "AUTO"},   // n/a
                                          {4, "GUIDED"}, // n/a
...
static const cmode_map px4_cmode_map{{
    ...
    {px4::define_mode_auto(px4::custom_mode::SUB_MODE_AUTO_MISSION),
     "AUTO.MISSION"},
    {px4::define_mode_auto(px4::custom_mode::SUB_MODE_AUTO_LOITER),
     "AUTO.LOITER"},
    ...
    {px4::define_mode_auto(px4::custom_mode::SUB_MODE_AUTO_READY),
     "AUTO.READY"},
    {px4::define_mode_auto(px4::custom_mode::SUB_MODE_AUTO_TAKEOFF),
     "AUTO.TAKEOFF"},
    {px4::define_mode_auto(px4::custom_mode::SUB_MODE_AUTO_FOLLOW_TARGET),
     "AUTO.FOLLOW_TARGET"},
    {px4::define_mode_auto(px4::custom_mode::SUB_MODE_AUTO_PRECLAND),
     "AUTO.PRECLAND"},
}};
...
static inline std::string str_mode_px4(uint32_t custom_mode_int) {
  ...
  if (custom_mode.mode.main_mode != px4::custom_mode::MAIN_MODE_AUTO) {
    RCLCPP_WARN_EXPRESSION(
        rclcpp::get_logger("uas"), custom_mode.mode.sub_mode != 0,
        "PX4: Unknown sub-mode %d.%d", custom_mode.mode.main_mode,
        custom_mode.mode.sub_mode);
...
static inline bool is_apm_copter(uas::MAV_TYPE type) {
  return type == uas::MAV_TYPE::QUADROTOR || type == uas::MAV_TYPE::HEXAROTOR ||
         type == uas::MAV_TYPE::OCTOROTOR || type == uas::MAV_TYPE::TRICOPTER ||
  ...
  if (MAV_AUTOPILOT::ARDUPILOTMEGA == ap) {
    if (is_apm_copter(type)) {
      ...
      RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 30000,
                           "MODE: Unknown APM based FCU! Type: %d",
                           enum_value(type));
      return str_custom_mode(custom_mode);
    }
...
static bool cmode_find_cmap(const cmode_map &cmap, std::string &cmode_str,
                            uint32_t &cmode) {
  // 1. try find by name
  for (auto &mode : cmap) {
...
bool UAS::cmode_from_str(std::string cmode_str, uint32_t &custom_mode) {
  // upper case
  std::transform(cmode_str.begin(), cmode_str.end(), cmode_str.begin(),
                 std::ref(toupper));
