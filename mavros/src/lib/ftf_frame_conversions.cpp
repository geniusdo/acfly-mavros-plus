 ...
 * +PI rotation about X (old East/new North)
 */
static const auto NED_ENU_Q = quaternion_from_rpy(M_PI, 0.0, M_PI_2);

/**
 * @brief Static quaternion needed for rotating between aircraft and base_link
 * frames +PI rotation around X (Forward) axis transforms from Forward, Right,
 * Down (aircraft) Fto Forward, Left, Up (base_link) frames.
 */
...
/**
 * @brief Static vector needed for rotating between aircraft and base_link
 * frames +PI rotation around X (Forward) axis transforms from Forward, Right,
 * Down (aircraft) Fto Forward, Left, Up (base_link) frames.
 */
...
/**
 * @brief 3-D matrices to fill 6-D rotation matrix applied to change covariance
...
static const auto AIRCRAFT_BASELINK_R =
    AIRCRAFT_BASELINK_Q.normalized().toRotationMatrix();
...
static const Eigen::PermutationMatrix<3>
    NED_ENU_REFLECTION_XY(Eigen::Vector3i(1, 0, 2));
...
Covariance3d transform_static_frame(const Covariance3d &cov,
  ...
  case StaticTF::NED_TO_ENU:
  case StaticTF::ENU_TO_NED:
    cov_out = NED_ENU_REFLECTION_XY *
              (NED_ENU_REFLECTION_Z * cov_in * NED_ENU_REFLECTION_Z) *
...
Covariance6d transform_static_frame(const Covariance6d &cov,
  ...
  case StaticTF::ENU_TO_NED: {
    ...
    cov_out = NED_ENU_REFLECTION_XY_6 *
              (NED_ENU_REFLECTION_Z_6 * cov_in * NED_ENU_REFLECTION_Z_6) *
  ...
  case StaticTF::AIRCRAFT_TO_BASELINK:
  case StaticTF::BASELINK_TO_AIRCRAFT:
    R.block<3, 3>(0, 0) = R.block<3, 3>(3, 3) = AIRCRAFT_BASELINK_R;
...
Covariance9d transform_static_frame(const Covariance9d &cov,
  ...
  case StaticTF::ENU_TO_NED: {
    ...
    cov_out = NED_ENU_REFLECTION_XY_9 *
              (NED_ENU_REFLECTION_Z_9 * cov_in * NED_ENU_REFLECTION_Z_9) *
  ...
  case StaticTF::AIRCRAFT_TO_BASELINK:
  case StaticTF::BASELINK_TO_AIRCRAFT:
    R.block<3, 3>(0, 0) = R.block<3, 3>(3, 3) = R.block<3, 3>(6, 6) =
        AIRCRAFT_BASELINK_R;
...
Eigen::Vector3d transform_static_frame(const Eigen::Vector3d &vec,
   ...
   * east-axis by an angle of 90 - ϕ. R = [-sinλ         cosλ         0.0
   ...
   */
  Eigen::Matrix3d R;
  R << -sin_lon, cos_lon, 0.0, -cos_lon * sin_lat, -sin_lon * sin_lat, cos_lat,
  ...
  case StaticEcefTF::ENU_TO_ECEF:
    // ENU to ECEF rotation is just an inverse rotation from ECEF to ENU, which
    // means transpose.
    R.transposeInPlace();
    return R * vec;
...
Covariance6d transform_frame(const Covariance6d &cov,
  ...
  R.block<3, 3>(0, 0) = R.block<3, 3>(3, 3) = q.normalized().toRotationMatrix();
