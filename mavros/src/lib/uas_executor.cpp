...
size_t UASExecutor::select_number_of_threads() {
  // return std::max<size_t>(16,
  // std::min<size_t>(std::thread::hardware_concurrency(), 4));
...
void UASExecutor::run(size_t thread_id) {
  utils::set_this_thread_name("uas-exec/%d.%d/%zu", source_system,
                              source_component, thread_id);
