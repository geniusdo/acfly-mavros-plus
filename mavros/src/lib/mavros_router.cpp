...
static inline uint8_t get_msg_byte(const mavlink_message_t *msg,
                                   uint8_t offset) {
  return _MAV_PAYLOAD(msg)[offset];
}

void Router::route_message(Endpoint::SharedPtr src,
...
retry:
  for (auto &kv : this->endpoints) {
    ...
    //              if it is needed perhaps better to add mavlink-router in
    //              front of mavros-router.
  ...
  if (sent_cnt == 0) {
    ...
    RCLCPP_WARN_THROTTLE(
        lg, *clock, 10000,
        "Message dropped: msgid: %d, source: %d.%d, target: %d.%d", msg->msgid,
        msg->sysid, msg->compid, target_addr >> 8, target_addr & 0xff);
  }
}
...
void Router::add_endpoint(
  ...
  this->diagnostic_updater.add(ep->diag_name(),
                               std::bind(&Endpoint::diag_run, ep, _1));
...
void Router::del_endpoint(
  ...
  RCLCPP_INFO(lg, "Requested to del endpoint type: %d url: %s", request->type,
              request->url.c_str());
...
Router::on_set_parameters_cb(const std::vector<rclcpp::Parameter> &parameters) {
  auto lg = get_logger();
  ...
  auto update_endpoints = [&, this](const rclcpp::Parameter &parameter,
                                    Type type) {
    RCLCPP_DEBUG(lg, "Processing urls parameter: %s",
    ...
    std::set_difference(urls_set.begin(), urls_set.end(), existing_set.begin(),
                        existing_set.end(),
                        std::inserter(to_add, to_add.begin()));
    std::set_difference(existing_set.begin(), existing_set.end(),
                        urls_set.begin(), urls_set.end(),
                        std::inserter(to_del, to_del.begin()));
...
void Router::periodic_reconnect_endpoints() {
  ...
  for (auto &kv : this->endpoints) {
    ...
    if (result.first) {
      ...
      RCLCPP_ERROR(lg, "link[%d] reconnect failed: %s", p->id,
                   result.second.c_str());
    }
  }
}
  ...
  for (auto &kv : this->endpoints) {
    ...
    for (auto addr : p->stale_addrs) {
      if (addr != 0) {
        ...
        RCLCPP_INFO(lg, "link[%d] removed stale remote address %d.%d", p->id,
                    addr >> 8, addr & 0xff);
      }
    }
...
void Endpoint::recv_message(const mavlink_message_t *msg,
                            const Framing framing) {
  ...
  if (sp.second || scp.second) {
    RCLCPP_INFO(nh->get_logger(), "link[%d] detected remote address %d.%d",
                this->id, msg->sysid, msg->compid);
  }
...
void MAVConnEndpoint::send_message(const mavlink_message_t *msg,
                                   const Framing framing,
...
void MAVConnEndpoint::diag_run(
  ...
  for (auto addr : this->remote_addrs) {
    stat.addf(utils::format("Remote [%d]", idx++), "%d.%d", addr >> 8,
              addr & 0xff);
  }
...
bool ROSEndpoint::is_open() { return this->source && this->sink; }
...
std::pair<bool, std::string> ROSEndpoint::open() {
  ...
  try {
    auto qos = QoS(1000).best_effort().durability_volatile();
    this->source = nh->create_publisher<mavros_msgs::msg::Mavlink>(
        utils::format("%s/%s", this->url.c_str(), "mavlink_source"), qos);
...
void ROSEndpoint::send_message(const mavlink_message_t *msg,
                               const Framing framing, id_t src_id) {
  rcpputils::assert_true(msg, "msg not null");

  auto rmsg = mavros_msgs::msg::Mavlink();
  auto ok =
      mavros_msgs::mavlink::convert(*msg, rmsg, utils::enum_value(framing));
...
void ROSEndpoint::ros_recv_message(
    const mavros_msgs::msg::Mavlink::SharedPtr rmsg) {
