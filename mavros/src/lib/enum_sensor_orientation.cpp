...
//
// cog.outl(
//     f"static const std::unordered_map<typename
//     std::underlying_type<{ename}>::type,\n" f"  const OrientationPair>
//     sensor_orientations{{{{")
// for k, e in enum:
...
static const std::unordered_map<
    typename std::underlying_type<MAV_SENSOR_ORIENTATION>::type,
...
Eigen::Quaterniond
sensor_orientation_matching(MAV_SENSOR_ORIENTATION orientation) {
  const auto idx = enum_value(orientation);
  auto it = sensor_orientations.find(idx);
...
int sensor_orientation_from_str(const std::string &sensor_orientation) {
  ...
  RCLCPP_ERROR_STREAM(logger,
                      "SENSOR: wrong orientation str: " << sensor_orientation);
