...
Data::Data()
    : imu_enu_data{}, imu_ned_data{}, gps_fix{}, gps_eph(NAN), gps_epv(NAN),
      gps_fix_type(0), gps_satellites_visible(0) {
...
void Data::init_geographiclib() {
  try {
    ...
    rcpputils::require_true(
        false, utils::format("UAS: GeographicLib exception: %s "
...
void Data::update_gps_fix_epts(const sensor_msgs::msg::NavSatFix &fix,
                               float eph, float epv, int fix_type,
...
void Data::get_gps_epts(float &eph, float &epv, int &fix_type,
                        int &satellites_visible) {
  s_shared_lock lock(mu);
