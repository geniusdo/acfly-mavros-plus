...
public:
  TimeSyncStatus(const std::string &name, size_t win_size)
      : diagnostic_updater::DiagnosticTask(name), times_(win_size),
        seq_nums_(win_size), window_size_(win_size), min_freq_(0.01),
        max_freq_(10), tolerance_(0.1), last_rtt(0), rtt_sum(0),
        last_remote_ts(0), offset(0) {
    clear();
  }
  ...
  void tick(int64_t rtt_ns, uint64_t remote_timestamp_ns,
  ...
  void run(diagnostic_updater::DiagnosticStatusWrapper &stat) {
    ...
    stat.addf("Mean RTT (ms)", "%0.6f",
              (count_) ? rtt_sum / count_ / 1e6 : 0.0);
...
public:
  ...
  explicit SystemTimePlugin(plugin::UASPtr uas_)
      : Plugin(uas_, "time"), dt_diag("Time Sync", 10), time_offset(0.0),
        time_skew(0.0), sequence(0), filter_alpha(0), filter_beta(0),
        high_rtt_count(0), high_deviation_count(0) {
    enable_node_watch_parameters();

    node_declare_and_watch_parameter(
        "time_ref_source", "fcu",
        [&](const rclcpp::Parameter &p) { time_ref_source = p.as_string(); });
    ...
    node_declare_and_watch_parameter(
        "timesync_mode", "MAVLINK", [&](const rclcpp::Parameter &p) {
          ...
          RCLCPP_INFO_STREAM(
              get_logger(), "TM: Timesync mode: " << utils::to_string(ts_mode));
        });
    ...
    node_declare_and_watch_parameter(
        "system_time_rate", 0.0, [&](const rclcpp::Parameter &p) {
          ...
          if (rate_d == 0) {
            ...
            sys_time_timer = node->create_wall_timer(
                rate.period(), std::bind(&SystemTimePlugin::sys_time_cb, this));
          }
        });
    ...
    node_declare_and_watch_parameter(
        "timesync_rate", 0.0, [&](const rclcpp::Parameter &p) {
          ...
          if (rate_d == 0) {
            ...
            timesync_timer = node->create_wall_timer(
                rate.period(), std::bind(&SystemTimePlugin::timesync_cb, this));
    ...
    // Filter gains
    //
    // Alpha : Used to smooth the overall clock offset estimate. Smaller values
    // will lead to a smoother estimate, but track time drift more slowly,
    // introducing a bias in the estimate. Larger values will cause
    // low-amplitude oscillations.
    //
    // Beta : Used to smooth the clock skew estimate. Smaller values will lead
    // to a tighter estimation of the skew (derivative), but will negatively
    // affect how fast the filter reacts to clock skewing (e.g cause by
    // temperature changes to the oscillator). Larger values will cause
    // large-amplitude oscillations.
    node_declare_and_watch_parameter("timesync_alpha_initial", 0.05,
                                     [&](const rclcpp::Parameter &p) {
                                       filter_alpha_initial = p.as_double();
                                       reset_filter();
                                     });
    node_declare_and_watch_parameter("timesync_beta_initial", 0.05,
                                     [&](const rclcpp::Parameter &p) {
                                       filter_beta_initial = p.as_double();
                                       reset_filter();
                                     });
    node_declare_and_watch_parameter("timesync_alpha_final", 0.003,
                                     [&](const rclcpp::Parameter &p) {
                                       filter_alpha_final = p.as_double();
                                       reset_filter();
                                     });
    node_declare_and_watch_parameter("timesync_beta_final", 0.003,
                                     [&](const rclcpp::Parameter &p) {
    ...
    //
    // The filter interpolates between the initial and final gains while the
    // number of exhanged timesync packets is less than convergence_window. A
    // lower value will allow the timesync to converge faster, but with
    // potentially less accurate initial offset and skew estimates.
    node_declare_and_watch_parameter(
        "convergence_window", 500,
        [&](const rclcpp::Parameter &p) { convergence_window = p.as_int(); });

    // Outlier rejection and filter reset
    //
    // Samples with round-trip time higher than max_rtt_sample are not used to
    // update the filter. More than max_consecutive_high_rtt number of such
    // events in a row will throw a warning but not reset the filter. Samples
    // whose calculated clock offset is more than max_deviation_sample off from
    // the current estimate are not used to update the filter. More than
    // max_consecutive_high_deviation number of such events in a row will reset
    // the filter. This usually happens only due to a time jump on the remote
    // system.
    node_declare_and_watch_parameter("max_rtt_sample", 10,
                                     [&](const rclcpp::Parameter &p) {
    ...
    node_declare_and_watch_parameter(
        ...
        [&](const rclcpp::Parameter &p) { max_cons_high_rtt = p.as_int(); });
    node_declare_and_watch_parameter("max_consecutive_high_deviation", 10,
                                     [&](const rclcpp::Parameter &p) {
    ...
    time_ref_pub = node->create_publisher<sensor_msgs::msg::TimeReference>(
        "time_reference", sensor_qos);
    timesync_status_pub =
        node->create_publisher<mavros_msgs::msg::TimesyncStatus>(
...
private:
  ...
  rclcpp::Publisher<mavros_msgs::msg::TimesyncStatus>::SharedPtr
      timesync_status_pub;
  ...
  void handle_system_time(const mavlink::mavlink_message_t *msg
    ...
    if (fcu_time_valid) {
      ...
      rclcpp::Time time_ref(mtime.time_unix_usec / 1000000,           // t_sec
  ...
  void handle_timesync(const mavlink::mavlink_message_t *msg [[maybe_unused]],
    ...
    if (tsync.tc1 == 0) {
      ...
      // Time offset between this system and the remote system is calculated
      // assuming RTT for the timesync packet is roughly equal both ways.
      add_timesync_observation((tsync.ts1 + now_ns - tsync.tc1 * 2) / 2,
                               tsync.ts1, tsync.tc1);
    }
  }
  ...
  void timesync_cb() {
    ...
    if (ts_mode == TSM::NONE || ts_mode == TSM::PASSTHROUGH) {
      ...
      // Calculate offset between CLOCK_REALTIME (ros::WallTime) and
      // CLOCK_MONOTONIC
      uint64_t realtime_now_ns = node->now().nanoseconds();
      uint64_t monotonic_now_ns = get_monotonic_now();

      add_timesync_observation(realtime_now_ns - monotonic_now_ns,
                               realtime_now_ns, monotonic_now_ns);
    }
  }
    ...
    if (rtt_ns < max_rtt_sample * 1000000ULL) { // Only use samples with low RTT
      if (sync_converged() && (deviation > max_deviation_sample * 1000000ULL)) {
        ...
        if (!sync_converged()) {
          ...
          filter_alpha =
              p * filter_alpha_final + (1.0f - p) * filter_alpha_initial;
          filter_beta =
              p * filter_beta_final + (1.0f - p) * filter_beta_initial;
        } else {
      ...
      if (high_rtt_count > max_cons_high_rtt) {
        ...
        RCLCPP_WARN(get_logger(), "TM: RTT too high for timesync: %0.2f ms.",
                    rtt_ns / 1000000.0);
  ...
  void add_sample(int64_t offset_ns) {
    /* Online exponential smoothing filter. The derivative of the estimate is
     * also estimated in order to produce an estimate without steady state lag:
    ...
    if (sequence == 0) { // First offset sample
      ...
      time_offset = filter_alpha * offset_ns +
                    (1.0 - filter_alpha) * (time_offset + time_skew);

      // Update the clock skew estimate
      time_skew = filter_beta * (time_offset - time_offset_prev) +
                  (1.0 - filter_beta) * time_skew;
    }
  }
