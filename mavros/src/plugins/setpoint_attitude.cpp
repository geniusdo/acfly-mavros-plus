...
public:
  explicit SetpointAttitudePlugin(plugin::UASPtr uas_)
      : Plugin(uas_, "setpoint_attitude"), reverse_thrust(false) {
    ...
    node_declare_and_watch_parameter(
        ...
        [&](const rclcpp::Parameter &p) { reverse_thrust = p.as_bool(); });
    ...
    node_declare_and_watch_parameter(
        "use_quaternion", false, [&](const rclcpp::Parameter &p) {
          ...
          if (use_quaternion) {
            /**
             * @brief Use message_filters to sync attitude and thrust msg coming
             * from different topics
             */
            pose_sub.subscribe(node, "~/attitude", qos.get_rmw_qos_profile());

            sync_pose = std::make_unique<SyncPoseThrust>(
                SyncPoseThrustPolicy(10), pose_sub, th_sub);
            sync_pose->registerCallback(
                &SetpointAttitudePlugin::attitude_pose_cb, this);

          } else {
            twist_sub.subscribe(node, "~/cmd_vel", qos.get_rmw_qos_profile());

            sync_twist = std::make_unique<SyncTwistThrust>(
                SyncTwistThrustPolicy(10), twist_sub, th_sub);
            sync_twist->registerCallback(
                &SetpointAttitudePlugin::attitude_twist_cb, this);
          }
        });
...
private:
  ...
  inline bool is_normalized(float thrust) {
    ...
    if (reverse_thrust) {
      if (thrust < -1.0) {
        RCLCPP_WARN(lg, "Not normalized reversed thrust! Thd(%f) < Min(%f)",
                    thrust, -1.0);
        return false;
      }
    } else {
      if (thrust < 0.0) {
        RCLCPP_WARN(lg, "Not normalized thrust! Thd(%f) < Min(%f)", thrust,
                    0.0);
        return false;
      }
    }
   ...
   */
  void send_attitude_quaternion(const rclcpp::Time &stamp,
                                const Eigen::Affine3d &tr, const float thrust) {
    /**
    ...
    set_attitude_target(get_time_boot_ms(stamp), ignore_all_except_q_and_thrust,
                        q, Eigen::Vector3d::Zero(), thrust);
  }

  /**
   * @brief Send angular velocity setpoint and thrust to FCU attitude controller
   */
  void send_attitude_ang_velocity(const rclcpp::Time &stamp,
    ...
    set_attitude_target(get_time_boot_ms(stamp), ignore_all_except_rpy,
  ...
  void
  attitude_pose_cb(const geometry_msgs::msg::PoseStamped::SharedPtr pose_msg,
  ...
  void attitude_twist_cb(const geometry_msgs::msg::TwistStamped::SharedPtr req,
                         const mavros_msgs::msg::Thrust::SharedPtr thrust_msg) {
