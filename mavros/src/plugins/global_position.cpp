...
public:
  ...
  explicit GlobalPositionPlugin(plugin::UASPtr uas_)
      : Plugin(uas_, "global_position"), tf_send(false), use_relative_alt(true),
        is_map_init(false), rot_cov(99999.0) {
    ...
    node_declare_and_watch_parameter(
        ...
        [&](const rclcpp::Parameter &p) { child_frame_id = p.as_string(); });
    node_declare_and_watch_parameter(
        "rot_covariance", 99999.0,
        [&](const rclcpp::Parameter &p) { rot_cov = p.as_double(); });
    node_declare_and_watch_parameter(
        "gps_uere", 1.0,
        [&](const rclcpp::Parameter &p) { gps_uere = p.as_double(); });
    node_declare_and_watch_parameter(
        "use_relative_alt", true,
        [&](const rclcpp::Parameter &p) { use_relative_alt = p.as_bool(); });
    ...
    // The global_origin should be represented as "earth" coordinate frame
    // (ECEF) (REP 105)
    node_declare_and_watch_parameter("tf.global_frame_id", "earth",
    ...
    uas->diagnostic_updater.add("GPS", this,
                                &GlobalPositionPlugin::gps_diag_run);

    auto sensor_qos = rclcpp::SensorDataQoS();

    // gps data
    raw_fix_pub = node->create_publisher<sensor_msgs::msg::NavSatFix>(
        "~/raw/fix", sensor_qos);
    raw_vel_pub = node->create_publisher<geometry_msgs::msg::TwistStamped>(
        "~/raw/gps_vel", sensor_qos);
    raw_sat_pub = node->create_publisher<std_msgs::msg::UInt32>(
        "~/raw/satellites", sensor_qos);

    // fused global position
    gp_fix_pub = node->create_publisher<sensor_msgs::msg::NavSatFix>(
        "~/global", sensor_qos);
    gp_odom_pub =
        node->create_publisher<nav_msgs::msg::Odometry>("~/local", sensor_qos);
    gp_rel_alt_pub =
        node->create_publisher<std_msgs::msg::Float64>("~/rel_alt", sensor_qos);
    gp_hdg_pub = node->create_publisher<std_msgs::msg::Float64>("~/compass_hdg",
                                                                sensor_qos);

    // global origin
    gp_global_origin_pub =
        node->create_publisher<geographic_msgs::msg::GeoPointStamped>(
    ...
    gp_global_offset_pub =
        node->create_publisher<geometry_msgs::msg::PoseStamped>(
            "~/gp_lp_offset", sensor_qos);
  }

  Subscriptions get_subscriptions() override {
    return {
        make_handler(&GlobalPositionPlugin::handle_gps_raw_int),
        // GPS_STATUS: there no corresponding ROS message, and it is not
        // supported by APM
...
private:
  ...
  inline void fill_lla(const MsgT &msg, sensor_msgs::msg::NavSatFix &fix) {
    ...
    fix.altitude =
        msg.alt / 1E3 + uas->data.geoid_to_ellipsoid_height(fix); // in meters
  }
  ...
  void handle_gps_raw_int(const mavlink::mavlink_message_t *msg
    ...
    if (msg->magic == MAVLINK_STX && raw_gps.h_acc > 0 && raw_gps.v_acc > 0) {
      ...
      gps_cov.diagonal() << std::pow(eph * gps_uere, 2),
          std::pow(eph * gps_uere, 2), std::pow(epv * gps_uere, 2);
    ...
    // store & publish
    uas->data.update_gps_fix_epts(fix, eph, epv, raw_gps.fix_type,
                                  raw_gps.satellites_visible);
  ...
  void handle_gps_global_origin(
    ...
    g_origin.header =
        uas->synchronized_header(tf_global_frame_id, glob_orig.time_usec);
    ...
    g_origin.position.altitude =
        ...
        uas->data.geoid_to_ellipsoid_height(
            g_origin
                .position); // convert height amsl to height above the ellipsoid

    try {
      /**
       * @brief Conversion from geodetic coordinates (LLA) to ECEF
       * (Earth-Centered, Earth-Fixed) Note: "earth" frame, in ECEF, of the
       * global origin
       */
  ...
  void handle_global_position_int(
    ...
    compass_heading.data =
        (gpos.hdg != UINT16_MAX) ? gpos.hdg / 1E2 : NAN; // in degrees
    ...
    // Linear velocity
    tf2::toMsg(Eigen::Vector3d(gpos.vy, gpos.vx, gpos.vz) / 1E2,
    ...
    try {
       ...
       *
       * Note: "ecef_origin" is the origin of "map" frame, in ECEF, and the
       * local coordinates are in spherical coordinates, with the orientation in
       * ENU (just like what is applied on Gazebo)
       */
       ...
       * - If not, and the home position is also not received, it sets the
       * current fix as the origin;
       * - If the home position is received, it sets the "map" origin;
       * - If the "map" origin is set, then it applies the rotations to the
       * offset between the origin and the current local geocentric coordinates.
       */
      // Current fix to ECEF
      map.Forward(fix.latitude, fix.longitude, fix.altitude, map_point.x(),
                  map_point.y(), map_point.z());
    ...
    odom.pose.pose.position =
        tf2::toMsg(ftf::transform_frame_ecef_enu(local_ecef, map_origin));

    /**
     * @brief By default, we are using the relative altitude instead of the
     * geocentric altitude, which is relative to the WGS-84 ellipsoid
     */
    if (use_relative_alt) {
  ...
  void handle_lpned_system_global_offset(
    ...
    global_offset.header =
        uas->synchronized_header(tf_global_frame_id, offset.time_boot_ms);

    auto enu_position = ftf::transform_frame_ned_enu(
        Eigen::Vector3d(offset.x, offset.y, offset.z));
    auto enu_baselink_orientation =
        ftf::transform_orientation_aircraft_baselink(
            ftf::transform_orientation_ned_enu(ftf::quaternion_from_rpy(
  ...
  void
  set_gp_origin_cb(const geographic_msgs::msg::GeoPointStamped::SharedPtr req) {
