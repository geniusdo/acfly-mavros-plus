...
public:
  ...
  /// mavlink_file_transfer_protocol_t.payload. We pad the structure ourselves
  /// to 32 bit alignment to avoid usage of any pack pragmas.
  ...
  inline const uint8_t *raw_payload() const { return payload.data(); }

  inline uint8_t *raw_payload() { return payload.data(); }
  ...
  inline const uint8_t *data() const { return header()->data; }

  inline uint8_t *data() { return header()->data; }

  inline const char *data_c() const {
    return reinterpret_cast<const char *>(header()->data);
  }

  inline char *data_c() { return reinterpret_cast<char *>(header()->data); }
  ...
  uint8_t get_target_system_id() const { return target_system; }
...
#ifdef FTP_LL_DEBUG
    auto hdr = header();
    RCLCPP_DEBUG(rclcpp::get_logger("ftp.request"),
                 "FTP:rm: SEQ(%u) SESS(%u) OPCODE(%u) RQOP(%u) SZ(%u) OFF(%u)",
                 hdr->seqNumber, hdr->session, hdr->opcode, hdr->req_opcode,
                 hdr->size, hdr->offset);
#endif
  ...
  void send(plugin::UASPtr uas, uint16_t seqNumber) {
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("ftp.request"),
                 ...
                 hdr->seqNumber, hdr->session, hdr->opcode, hdr->size,
                 hdr->offset);
#endif
  ...
  FTPRequest() : mavlink::common::msg::FILE_TRANSFER_PROTOCOL{} {}
...
public:
  explicit FTPPlugin(plugin::UASPtr uas_)
      : Plugin(uas_, "ftp"), op_state(OP::IDLE), last_send_seqnr(0),
        active_session(0), is_error(false), r_errno(0), list_offset(0),
        open_size(0), read_size(0), read_offset(0), read_buffer{},
        write_offset(0), write_buffer{}, write_it{}, checksum_crc32(0) {
    // since C++ generator do not produce field length defs make check explicit.
    FTPRequest r;
    rcpputils::assert_true(
        (r.payload.size() - sizeof(FTPRequest::PayloadHeader)) == r.DATA_MAXSZ);

    list_srv = node->create_service<mavros_msgs::srv::FileList>(
        "~/list", std::bind(&FTPPlugin::list_cb, this, _1, _2));
    open_srv = node->create_service<mavros_msgs::srv::FileOpen>(
        "~/open", std::bind(&FTPPlugin::open_cb, this, _1, _2));
    close_srv = node->create_service<mavros_msgs::srv::FileClose>(
        "~/close", std::bind(&FTPPlugin::close_cb, this, _1, _2));
    read_srv = node->create_service<mavros_msgs::srv::FileRead>(
        "~/read", std::bind(&FTPPlugin::read_cb, this, _1, _2));
    write_srv = node->create_service<mavros_msgs::srv::FileWrite>(
        "~/write", std::bind(&FTPPlugin::write_cb, this, _1, _2));
    mkdir_srv = node->create_service<mavros_msgs::srv::FileMakeDir>(
        "~/mkdir", std::bind(&FTPPlugin::mkdir_cb, this, _1, _2));
    rmdir_srv = node->create_service<mavros_msgs::srv::FileRemoveDir>(
        "~/rmdir", std::bind(&FTPPlugin::rmdir_cb, this, _1, _2));
    remove_srv = node->create_service<mavros_msgs::srv::FileRemove>(
        "~/remove", std::bind(&FTPPlugin::remove_cb, this, _1, _2));
    truncate_srv = node->create_service<mavros_msgs::srv::FileTruncate>(
        "~/truncate", std::bind(&FTPPlugin::truncate_cb, this, _1, _2));
    reset_srv = node->create_service<std_srvs::srv::Empty>(
        "~/reset", std::bind(&FTPPlugin::reset_cb, this, _1, _2));
    rename_srv = node->create_service<mavros_msgs::srv::FileRename>(
        "~/rename", std::bind(&FTPPlugin::rename_cb, this, _1, _2));
    checksum_srv = node->create_service<mavros_msgs::srv::FileChecksum>(
        "~/checksum", std::bind(&FTPPlugin::checksum_cb, this, _1, _2));
  }
...
private:
  ...
  void handle_file_transfer_protocol(const mavlink::mavlink_message_t *msg
                                     [[maybe_unused]],
                                     FTPRequest &req,
    ...
    if (incoming_seqnr != expected_seqnr) {
      RCLCPP_WARN(get_logger(), "FTP: Lost sync! seqnr: %u != %u",
    ...
    if (req.header()->opcode == FTPRequest::kRspAck) {
      ...
      RCLCPP_ERROR(get_logger(), "FTP: Unknown request response: %u",
                   req.header()->opcode);
  ...
  void handle_req_nack(const FTPRequest &req) {
    ...
    RCLCPP_ERROR(get_logger(),
                 "FTP: NAK: %u Opcode: %u State: %u Errno: %d (%s)", error_code,
                 hdr->req_opcode, enum_value(prev_op), r_errno,
                 strerror(r_errno));
    go_idle(true);
  }
  ...
  void handle_ack_list(const FTPRequest &req) {
    ...
    RCLCPP_DEBUG(get_logger(), "FTP:m: ACK List SZ(%u) OFF(%u)", hdr->size,
                 hdr->offset);
    if (hdr->offset != list_offset) {
      RCLCPP_ERROR(get_logger(), "FTP: Wrong list offset, req %u, ret %u",
    ...
    while (off < hdr->size) {
      ...
      if ((ptr[0] == FTPRequest::DIRENT_SKIP && slen > 1) ||
        ...
        RCLCPP_ERROR(get_logger(),
                     "FTP: Missing NULL termination in list entry");
    ...
    if (hdr->size == 0) {
      ...
      rcpputils::assert_true(n_list_entries > 0,
  ...
  void handle_ack_open(const FTPRequest &req) {
    ...
    RCLCPP_INFO(get_logger(), "FTP:Open %s: success, session %u, size %zu",
  ...
  void handle_ack_read(const FTPRequest &req) {
    ...
    read_buffer.insert(read_buffer.end(), req.data(),
                       req.data() + bytes_to_copy);
  ...
  void handle_ack_write(const FTPRequest &req) {
    ...
    const size_t bytes_left_before_advance =
        std::distance(write_it, write_buffer.end());
    rcpputils::assert_true(bytes_written <= bytes_left_before_advance,
                           "Bad write size");
  ...
  void send_reset() {
    ...
    if (!session_file_map.empty()) {
      RCLCPP_WARN(get_logger(), "FTP: Reset closes %zu sessons",
  ...
  inline void send_any_path_command(const FTPRequest::Opcode op,
                                    const std::string &debug_msg,
                                    const std::string &path,
                                    const uint32_t offset) {
    RCLCPP_DEBUG_STREAM(get_logger(),
                        "FTP:m: " << debug_msg << path << " off: " << offset);
    FTPRequest req(op);
  ...
  void send_list_command() {
    send_any_path_command(FTPRequest::kCmdListDirectory,
                          "kCmdListDirectory: ", list_path, list_offset);
  }

  void send_open_ro_command() {
    send_any_path_command(FTPRequest::kCmdOpenFileRO,
                          "kCmdOpenFileRO: ", open_path, 0);
  }

  void send_open_wo_command() {
    send_any_path_command(FTPRequest::kCmdOpenFileWO,
                          "kCmdOpenFileWO: ", open_path, 0);
  }

  void send_create_command() {
    send_any_path_command(FTPRequest::kCmdCreateFile,
                          "kCmdCreateFile: ", open_path, 0);
  }

  void send_terminate_command(uint32_t session) {
    RCLCPP_DEBUG_STREAM(get_logger(),
                        "FTP:m: kCmdTerminateSession: " << session);
  ...
  void send_read_command() {
    ...
    RCLCPP_DEBUG_STREAM(get_logger(),
                        "FTP:m: kCmdReadFile: " << active_session
                                                << " off: " << read_offset);
  ...
  bool send_rename_command(const std::string &old_path,
    ...
    if (paths.size() >= FTPRequest::DATA_MAXSZ) {
      RCLCPP_ERROR(get_logger(), "FTP: rename file paths is too long: %zu",
                   paths.size());
  ...
  void send_truncate_command(const std::string &path, size_t length) {
    send_any_path_command(FTPRequest::kCmdTruncateFile,
                          "kCmdTruncateFile: ", path, length);
  }

  void send_create_dir_command(const std::string &path) {
    send_any_path_command(FTPRequest::kCmdCreateDirectory,
                          "kCmdCreateDirectory: ", path, 0);
  }

  void send_remove_dir_command(const std::string &path) {
    send_any_path_command(FTPRequest::kCmdRemoveDirectory,
                          "kCmdRemoveDirectory: ", path, 0);
  }

  void send_calc_file_crc32_command(const std::string &path) {
    send_any_path_command(FTPRequest::kCmdCalcFileCRC32,
                          "kCmdCalcFileCRC32: ", path, 0);
  }
  ...
  void add_dirent(const char *ptr, size_t slen) {
    ...
    if (ptr[0] == FTPRequest::DIRENT_DIR) {
      ...
      RCLCPP_DEBUG_STREAM(get_logger(),
                          "FTP:List File: " << ent.name << " SZ: " << ent.size);
    }
...
#define SERVICE_IDLE_CHECK()                                                   \
  ...
  void open_cb(const mavros_msgs::srv::FileOpen::Request::SharedPtr req,
    ...
    if (it != session_file_map.end()) {
      RCLCPP_ERROR(get_logger(), "FTP: File %s: already opened",
                   req->file_path.c_str());
