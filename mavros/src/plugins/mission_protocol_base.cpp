...
std::ostream &mavros::plugin::operator<<(std::ostream &os,
                                         const MissionItem &mi) {
  os << '#' << mi.seq << (mi.is_current ? '*' : ' ') << " F:" << +mi.frame
     << " C:" << std::setw(3) << mi.command;
  os << std::setprecision(7) << " p: " << mi.param1 << ' ' << mi.param2 << ' '
     << mi.param3 << ' ' << mi.param4;
  os << std::setprecision(7) << " x: " << mi.x_lat << " y: " << mi.y_long
     << " z: " << mi.z_alt;
  return os;
}

void MissionBase::handle_mission_item(const mavlink::mavlink_message_t *msg
                                      [[maybe_unused]],
                                      MISSION_ITEM &wpi,
  ...
  if (wp_state == WP::RXWP) {
    ...
    RCLCPP_DEBUG(get_logger(), "%s: rejecting item, wrong state %d", log_prefix,
                 enum_value(wp_state));
...
void MissionBase::handle_mission_item_int(const mavlink::mavlink_message_t *msg
                                          [[maybe_unused]],
...
void MissionBase::handle_mission_request(
  ...
  if ((wp_state == WP::TXLIST && mreq.seq == 0) ||
      (wp_state == WP::TXPARTIAL && mreq.seq == wp_start_id) ||
      (wp_state == WP::TXWP) || (wp_state == WP::TXWPINT)) {
    ...
    if (mreq.seq < wp_end_id) {
      RCLCPP_DEBUG(get_logger(), "%s: FCU requested MISSION_ITEM waypoint %d",
                   log_prefix, mreq.seq);
      wp_cur_id = mreq.seq;
      if (use_mission_item_int) {
        RCLCPP_DEBUG(get_logger(),
                     "%s: Trying to send a MISSION_ITEM_INT instead",
                     log_prefix);
        wp_state = WP::TXWPINT;
      ...
      RCLCPP_ERROR(get_logger(), "%s: FCU require seq out of range",
                   log_prefix);
    }
  } else {
    RCLCPP_DEBUG(get_logger(), "%s: rejecting request, wrong state %d",
                 log_prefix, enum_value(wp_state));
  }
}
...
void MissionBase::handle_mission_request_int(
  ...
  if ((wp_state == WP::TXLIST && mreq.seq == 0) ||
    ...
    if (mreq.seq < wp_end_id) {
      RCLCPP_DEBUG(get_logger(),
                   "%s: FCU reqested MISSION_ITEM_INT waypoint %d", log_prefix,
                   mreq.seq);
      wp_state = WP::TXWPINT;
      wp_cur_id = mreq.seq;
      send_waypoint<MISSION_ITEM_INT>(wp_cur_id);
...
void MissionBase::handle_mission_count(
  ...
  if (wp_state == WP::RXLIST) {
    ...
    if (do_pull_after_gcs) {
      RCLCPP_INFO(get_logger(), "%s: scheduling pull after GCS is done",
                  log_prefix);
...
void MissionBase::handle_mission_ack(const mavlink::mavlink_message_t *msg
  ...
  auto is_tx_done = [&]() -> bool {
    return (wp_state == WP::TXLIST || wp_state == WP::TXPARTIAL ||
            wp_state == WP::TXWP || wp_state == WP::TXWPINT) &&
           (wp_cur_id == wp_end_id - 1) && (ack_type == MRES::ACCEPTED);
  };

  auto is_tx_seq_error = [&]() -> bool {
    return (wp_state == WP::TXWP || wp_state == WP::TXWPINT) &&
  ...
  if (is_tx_done()) {
    ...
    RCLCPP_ERROR_STREAM(get_logger(), log_prefix << ": upload failed: "
                                                 << utils::to_string(ack_type));
  } else if (wp_state == WP::CLEAR) {
    go_idle();
    if (ack_type != MRES::ACCEPTED) {
      is_timedout = true;
      lock.unlock();
      RCLCPP_ERROR_STREAM(get_logger(), log_prefix
                                            << ": clear failed: "
                                            << utils::to_string(ack_type));
    } else {
      waypoints.clear();
    ...
    RCLCPP_DEBUG(get_logger(), "%s: not planned ACK, type: %d", log_prefix,
                 mack.type);
  }
}
...
void MissionBase::handle_mission_current(
  ...
  if (wp_state == WP::SET_CUR) {
    ...
    RCLCPP_DEBUG(get_logger(), "%s: set current #%d done", log_prefix,
                 mcur.seq);
    go_idle();
...
void MissionBase::timeout_cb() {
  ...
  if (wp_retries > 0) {
    wp_retries--;
    RCLCPP_WARN(get_logger(), "%s: timeout, retries left %zu", log_prefix,
                wp_retries);
  ...
  if (wp_state == WP::TXWPINT && use_int) {
    RCLCPP_ERROR(
        get_logger(),
        "%s: mission_item_int timed out, falling back to mission_item.",
        log_prefix);
...
void MissionBase::mission_write_partial_list(const uint16_t start_index,
                                             const uint16_t end_index) {
  RCLCPP_DEBUG(get_logger(), "%s:m: write partial list %u - %u", log_prefix,
