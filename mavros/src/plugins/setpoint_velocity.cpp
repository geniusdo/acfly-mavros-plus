...
public:
  explicit SetpointVelocityPlugin(plugin::UASPtr uas_)
    ...
    // cmd_vel usually is the topic used for velocity control in many
    // controllers / planners
    vel_sub = node->create_subscription<geometry_msgs::msg::TwistStamped>(
        "~/cmd_vel", sensor_qos,
        std::bind(&SetpointVelocityPlugin::vel_cb, this, _1));
    vel_unstamped_sub = node->create_subscription<geometry_msgs::msg::Twist>(
        "~/cmd_vel_unstamped", sensor_qos,
        std::bind(&SetpointVelocityPlugin::vel_unstamped_cb, this, _1));
  }
...
private:
   ...
   */
  void send_setpoint_velocity(const rclcpp::Time &stamp,
    ...
    auto yr = [&]() {
      if (mav_frame == MAV_FRAME::BODY_NED ||
        ...
        return ftf::transform_frame_ned_enu(
            Eigen::Vector3d(0.0, 0.0, yaw_rate));
      }
    }();
    ...
    set_position_target_local_ned(
        ...
        ignore_all_except_v_xyz_yr, Eigen::Vector3d::Zero(), vel,
        Eigen::Vector3d::Zero(), 0.0, yr.z());
  }
  ...
  void vel_unstamped_cb(const geometry_msgs::msg::Twist::SharedPtr req) {
    ...
    send_setpoint_velocity(node->now(), vel_enu, req->angular.z);
  }
};
