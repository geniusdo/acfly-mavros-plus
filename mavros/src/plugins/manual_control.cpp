...
public:
  explicit ManualControlPlugin(plugin::UASPtr uas_)
    ...
    control_pub = node->create_publisher<mavros_msgs::msg::ManualControl>(
        "~/control", 10);
    send_sub = node->create_subscription<mavros_msgs::msg::ManualControl>(
        "~/send", 10, std::bind(&ManualControlPlugin::send_cb, this, _1));
  }
...
private:
  ...
  void
  handle_manual_control(const mavlink::mavlink_message_t *msg [[maybe_unused]],
