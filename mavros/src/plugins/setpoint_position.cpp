...
public:
  ...
  explicit SetpointPositionPlugin(plugin::UASPtr uas_)
    ...
    setpoint_sub = node->create_subscription<geometry_msgs::msg::PoseStamped>(
        "~/local", sensor_qos,
        std::bind(&SetpointPositionPlugin::setpoint_cb, this, _1));
    setpointg_sub =
        node->create_subscription<geographic_msgs::msg::GeoPoseStamped>(
            "~/global", sensor_qos,
            std::bind(&SetpointPositionPlugin::setpointg_cb, this, _1));
    setpointg2l_sub =
        node->create_subscription<geographic_msgs::msg::GeoPoseStamped>(
    ...
    gps_sub = node->create_subscription<sensor_msgs::msg::NavSatFix>(
        "global_position/global", sensor_qos,
...
private:
   ...
   */
  void send_position_target(const rclcpp::Time &stamp,
    ...
    set_position_target_local_ned(
        ...
        ignore_all_except_xyz_y, p, Eigen::Vector3d::Zero(),
  ...
  void setpointg_cb(const geographic_msgs::msg::GeoPoseStamped::SharedPtr req) {
    ...
    set_position_target_global_int(
        get_time_boot_ms(req->header.stamp), uint8_t(MAV_FRAME::GLOBAL_INT),
        type_mask, req->pose.position.latitude * 1e7,
        req->pose.position.longitude * 1e7, req->pose.position.altitude,
   ...
   */
  void
  setpointg2l_cb(const geographic_msgs::msg::GeoPoseStamped::SharedPtr req) {
    /**
     * The idea is to convert the change in LLA(goal_gps-current_gps) to change
     * in ENU
    ...
    Eigen::Vector3d goal_gps(req->pose.position.latitude,
                             req->pose.position.longitude,
    ...
    earth.Forward(goal_gps.x(), goal_gps.y(), goal_gps.z(), goal_ecef.x(),
                  goal_ecef.y(), goal_ecef.z());

    // get ENU offset from ECEF offset
    Eigen::Vector3d ecef_offset = goal_ecef - current_ecef;
    Eigen::Vector3d enu_offset =
        ftf::transform_frame_ecef_enu(ecef_offset, current_gps);
