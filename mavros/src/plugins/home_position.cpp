...
private:
  ...
  void handle_home_position(const mavlink::mavlink_message_t *msg
    ...
    auto pos = ftf::transform_frame_ned_enu(
        Eigen::Vector3d(home_position.x, home_position.y, home_position.z));
    auto q = ftf::transform_orientation_ned_enu(
        ftf::mavlink_to_quaternion(home_position.q));
    auto hp_approach_enu = ftf::transform_frame_ned_enu(
        Eigen::Vector3d(home_position.approach_x, home_position.approach_y,
    ...
    RCLCPP_DEBUG(get_logger(), "HP: Home lat %f, long %f, alt %f",
                 hp.geo.latitude, hp.geo.longitude, hp.geo.altitude);
  ...
  void home_position_cb(const mavros_msgs::msg::HomePosition::SharedPtr req) {
    ...
    hp.altitude =
        req->geo.altitude * 1e3 + uas->data.ellipsoid_to_geoid_height(req->geo);
    // [[[cog:
  ...
  void req_update_cb(const std_srvs::srv::Trigger::Request::SharedPtr req
                     [[maybe_unused]],
