...
public:
  explicit WindEstimationPlugin(plugin::UASPtr uas_) : Plugin(uas_, "wind") {
...
private:
   ...
   */
  void handle_apm_wind(const mavlink::mavlink_message_t *msg [[maybe_unused]],
   ...
   */
  void handle_px4_wind(const mavlink::mavlink_message_t *msg [[maybe_unused]],
                       mavlink::common::msg::WIND_COV &wind,
                       plugin::filter::SystemAndOk filter [[maybe_unused]]) {
    auto twist_cov = geometry_msgs::msg::TwistWithCovarianceStamped();
    twist_cov.header.stamp = uas->synchronise_stamp(wind.time_usec);

    tf2::toMsg(ftf::transform_frame_ned_enu(
                   Eigen::Vector3d(wind.wind_x, wind.wind_y, wind.wind_z)),
               twist_cov.twist.twist.linear);

    // fill available covariance elements
    ftf::EigenMapCovariance6d cov_map(twist_cov.twist.covariance.data());
    cov_map.setZero();
    // NOTE: this is a summed covariance for both x and y horizontal wind
    // components
