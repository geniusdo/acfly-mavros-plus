 ...
 *
 * Receive trajectory setpoints and send setpoint_raw setpoints along the
 * trajectory.
 */
class SetpointTrajectoryPlugin
  ...
  explicit SetpointTrajectoryPlugin(plugin::UASPtr uas_)
    ...
    node_declare_and_watch_parameter(
        "mav_frame", "LOCAL_NED", [&](const rclcpp::Parameter &p) {
          ...
          if (new_mav_frame == MAV_FRAME::LOCAL_NED &&
              mav_frame_str != "LOCAL_NED") {
            throw rclcpp::exceptions::InvalidParameterValueException(
                utils::format("unknown MAV_FRAME: %s", mav_frame_str.c_str()));
          }
    ...
    local_sub = node->create_subscription<
        trajectory_msgs::msg::MultiDOFJointTrajectory>(
        "~/local", sensor_qos,
        std::bind(&SetpointTrajectoryPlugin::local_cb, this, _1));
    desired_pub =
        node->create_publisher<nav_msgs::msg::Path>("~/desired", sensor_qos);

    trajectory_reset_srv = node->create_service<std_srvs::srv::Trigger>(
        "~/reset",
...
private:
  ...
  using V_Point =
      std::vector<trajectory_msgs::msg::MultiDOFJointTrajectoryPoint>;
  ...
  trajectory_msgs::msg::MultiDOFJointTrajectory::SharedPtr
  ...
  void reset_timer(const std::chrono::nanoseconds dur) {
    ...
    sp_timer = node->create_wall_timer(
        dur, std::bind(&SetpointTrajectoryPlugin::reference_cb, this));
  }

  void publish_path(
      const trajectory_msgs::msg::MultiDOFJointTrajectory::SharedPtr req) {
  ...
  void
  local_cb(const trajectory_msgs::msg::MultiDOFJointTrajectory::SharedPtr req) {
  ...
  void reference_cb() {
    ...
    if (!setpoint_target->transforms.empty()) {
      position = ftf::detail::transform_static_frame(
          ftf::to_eigen(setpoint_target->transforms[0].translation), transform);
      attitude = ftf::detail::transform_orientation(
          ftf::to_eigen(setpoint_target->transforms[0].rotation), transform);
    ...
    if (!setpoint_target->velocities.empty()) {
      velocity = ftf::detail::transform_static_frame(
          ftf::to_eigen(setpoint_target->velocities[0].linear), transform);
    ...
    if (!setpoint_target->accelerations.empty()) {
      af = ftf::detail::transform_static_frame(
          ftf::to_eigen(setpoint_target->accelerations[0].linear), transform);
    ...
    set_position_target_local_ned(
        get_time_boot_ms(), utils::enum_value(mav_frame), type_mask, position,
        velocity, af, ftf::quaternion_get_yaw(attitude), yaw_rate);
  ...
  void reset_cb(std_srvs::srv::Trigger::Request::SharedPtr req [[maybe_unused]],
