...
static constexpr const char *set_parameters_atomically =
...
public:
  ...
  explicit Parameter(
      const std::string &param_id_, const uint16_t param_index_ = 0,
      const uint16_t param_count_ = 0,
      const rclcpp::ParameterValue &param_value_ = rclcpp::ParameterValue())
      : stamp{}, param_id(param_id_), param_value(param_value_),
        param_index(param_index_), param_count(param_count_) {}
  ...
  void set_value(mavlink::common::msg::PARAM_VALUE &pmsg) {
    ...
    default:
      RCLCPP_WARN(get_logger(), "PR: Unsupported param %.16s (%u/%u) type: %u",
  ...
  PARAM_SET to_param_set() const {
    ...
    default:
      RCLCPP_WARN_STREAM(get_logger(),
                         "PR: Unsupported ParameterValue type: "
                             << rclcpp::to_string(param_value.get_type()));
    }
  ...
  std::string to_string() const {
    ...
    return utils::format("%s (%u/%u): %s", param_id.c_str(), param_index,
                         param_count, pv.c_str());
  }
  ...
  rclcpp::Parameter to_rcl() const { return {param_id, param_value}; }
  ...
  static bool check_exclude_param_id(const std::string &param_id) {
    static const std::set<std::string> exclude_ids{
        "_HASH_CHECK",   "SYSID_SW_MREV",  "SYS_NUM_RESETS", "ARSPD_OFFSET",
        "GND_ABS_PRESS", "GND_ABS_PRESS2", "GND_ABS_PRESS3", "STAT_BOOTCNT",
        "STAT_FLTTIME",  "STAT_RESET",     "STAT_RUNTIME",   "GND_TEMP",
        "CMD_TOTAL",     "CMD_INDEX",      "LOG_LASTFILE",   "MIS_TOTAL",
...
public:
  ...
  ParamSetOpt(const Parameter &_p, size_t _rem)
      : param(_p), retries_remaining(_rem) {}
...
public:
  explicit ParamPlugin(plugin::UASPtr uas_)
      : Plugin(uas_, "param",
        ...
        BOOTUP_TIME(10s), LIST_TIMEOUT(30s), PARAM_TIMEOUT(1s),
        RETRIES_COUNT(3), param_count(-1), param_state(PR::IDLE),
    ...
    param_event_pub = node->create_publisher<mavros_msgs::msg::ParamEvent>(
        "~/event", event_qos);
    std_event_pub = node->create_publisher<rcl_interfaces::msg::ParameterEvent>(
        PSN::events, event_qos);

    // Custom parameter services
    pull_srv = node->create_service<mavros_msgs::srv::ParamPull>(
        "~/pull", std::bind(&ParamPlugin::pull_cb, this, _1, _2), qos);
    set_srv = node->create_service<mavros_msgs::srv::ParamSetV2>(
        "~/set", std::bind(&ParamPlugin::set_cb, this, _1, _2), qos);
    ...
    set_parameters_atomically_srv =
        node->create_service<rcl_interfaces::srv::SetParametersAtomically>(
            ...
            std::bind(&ParamPlugin::set_parameters_atomically_cb, this, _1, _2),
            qos);
    describe_parameters_srv =
    ...
    list_parameters_srv =
        node->create_service<rcl_interfaces::srv::ListParameters>(
            PSN::list_parameters,
            std::bind(&ParamPlugin::list_parameters_cb, this, _1, _2), qos);

    schedule_timer = node->create_wall_timer(
        BOOTUP_TIME, std::bind(&ParamPlugin::schedule_cb, this));
    schedule_timer->cancel();

    timeout_timer = node->create_wall_timer(
        PARAM_TIMEOUT, std::bind(&ParamPlugin::timeout_cb, this));
...
private:
  ...
  rclcpp::Service<rcl_interfaces::srv::GetParameters>::SharedPtr
      get_parameters_srv;
  rclcpp::Service<rcl_interfaces::srv::GetParameterTypes>::SharedPtr
      get_parameter_types_srv;
  rclcpp::Service<rcl_interfaces::srv::SetParameters>::SharedPtr
      set_parameters_srv;
  ...
  rclcpp::Service<rcl_interfaces::srv::DescribeParameters>::SharedPtr
      describe_parameters_srv;
  rclcpp::Service<rcl_interfaces::srv::ListParameters>::SharedPtr
      list_parameters_srv;
  ...
  void handle_param_value(const mavlink::mavlink_message_t *msg
    ...
    auto update_parameter = [this, &pmsg](Parameter &p, bool is_new) {
      ...
      RCLCPP_WARN_STREAM_EXPRESSION(
          ...
          "PR: Param " << p.to_string() << " different index: "
    ...
    if (param_it != parameters.end()) {
      ...
      // insert new element
      auto pp = parameters.emplace(
          param_id, Parameter(param_id, pmsg.param_index, pmsg.param_count));
    ...
    if (param_state == PR::RXLIST || param_state == PR::RXPARAM ||
      ...
      if (param_state == PR::RXLIST) {
        ...
        if (param_count != UINT16_MAX) {
          ...
          RCLCPP_WARN(lg, "PR: FCU does not know index for first element! "
      ...
      // Issue #276
      bool it_is_first_requested =
          parameters_missing_idx.front() == pmsg.param_index;
      ...
      if (parameters_missing_idx.empty()) {
        ...
        RCLCPP_DEBUG(lg, "PR: requesting next timed out parameter idx=%u",
                     first_miss_idx);
  ...
  void schedule_pull() { schedule_timer->reset(); }
  ...
  void timeout_cb() {
    ...
    if (param_state == PR::RXLIST && param_rx_retries > 0) {
      param_rx_retries--;
      RCLCPP_WARN(lg, "PR: request list timeout, retries left %zu",
                  param_rx_retries);
      ...
      if (param_rx_retries > 0) {
        param_rx_retries--;
        RCLCPP_WARN(lg,
                    "PR: request param #%u timeout, retries left %zu, and %zu "
                    "params still missing",
                    first_miss_idx, param_rx_retries,
                    parameters_missing_idx.size());
        restart_timeout_timer();
        param_request_read("", first_miss_idx);

      } else {
        RCLCPP_ERROR(lg, "PR: request param #%u completely missing.",
                     first_miss_idx);
      ...
      if (it->second->retries_remaining > 0) {
        ...
        RCLCPP_WARN(lg, "PR: Resend param set for %s, retries left %zu",
        ...
        RCLCPP_ERROR(lg, "PR: Param set for %s timed out.",
                     it->second->param.param_id.c_str());
  ...
  bool wait_fetch_all() {
    ...
    return list_receiving.wait_for(lock, LIST_TIMEOUT) ==
               std::cv_status::no_timeout &&
           !is_timedout;
  }

  ParamSetOpt::Result
  wait_param_set_ack_for(const std::shared_ptr<ParamSetOpt> opt) {
   ...
   * @service ~param/pull
   */
  void pull_cb(const mavros_msgs::srv::ParamPull::Request::SharedPtr req,
               mavros_msgs::srv::ParamPull::Response::SharedPtr res) {
    unique_lock lock(mutex);

    auto is_in_progress = [&]() -> bool {
      return param_state == PR::RXLIST || param_state == PR::RXPARAM ||
             param_state == PR::RXPARAM_TIMEDOUT;
    };

    if ((param_state == PR::IDLE && parameters.empty()) || req->force_pull) {
      if (!req->force_pull) {
   ...
   * @service ~param/set
   */
  void set_cb(const mavros_msgs::srv::ParamSetV2::Request::SharedPtr req,
    ...
    if (Parameter::check_exclude_param_id(req->param_id) && !req->force_set) {
      RCLCPP_INFO_STREAM(get_logger(),
                         "PR: parameter set excluded: " << req->param_id);
      res->success = false;
      return;
    }
    ...
    if (to_send.param_value.get_type() == rclcpp::PARAMETER_NOT_SET &&
      ...
      RCLCPP_ERROR_STREAM(get_logger(),
                          "PR: Unknown parameter to set: " << req->param_id);
      res->success = false;
      return;
    }
  ...
  void get_parameters_cb(
    ...
    for (auto name : req->names) {
      ...
      if (it == parameters.end()) {
        RCLCPP_WARN_STREAM(get_logger(),
                           "PR: Failed to get parameter type: " << name);
  ...
  void set_parameters_cb(
    ...
    for (const auto &p : req->parameters) {
      ...
      if (Parameter::check_exclude_param_id(p.name)) {
        RCLCPP_WARN_STREAM(get_logger(),
                           "PR: parameter set excluded: " << p.name);
      ...
      if (to_send.param_value.get_type() == rclcpp::PARAMETER_NOT_SET) {
        RCLCPP_ERROR_STREAM(get_logger(),
                            "PR: Unknown parameter to set: " << p.name);
  ...
  void set_parameters_atomically_cb(
      const rcl_interfaces::srv::SetParametersAtomically::Request::SharedPtr req
      [[maybe_unused]],
      rcl_interfaces::srv::SetParametersAtomically::Response::SharedPtr res) {
    RCLCPP_ERROR(get_logger(),
                 "PR: Client calls unsupported ~/set_parameters_atomically");
  ...
  void describe_parameters_cb(
    ...
    for (auto name : req->names) {
      ...
      if (it == parameters.end()) {
        RCLCPP_WARN_STREAM(get_logger(),
                           "PR: Failed to describe parameters: " << name);
  ...
  void list_parameters_cb(
      const rcl_interfaces::srv::ListParameters::Request::SharedPtr req
      [[maybe_unused]],
      rcl_interfaces::srv::ListParameters::Response::SharedPtr res) {
    unique_lock lock(mutex);

    // NOTE(vooon): all fcu parameters are "flat", so no need to check req
    // prefixes or depth
