...
public:
  explicit WaypointPlugin(plugin::UASPtr uas_)
      : MissionBase(uas_, "mission"), enable_partial_push_auto(true) {
    ...
    // NOTE(vooon): I'm not quite sure that this option would work with mavros
    // router
    node_declare_and_watch_parameter(
        "pull_after_gcs", true,
        [&](const rclcpp::Parameter &p) { do_pull_after_gcs = p.as_bool(); });

    node_declare_and_watch_parameter("use_mission_item_int", true,
                                     [&](const rclcpp::Parameter &p) {
                                       use_mission_item_int = p.as_bool();
                                     });

    node_declare_and_watch_parameter(
        "enable_partial_push", 2,
        [&](const rclcpp::Parameter &p) {
          RCLCPP_DEBUG_STREAM(get_logger(),
                              log_prefix << ": enable_partial_push = " << p);
    ...
    pull_srv = node->create_service<mavros_msgs::srv::WaypointPull>(
        "~/pull", std::bind(&WaypointPlugin::pull_cb, this, _1, _2));
    push_srv = node->create_service<mavros_msgs::srv::WaypointPush>(
        "~/push", std::bind(&WaypointPlugin::push_cb, this, _1, _2));
    clear_srv = node->create_service<mavros_msgs::srv::WaypointClear>(
        "~/clear", std::bind(&WaypointPlugin::clear_cb, this, _1, _2));
    set_cur_srv = node->create_service<mavros_msgs::srv::WaypointSetCurrent>(
        "~/set_current", std::bind(&WaypointPlugin::set_cur_cb, this, _1, _2));
...
private:
  ...
  rclcpp::Publisher<mavros_msgs::msg::WaypointReached>::SharedPtr
      wp_reached_pub;
  ...
  bool detect_partial_push() { return uas->is_ardupilotmega(); }

  /* -*- ROS callbacks -*- */

  void pull_cb(const mavros_msgs::srv::WaypointPull::Request::SharedPtr req
               [[maybe_unused]],
  ...
  void push_cb(const mavros_msgs::srv::WaypointPush::Request::SharedPtr req,
    ...
    if (req->start_index) {
      ...
      if (!enable_partial_push) {
        RCLCPP_WARN(get_logger(),
                    "%s: Partial Push not enabled. (Only supported on APM)",
                    log_prefix);
        res->success = false;
        res->wp_transfered = 0;
        return;
      ...
      if (waypoints.size() < req->start_index + req->waypoints.size()) {
        RCLCPP_WARN(get_logger(), "%s: Partial push out of range rejected.",
                    log_prefix);
        res->success = false;
        res->wp_transfered = 0;
        return;
  ...
  void clear_cb(const mavros_msgs::srv::WaypointClear::Request::SharedPtr req
                [[maybe_unused]],
  ...
  void
  set_cur_cb(const mavros_msgs::srv::WaypointSetCurrent::Request::SharedPtr req,
