...
public:
  explicit RallypointPlugin(plugin::UASPtr uas_)
    ...
    pull_srv = node->create_service<mavros_msgs::srv::WaypointPull>(
        "~/pull", std::bind(&RallypointPlugin::pull_cb, this, _1, _2));
    push_srv = node->create_service<mavros_msgs::srv::WaypointPush>(
        "~/push", std::bind(&RallypointPlugin::push_cb, this, _1, _2));
    clear_srv = node->create_service<mavros_msgs::srv::WaypointClear>(
        "~/clear", std::bind(&RallypointPlugin::clear_cb, this, _1, _2));
  ...
  void push_cb(const mavros_msgs::srv::WaypointPush::Request::SharedPtr req,
    ...
    if (req->start_index) {
      ...
      RCLCPP_WARN(get_logger(),
                  "%s: Partial update for rallypoints not supported",
                  log_prefix);
      res->success = false;
      return;
    }
