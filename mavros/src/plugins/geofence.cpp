...
public:
  explicit GeofencePlugin(plugin::UASPtr uas_)
      : MissionBase(uas_, "geofence", plugin::MTYPE::FENCE, "GF", 25s) {
    ...
    pull_srv = node->create_service<mavros_msgs::srv::WaypointPull>(
        "~/pull", std::bind(&GeofencePlugin::pull_cb, this, _1, _2));
    push_srv = node->create_service<mavros_msgs::srv::WaypointPush>(
        "~/push", std::bind(&GeofencePlugin::push_cb, this, _1, _2));
    clear_srv = node->create_service<mavros_msgs::srv::WaypointClear>(
        "~/clear", std::bind(&GeofencePlugin::clear_cb, this, _1, _2));
...
private:
  rclcpp::Publisher<mavros_msgs::msg::WaypointList>::SharedPtr gf_list_pub;
  ...
  void push_cb(const mavros_msgs::srv::WaypointPush::Request::SharedPtr req,
    ...
    if (req->start_index) {
      ...
      RCLCPP_WARN(get_logger(),
                  "%s: Partial update for fence points not supported",
                  log_prefix);
      res->success = false;
      return;
    }
...
MAVROS_PLUGIN_REGISTER(mavros::std_plugins::GeofencePlugin)
