...
public:
  explicit LocalPositionPlugin(plugin::UASPtr uas_)
    ...
    local_position = node->create_publisher<geometry_msgs::msg::PoseStamped>(
        "~/pose", sensor_qos);
    local_position_cov =
    ...
    local_velocity_body =
        node->create_publisher<geometry_msgs::msg::TwistStamped>(
            "~/velocity_body", sensor_qos);
    local_velocity_cov =
        node->create_publisher<geometry_msgs::msg::TwistWithCovarianceStamped>(
            "~/velocity_body_cov", sensor_qos);
    local_accel =
        node->create_publisher<geometry_msgs::msg::AccelWithCovarianceStamped>(
            "~/accel", sensor_qos);
    local_odom =
        node->create_publisher<nav_msgs::msg::Odometry>("~/odom", sensor_qos);
  }
...
private:
  ...
  void handle_local_position_ned(
    ...
    auto enu_velocity = ftf::transform_frame_ned_enu(
        Eigen::Vector3d(pos_ned.vx, pos_ned.vy, pos_ned.vz));
    ...
    auto baselink_linear = ftf::transform_frame_enu_baselink(
        enu_velocity, enu_orientation.inverse());
    ...
    tf2::toMsg(ftf::transform_frame_baselink_enu(
                   ftf::to_eigen(baselink_angular_msg), enu_orientation),
  ...
  void handle_local_position_ned_cov(
    ...
    // TODO(vooon): orientation + angular velocity covariances from
    // ATTITUDE_QUATERION_COV
    ...
    auto enu_accel = ftf::transform_frame_ned_enu(
        Eigen::Vector3d(pos_ned.ax, pos_ned.ay, pos_ned.az));
