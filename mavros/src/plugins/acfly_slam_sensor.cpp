 ...
 */
class AcflySlamSensorPlugin
    : public plugin::AcflyPositionSensorBase,
      private plugin::TF2ListenerMixin<AcflySlamSensorPlugin> {
public:
  explicit AcflySlamSensorPlugin(plugin::UASPtr uas_)
      : AcflyPositionSensorBase(uas_, "~acfly_slam_sensor"), tf_rate(10.0) {
    ...
    node_declare_and_watch_parameter(
        "sensor_id", "camera",
        [&](const rclcpp::Parameter &p) { sensor_id = p.as_string(); });

    node_declare_and_watch_parameter(
        "body_id", "base_link",
        [&](const rclcpp::Parameter &p) { body_id = p.as_string(); });
    ...
    if (node->get_parameter("sensor_body_rotation", rot)) {
      tr_sensor_body =
          Eigen::Affine3d(ftf::quaternion_from_rpy(rot[0], rot[1], rot[2]));
    } else {
      tr_sensor_body = Eigen::Affine3d(ftf::quaternion_from_rpy(0, 0, 0));
      RCLCPP_WARN("acfly_slam_sensor",
                  "ASS: No rotation parameter between sensor and body, set to "
                  "default(0, 0, 0)");
    }
    if (node->get_parameter("sensor_body_translation", trans)) {
      tr_sensor_body.translation() =
          Eigen::Vector3d(trans[0], trans[1], trans[2]);
    } else {
      tr_sensor_body.translation() = Eigen::Vector3d(0, 0, 0);
      RCLCPP_WARN("acfly_slam_sensor",
                  "ASS: No translation parameter between sensor and body, set "
                  "to default(0, 0, 0)");
    }
    ...
    node_declare_and_watch_parameter(
        "tf/listen", true, [&](const rclcpp::Parameter &p) { tf_listen = p; });
    node_declare_and_watch_parameter(
        "tf/rate_limit", 10.0,
        [&](const rclcpp::Parameter &p) { tf_rate = p; });
    ...
    uas->add_static_transform(sensor_id, body_id, tr_sensor_body,
                              transform_vector);
    ...
    if (tf_listen) {
      ...
      pose_cov_sub =
          node->create_subscription<geometry_msgs::PoseWithCovarianceStamped>(
              "pose_cov", 10,
              std::bind(&AcflySlamSensorPlugin::pose_cov_cb, this, _1));
    }
  ...
  Subscriptions get_subscriptions() override { return {/* 禁用接收 */}; }

private:
  friend class TF2ListenerMixin;

  rclcpp::Subscription<geometry_msgs::PoseStamped>::SharedPtr pose_sub;
  rclcpp::Subscription<geometry_msgs::PoseWithCovarianceStamped>::SharedPtr
      pose_cov_sub;
  ...
  void pose_cov_cb(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr
                       &pose_cov_stamp) {
    if (!uas->get_pos_sensor_connection_status(sensor_ind)) {
      register_position_sensor();
    } else {
...
PLUGINLIB_EXPORT_CLASS(mavros::std_plugins::AcflySlamSensorPlugin,
