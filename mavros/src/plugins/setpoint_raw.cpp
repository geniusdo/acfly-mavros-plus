...
public:
  explicit SetpointRawPlugin(plugin::UASPtr uas_)
      : Plugin(uas_, "setpoint_raw"), thrust_scaling(1.0) {
    enable_node_watch_parameters();

    node_declare_and_watch_parameter(
        "thrust_scaling", NAN,
        [&](const rclcpp::Parameter &p) { thrust_scaling = p.as_double(); });

    auto sensor_qos = rclcpp::SensorDataQoS();

    local_sub = node->create_subscription<mavros_msgs::msg::PositionTarget>(
        "~/local", sensor_qos,
        std::bind(&SetpointRawPlugin::local_cb, this, _1));
    global_sub =
        node->create_subscription<mavros_msgs::msg::GlobalPositionTarget>(
            "~/global", sensor_qos,
            std::bind(&SetpointRawPlugin::global_cb, this, _1));
    attitude_sub = node->create_subscription<mavros_msgs::msg::AttitudeTarget>(
        "~/attitude", sensor_qos,
        std::bind(&SetpointRawPlugin::attitude_cb, this, _1));

    target_local_pub = node->create_publisher<mavros_msgs::msg::PositionTarget>(
        "~/target_local", sensor_qos);
    target_global_pub =
        node->create_publisher<mavros_msgs::msg::GlobalPositionTarget>(
            "~/target_global", sensor_qos);
    target_attitude_pub =
        node->create_publisher<mavros_msgs::msg::AttitudeTarget>(
...
private:
  ...
  rclcpp::Subscription<mavros_msgs::msg::GlobalPositionTarget>::SharedPtr
      global_sub;
  ...
  rclcpp::Publisher<mavros_msgs::msg::PositionTarget>::SharedPtr
      target_local_pub;
  rclcpp::Publisher<mavros_msgs::msg::GlobalPositionTarget>::SharedPtr
      target_global_pub;
  ...
  void handle_position_target_local_ned(
    ...
    auto position =
        ftf::transform_frame_ned_enu(Eigen::Vector3d(tgt.x, tgt.y, tgt.z));
    auto velocity =
        ftf::transform_frame_ned_enu(Eigen::Vector3d(tgt.vx, tgt.vy, tgt.vz));
    auto af = ftf::transform_frame_ned_enu(
        Eigen::Vector3d(tgt.afx, tgt.afy, tgt.afz));
    float yaw =
        ftf::quaternion_get_yaw(ftf::transform_orientation_aircraft_baselink(
  ...
  void handle_attitude_target(const mavlink::mavlink_message_t *msg
    ...
    auto body_rate = ftf::transform_frame_baselink_aircraft(Eigen::Vector3d(
        tgt.body_roll_rate, tgt.body_pitch_rate, tgt.body_yaw_rate));
  ...
  void local_cb(const mavros_msgs::msg::PositionTarget::SharedPtr req) {
    ...
    set_position_target_local_ned(get_time_boot_ms(req->header.stamp),
                                  req->coordinate_frame, req->type_mask,
                                  position, velocity, af, yaw, yaw_rate);
  }
  ...
  void global_cb(const mavros_msgs::msg::GlobalPositionTarget::SharedPtr req) {
    ...
    velocity = ftf::transform_frame_enu_ned(velocity);
    af = ftf::transform_frame_enu_ned(af);
    yaw = ftf::quaternion_get_yaw(ftf::transform_orientation_aircraft_baselink(
        ftf::transform_orientation_ned_enu(
            ftf::quaternion_from_rpy(0.0, 0.0, req->yaw))));
    Eigen::Vector3d ang_vel_enu(0.0, 0.0, req->yaw_rate);
    auto ang_vel_ned = ftf::transform_frame_ned_enu(ang_vel_enu);
    ...
    set_position_target_global_int(get_time_boot_ms(req->header.stamp),
                                   req->coordinate_frame, req->type_mask,
                                   req->latitude * 1e7, req->longitude * 1e7,
                                   req->altitude, velocity, af, yaw, yaw_rate);
  }
  ...
  void attitude_cb(const mavros_msgs::msg::AttitudeTarget::SharedPtr req) {
    ...
    // ignore thrust is false by default, unless no thrust scaling is set or
    // thrust is zero
    auto ignore_thrust = req->thrust != 0.0 && std::isnan(thrust_scaling);

    if (ignore_thrust) {
      // I believe it's safer without sending zero thrust, but actually ignoring
      // the actuation.
      RCLCPP_ERROR_THROTTLE(
          get_logger(), *get_clock(), 5000,
          "Recieved thrust, but ignore_thrust is true: "
          "the most likely cause of this is a failure to specify the "
          "thrust_scaling parameters "
      ...
      body_rate =
          ftf::transform_frame_baselink_aircraft(ftf::to_eigen(req->body_rate));

      set_attitude_target(get_time_boot_ms(req->header.stamp), req->type_mask,
