...
public:
  explicit ActuatorControlPlugin(plugin::UASPtr uas_)
    ...
    target_actuator_control_pub =
        node->create_publisher<mavros_msgs::msg::ActuatorControl>(
            "target_actuator_control", sensor_qos);
    actuator_control_sub =
        node->create_subscription<mavros_msgs::msg::ActuatorControl>(
            "actuator_control", sensor_qos,
            std::bind(&ActuatorControlPlugin::actuator_control_cb, this, _1));
  }
...
private:
  rclcpp::Publisher<mavros_msgs::msg::ActuatorControl>::SharedPtr
      target_actuator_control_pub;
  rclcpp::Subscription<mavros_msgs::msg::ActuatorControl>::SharedPtr
      actuator_control_sub;
  ...
  void
  actuator_control_cb(const mavros_msgs::msg::ActuatorControl::SharedPtr req) {
    //! about groups, mixing and channels: @p https://pixhawk.org/dev/mixing
    //! message definiton here: @p
