...
public:
  HeartbeatStatus(const std::string &name, size_t win_size)
      : diagnostic_updater::DiagnosticTask(name), times_(win_size),
        seq_nums_(win_size), window_size_(win_size), min_freq_(0.2),
        max_freq_(100), tolerance_(0.1), autopilot(MAV_AUTOPILOT::GENERIC),
        type(MAV_TYPE::GENERIC), system_status(MAV_STATE::UNINIT) {
    clear();
  }
  ...
  void tick(uint8_t type_, uint8_t autopilot_, std::string &mode_,
...
public:
  ...
  void run(diagnostic_updater::DiagnosticStatusWrapper &stat) {
    ...
    if ((last_st.onboard_control_sensors_health &
    ...
    stat.addf("Sensor present", "0x%08X",
              last_st.onboard_control_sensors_present);
    stat.addf("Sensor enabled", "0x%08X",
              last_st.onboard_control_sensors_enabled);
    stat.addf("Sensor health", "0x%08X",
    ...
    auto check_flag = [&](const std::string &name, STS flag) {
      if (last_st.onboard_control_sensors_enabled & enum_value(flag)) {
        stat.add(name,
                 (last_st.onboard_control_sensors_health & enum_value(flag))
                     ? "Ok"
                     : "Fail");
      }
    };
    ...
    check_flag("external ground truth (Vicon or Leica)",
               STS::EXTERNAL_GROUND_TRUTH);
    ...
    check_flag("pre-arm check status. Always healthy when armed",
               STS::PREARM_CHECK);
    check_flag("Avoidance/collision prevention", STS::OBSTACLE_AVOIDANCE);
    check_flag("propulsion (actuator, esc, motor or propellor)",
               STS::PROPULSION);
...
public:
  explicit BatteryStatusDiag(const std::string &name)
      : diagnostic_updater::DiagnosticTask(name), voltage(-1.0f), current(0.0f),
        remaining(0.0f), min_voltage(6.0f) {}

  // Move constructor, required to dynamically create an array of instances of
  // this class because it contains an unique mutex object
  BatteryStatusDiag(BatteryStatusDiag &&other) noexcept
      : diagnostic_updater::DiagnosticTask(""), voltage(-1.0f), current(0.0f),
        remaining(0.0f), min_voltage(6.0f) {
    *this = std::move(other);
  }

  // Move assignment operator, required to dynamically create an array of
  // instances of this class because it contains an unique mutex object
...
public:
  MemInfo(const std::string &name)
      : diagnostic_updater::DiagnosticTask(name), freemem(UINT32_MAX),
        brkval(0), last_rcd(0) {}
...
public:
  HwStatus(const std::string &name)
      : diagnostic_updater::DiagnosticTask(name), vcc(-1.0), i2cerr(0),
        i2cerr_last(0), last_rcd(0) {}
...
public:
  explicit SystemStatusPlugin(plugin::UASPtr uas_)
      : Plugin(uas_, "sys"), hb_diag("Heartbeat", 10), mem_diag("APM Memory"),
        hwst_diag("APM Hardware"), sys_diag("System"),
        conn_heartbeat_mav_type(MAV_TYPE::ONBOARD_CONTROLLER),
        version_retries(RETRIES_COUNT), disable_diag(false),
    ...
    node_declare_and_watch_parameter(
        "heartbeat_mav_type", utils::enum_to_name(conn_heartbeat_mav_type),
        [&](const rclcpp::Parameter &p) {
    ...
    state_pub =
        node->create_publisher<mavros_msgs::msg::State>("state", state_qos);
    extended_state_pub =
        node->create_publisher<mavros_msgs::msg::ExtendedState>(
            "extended_state", state_qos);
    estimator_status_pub =
        node->create_publisher<mavros_msgs::msg::EstimatorStatus>(
    ...
    mode_srv = node->create_service<mavros_msgs::srv::SetMode>(
        "set_mode", std::bind(&SystemStatusPlugin::set_mode_cb, this, _1, _2),
    ...
    message_interval_srv =
        node->create_service<mavros_msgs::srv::MessageInterval>(
            "set_message_interval",
            std::bind(&SystemStatusPlugin::set_message_interval_cb, this, _1,
                      _2),
            rmw_qos_profile_services_default, srv_cg);
    vehicle_info_get_srv =
        node->create_service<mavros_msgs::srv::VehicleInfoGet>(
            "vehicle_info_get",
            std::bind(&SystemStatusPlugin::vehicle_info_get_cb, this, _1, _2),
            rmw_qos_profile_services_default, srv_cg);

    uas->diagnostic_updater.add(hb_diag);

    autopilot_version_timer = node->create_wall_timer(
        1s, std::bind(&SystemStatusPlugin::autopilot_version_cb, this), srv_cg);
...
private:
  ...
  rclcpp::Publisher<mavros_msgs::msg::ExtendedState>::SharedPtr
      extended_state_pub;
  rclcpp::Publisher<mavros_msgs::msg::EstimatorStatus>::SharedPtr
      estimator_status_pub;
  ...
  rclcpp::Service<mavros_msgs::srv::MessageInterval>::SharedPtr
      message_interval_srv;
  rclcpp::Service<mavros_msgs::srv::SetMode>::SharedPtr mode_srv;
  rclcpp::Service<mavros_msgs::srv::VehicleInfoGet>::SharedPtr
      vehicle_info_get_srv;
  ...
  using M_VehicleInfo =
      std::unordered_map<uint16_t, mavros_msgs::msg::VehicleInfo>;
  ...
  inline M_VehicleInfo::iterator find_or_create_vehicle_info(uint8_t sysid,
                                                             uint8_t compid) {
  ...
  void process_statustext_normal(uint8_t severity, const std::string &text) {
    ...
    //     cog.outl(f"  RCLCPP_{l2}_STREAM(node->get_logger(), \"FCU: \" <<
    //     text);") cog.outl(f"  break;")
    // ]]]
    ...
    default:
      RCLCPP_WARN_STREAM(node->get_logger(),
                         "FCU: UNK(" << +severity << "): " << text);
      break;
    }
  }

  static std::string
  custom_version_to_hex_string(const std::array<uint8_t, 8> &array) {
  ...
  void
  process_autopilot_version_normal(mavlink::common::msg::AUTOPILOT_VERSION &apv,
  ...
  void process_autopilot_version_apm_quirk(
    ...
    log_info("%s: Capabilities         0x%016llx", apv.capabilities);
    log_info("%s: Flight software:     %08x (%*s)", apv.flight_sw_version, 8,
             apv.flight_custom_version.data());
    log_info("%s: Middleware software: %08x (%*s)", apv.middleware_sw_version,
             8, apv.middleware_custom_version.data());
    log_info("%s: OS software:         %08x (%*s)", apv.os_sw_version, 8,
  ...
  void handle_heartbeat(const mavlink::mavlink_message_t *msg,
    ...
    // XXX(vooon): i assume that UAS not interested in HBs from non-target
    // system.
    ...
    it->second.available_info |=
        mavros_msgs::msg::VehicleInfo::HAVE_INFO_HEARTBEAT;
    ...
    if (!uas->is_my_target(msg->sysid, msg->compid)) {
      RCLCPP_DEBUG(node->get_logger(), "HEARTBEAT from %d.%d dropped.",
    ...
    state_msg.armed =
        !!(hb.base_mode & enum_value(MAV_MODE_FLAG::SAFETY_ARMED));
    state_msg.guided =
        !!(hb.base_mode & enum_value(MAV_MODE_FLAG::GUIDED_ENABLED));
  ...
  void handle_sys_status(const mavlink::mavlink_message_t *msg [[maybe_unused]],
    ...
    batt_msg.power_supply_technology =
        BatteryMsg::POWER_SUPPLY_TECHNOLOGY_UNKNOWN;
    batt_msg.present = true;
    batt_msg.cell_voltage
        .clear(); // not necessary. Cell count and Voltage unknown.
    batt_msg.location = "";
  ...
  void handle_statustext(const mavlink::mavlink_message_t *msg [[maybe_unused]],
  ...
  void handle_meminfo(const mavlink::mavlink_message_t *msg [[maybe_unused]],
                      mavlink::ardupilotmega::msg::MEMINFO &mem,
                      plugin::filter::SystemAndOk filter [[maybe_unused]]) {
    mem_diag.set(std::max(static_cast<uint32_t>(mem.freemem), mem.freemem32),
                 mem.brkval);
  }

  void handle_hwstatus(const mavlink::mavlink_message_t *msg [[maybe_unused]],
  ...
  void handle_autopilot_version(const mavlink::mavlink_message_t *msg,
    ...
    it->second.available_info |=
        mavros_msgs::msg::VehicleInfo::HAVE_INFO_AUTOPILOT_VERSION;
    ...
    it->second.flight_custom_version =
        custom_version_to_hex_string(apv.flight_custom_version);
  ...
  void handle_battery_status(const mavlink::mavlink_message_t *msg
    ...
    case enum_value(BT::LIPO):
      batt_msg.power_supply_technology =
          BatteryMsg::POWER_SUPPLY_TECHNOLOGY_LIPO;
      break;
    ...
    case enum_value(BT::LION):
      batt_msg.power_supply_technology =
          BatteryMsg::POWER_SUPPLY_TECHNOLOGY_LION;
      break;
    ...
    constexpr float coalesce_voltage =
        (UINT16_MAX - 1) *
        0.001f; // 65,534V cell voltage means that the next element in the array
                // must be added to this one
    ...
    for (auto v : bs.voltages_ext) {
      if (v == UINT16_MAX ||
          v == 0) { // this one is different from the for loop above to support
                    // mavlink2 message truncation
        break;
      }
    ...
    if (!disable_diag && bs.id >= 0 && bs.id < MAX_NR_BATTERY_STATUS) {
      batt_diag[bs.id].set(total_voltage, batt_msg.current,
                           batt_msg.percentage);
  ...
  void handle_estimator_status(const mavlink::mavlink_message_t *msg
    ...
    //     if esf[0].isdigit():
    //         esf = 'SENSOR_' + esf
    //     cog.outl(f"est_status_msg.{esf.lower()}_status_flag =
    //     check_flag(ESF::{esf});")
    // ]]]
    ...
    est_status_msg.pred_pos_horiz_rel_status_flag =
        check_flag(ESF::PRED_POS_HORIZ_REL);
    est_status_msg.pred_pos_horiz_abs_status_flag =
        check_flag(ESF::PRED_POS_HORIZ_ABS);
  ...
  void timeout_cb() { uas->update_connection_status(false); }
  ...
  void autopilot_version_cb() {
    ...
    try {
      auto client =
          node->create_client<mavros_msgs::srv::CommandLong>("cmd/command");
      ...
      RCLCPP_DEBUG(lg, "VER: Sending %s request.",
    ...
    RCLCPP_ERROR_EXPRESSION(lg, !ret,
                            "VER: command plugin service call failed!");

    if (version_retries > 0) {
      version_retries--;
      RCLCPP_WARN_EXPRESSION(lg, version_retries != RETRIES_COUNT - 1,
      ...
      autopilot_version_timer->cancel();
      RCLCPP_WARN(lg, "VER: your FCU don't support AUTOPILOT_VERSION, "
  ...
  void set_rate_cb(const mavros_msgs::srv::StreamRate::Request::SharedPtr req,
  ...
  void set_message_interval_cb(
    ...
    try {
      ...
      RCLCPP_DEBUG(lg, "SYS: Request msgid %u at %f hz", req->message_id,
                   req->message_rate);
    ...
    RCLCPP_ERROR_EXPRESSION(lg, !res->success,
                            "SYS: command plugin service call failed!");
  }

  void set_mode_cb(const mavros_msgs::srv::SetMode::Request::SharedPtr req,
