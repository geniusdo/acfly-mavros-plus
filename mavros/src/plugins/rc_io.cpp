...
public:
  explicit RCIOPlugin(plugin::UASPtr uas_)
      : Plugin(uas_, "rc"), raw_rc_in(0), raw_rc_out(0),
    ...
    override_sub = node->create_subscription<mavros_msgs::msg::OverrideRCIn>(
        "~/override", 10, std::bind(&RCIOPlugin::override_cb, this, _1));
...
private:
  ...
  void handle_rc_channels(const mavlink::mavlink_message_t *msg
    ...
    RCLCPP_INFO_EXPRESSION(get_logger(), !has_rc_channels_msg.exchange(true),
    ...
    //     cog.outl(f"case {i}: raw_rc_in[{i - 1}] = channels.chan{i}_raw;
    //     [[fallthrough]];")
    // ]]]
    case 18:
