...
public:
  ...
  MOCK_METHOD3(send_message, void(const mavlink_message_t *msg,
                                  const Framing framing, id_t src_id));
  MOCK_METHOD2(recv_message,
               void(const mavlink_message_t *msg, const Framing framing));
...
public:
  ...
  Router::SharedPtr create_node() {
    ...
    auto make_and_add_mock_endpoint = [router](id_t id, const std::string &url,
                                               LT type,
      ...
      // XXX(vooon): another strange thing: mock reports about not freed mock
      // objects.
    ...
    make_and_add_mock_endpoint(1000, "mock://fcu1", LT::fcu,
                               {0x0000, 0x0100, 0x0101});
    make_and_add_mock_endpoint(1001, "mock://fcu2", LT::fcu,
                               {0x0000, 0x0200, 0x0201});
    make_and_add_mock_endpoint(1002, "/uas1", LT::uas,
                               {0x0000, 0x0100, 0x01BF});
    make_and_add_mock_endpoint(1003, "/uas2", LT::uas,
                               {0x0000, 0x0200, 0x02BF});
    make_and_add_mock_endpoint(1004, "mock://gcs1", LT::gcs,
                               {0x0000, 0xFF00, 0xFFBE});
    make_and_add_mock_endpoint(1005, "mock://gcs2", LT::gcs,
                               {0x0000, 0xFF00, 0xFFBD});
  ...
  std::unordered_map<id_t, Endpoint::SharedPtr> &
  get_endpoints(Router::SharedPtr router) {
    return router->endpoints;
  }

  mavlink_message_t convert_message(const mavlink::Message &msg,
                                    const addr_t source = 0x0101) {
    ...
    //   std::cout <<
    //     utils::format("byte: %2zu: %02x", i, (_MAV_PAYLOAD(&ret)[i] & 0xff))
    //     << std::endl;
    // }
...
TEST_F(TestRouter, set_parameter) {
  ...
  router->set_parameters(std::vector<rclcpp::Parameter>{
      rclcpp::Parameter("fcu_urls", std::vector<std::string>{"udp://:45001@",
                                                             "tcp-l://:57601"}),
      rclcpp::Parameter("gcs_urls", std::vector<std::string>{"udp://:45002@",
                                                             "tcp-l://:57602"}),
      rclcpp::Parameter("uas_urls",
                        std::vector<std::string>{"/uas1", "/uas2"})});

  auto expected_values = std::vector<std::pair<std::string, LT>>{
      {"tcp-l://:57601", LT::fcu}, // NOTE(vooon): std::set change order of tcp
                                   // and udp
      {"udp://:45001@", LT::fcu},
      {"tcp-l://:57602", LT::gcs},
      {"udp://:45002@", LT::gcs},
...
TEST_F(TestRouter, route_targeted_one_system) {
  ...
  set_mode.base_mode =
      enum_value(MF::SAFETY_ARMED) | enum_value(MF::TEST_ENABLED);
