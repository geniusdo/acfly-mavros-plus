...
TEST(FRAME_TF, quaternion_from_rpy__check_compatibility) {
  ...
  tf2::Quaternion bt_q;
  bt_q.setRPY(1.0, 2.0, 3.0);
  tf2::Quaternion bt_neg_q;
  bt_neg_q.setRPY(-1.0, -2.0, -3.0);
  ...
  for (ssize_t roll = min; roll <= max; roll += step) {
    for (ssize_t pitch = min; pitch <= max; pitch += step) {
      for (ssize_t yaw = min; yaw <= max; yaw += step) {
        ...
        ss << "DEG(" << expected_deg.x() << ", " << expected_deg.y() << ", "
           << expected_deg.z() << ")  ";
        ss << "RAD(" << expected.x() << ", " << expected.y() << ", "
           << expected.z() << ")";
        ...
        // instead of direct comparision we rotate other quaternion and then
        // compare results
