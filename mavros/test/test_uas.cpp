...
public:
  ...
  explicit MockUAS(const std::string name_) : UAS(name_) {}

  MOCK_METHOD1(create_plugin_instance,
               plugin::Plugin::SharedPtr(const std::string &pl_name));
};
...
public:
  ...
  explicit MockPlugin(UAS::SharedPtr uas_) : Plugin(uas_) {}
  ...
  inline Subscriptions allsubs() {
    return {make_handler(mavlink::common::msg::STATUSTEXT::MSG_ID,
                         &MockPlugin::handle_statustext_raw),
  ...
  MOCK_METHOD2(handle_statustext_raw,
               void(const mavlink_message_t *msg, const Framing framing));
  MOCK_METHOD3(handle_heartbeat_anyok,
               void(const mavlink_message_t *msg,
                    mavlink::common::msg::HEARTBEAT &hb, filter::AnyOk filter));
  MOCK_METHOD3(handle_heartbeat_systemandok,
               void(const mavlink_message_t *msg,
                    mavlink::common::msg::HEARTBEAT &hb,
                    filter::SystemAndOk filter));
  MOCK_METHOD3(handle_heartbeat_componentandok,
               void(const mavlink_message_t *msg,
                    mavlink::common::msg::HEARTBEAT &hb,
...
public:
  ...
  void set_lists(MockUAS::SharedPtr p, UAS::StrV denylist,
                 UAS::StrV allowlist) {
  ...
  void plugin_route(MockUAS::SharedPtr p, const mavlink_message_t *msg,
                    const Framing framing) {
    p->plugin_route(msg, framing);
  }

  mavlink_message_t convert_message(const mavlink::Message &msg,
                                    const uint16_t source = 0x0101) {
...
TEST_F(TestUAS, add_plugin__route_message__filter) {
  ...
  EXPECT_CALL(*uas, create_plugin_instance("test1"))
      .WillRepeatedly(Return(plugin1));
  EXPECT_CALL(*uas, create_plugin_instance("test2"))
      .WillRepeatedly(Return(plugin2));
