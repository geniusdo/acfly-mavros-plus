...
class Filter {
  virtual bool operator()(UASPtr uas, const mavlink::mavlink_message_t *cmsg,
...
public:
  ...
  explicit Plugin(UASPtr uas_, const std::string &subnode,
      ...
      : uas(uas_),
        ...
        node(rclcpp::Node::make_shared(
            subnode,
            std::dynamic_pointer_cast<rclcpp::Node>(uas_)
  ...
  virtual rclcpp::Node::SharedPtr get_node() const { return node; }

  virtual rclcpp::Logger get_logger() const { return node->get_logger(); }
...
protected:
  ...
  HandlerInfo make_handler(const mavlink::msgid_t id,
    ...
    auto bfn = std::bind(fn, std::static_pointer_cast<_C>(shared_from_this()),
  ...
  HandlerInfo make_handler(void (_C::*fn)(const mavlink::mavlink_message_t *,
                                          _T &, _F)) {
    static_assert(std::is_base_of<Filter, _F>::value,
    ...
    return HandlerInfo{id, name, type_hash_,
                       [bfn, uas_](const mavlink::mavlink_message_t *msg,
                                   const mavconn::Framing framing) {
                         auto filter = _F();
   ...
   */
  virtual SetParametersResult
  node_on_set_parameters_cb(const std::vector<rclcpp::Parameter> &parameters);
  ...
  auto node_declare_and_watch_parameter(
    ...
    // NOTE(vooon): for Foxy:
    return node->declare_parameter(name, rclcpp::ParameterValue(),
                                   parameter_descriptor, ignore_override);
#else
    // NOTE(vooon): for Galactic:
    return node->declare_parameter<ParameterT>(name, parameter_descriptor,
                                               ignore_override);
#endif
  }
  ...
  auto node_declare_and_watch_parameter(
      ...
      bool ignore_override = false) {
    node_watch_parameters[name] = cb;
    return node->declare_parameter(name, default_value, parameter_descriptor,
                                   ignore_override);
  }
  ...
  inline uint64_t get_time_usec() { return get_time_usec(node->now()); }
  ...
  inline uint32_t get_time_boot_ms() { return get_time_boot_ms(node->now()); }
};
...
template <typename _T> class PluginFactoryTemplate : public PluginFactory {
public:
  ...
  Plugin::SharedPtr create_plugin_instance(UASPtr uas) override {
    static_assert(std::is_base_of<Plugin, _T>::value,
