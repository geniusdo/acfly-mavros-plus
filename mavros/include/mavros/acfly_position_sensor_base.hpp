...
public:
  explicit AcflyPositionSensorBase(plugin::UASPtr uas_)
      : Plugin(uas_, "AcflyPositionSensor"), reset_counter(0) {
  ...
  void get_sensor_info() {
    ...
    node->get_parameter("sensor/data_frame",
                        sensor_data_frame); // SLAM坐标系下的位置
  ...
  void register_position_sensor() {
    send_register_position_sensor_frame(
        static_cast<uint8_t>(sensor_data_frame) | (1 << 7));
  }

  void unregister_position_sensor() {
    send_register_position_sensor_frame(
        static_cast<uint8_t>(sensor_data_frame) | (1 << 6));
  }

  void set_position_sensor_unavailable() {
    send_register_position_sensor_frame(
        static_cast<uint8_t>(sensor_data_frame));
  }

  void update_position_sensor(
      const builtin_interfaces::msg::Time &stamp, Eigen::Vector3d pos,
      Eigen::Vector3d vel,
