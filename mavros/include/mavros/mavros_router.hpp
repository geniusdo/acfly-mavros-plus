...
public:
  ...
  Endpoint()
      : parent(), id(0),
        link_type(Type::fcu), url{}, remote_addrs{}, stale_addrs{} {
  ...
  virtual void send_message(const mavlink_message_t *msg,
                            const Framing framing = Framing::ok,
                            id_t src_id = 0) = 0;
  virtual void recv_message(const mavlink_message_t *msg,
                            const Framing framing = Framing::ok);
 ...
 * - a UAS endpoint - special type of endpoint that is used to connect MAVROS
 * UAS node(s) to FCU(s)
 *
 * Some traffic rules:
 * 1. FCU broadcast -> GCS, UAS, but not to any other FCU
 * 2. FCU targeted system/component -> GCS/UAS endpoint that have matching
 * address
...
public:
    ...
    diagnostic_updater(this, 1.0)
  {
    RCLCPP_DEBUG(this->get_logger(),
                 "Start mavros::router::Router initialization...");

    set_parameters_handle_ptr = this->add_on_set_parameters_callback(
        std::bind(&Router::on_set_parameters_cb, this, _1));
    ...
    add_service = this->create_service<mavros_msgs::srv::EndpointAdd>(
        "~/add_endpoint", std::bind(&Router::add_endpoint, this, _1, _2));
    del_service = this->create_service<mavros_msgs::srv::EndpointDel>(
        "~/del_endpoint", std::bind(&Router::del_endpoint, this, _1, _2));

    // try to reconnect endpoint each 30 seconds
    reconnect_timer = this->create_wall_timer(
        30s, std::bind(&Router::periodic_reconnect_endpoints, this));

    // collect garbage addrs each minute
    stale_addrs_timer = this->create_wall_timer(
        60s, std::bind(&Router::periodic_clear_stale_remote_addrs, this));

    diagnostic_updater.setHardwareID(
        "none"); // NOTE: router connects several hardwares
  ...
  void route_message(Endpoint::SharedPtr src, const mavlink_message_t *msg,
...
private:
  ...
  std::atomic<size_t>
      stat_msg_routed; //!< amount of messages came to route_messages()
  ...
  void
  add_endpoint(const mavros_msgs::srv::EndpointAdd::Request::SharedPtr request,
               mavros_msgs::srv::EndpointAdd::Response::SharedPtr response);
  void
  del_endpoint(const mavros_msgs::srv::EndpointDel::Request::SharedPtr request,
  ...
  on_set_parameters_cb(const std::vector<rclcpp::Parameter> &parameters);
...
public:
  MAVConnEndpoint() : Endpoint(), stat_last_drop_count(0) {}
