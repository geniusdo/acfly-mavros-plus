...
public:
  ...
  //! Store GPS RAW data
  void update_gps_fix_epts(const sensor_msgs::msg::NavSatFix &fix, float eph,
                           float epv, int fix_type, int satellites_visible);

  //! Returns EPH, EPV, Fix type and satellites visible
  void get_gps_epts(float &eph, float &epv, int &fix_type,
 ...
 *
 * Also each plugin can use that node to create sub-nodes or other RCLCPP
 * objects.
 *
 * UAS Node provides:
 * - FCU messaging link
...
public:
  ...
  explicit UAS(const rclcpp::NodeOptions &options_ = rclcpp::NodeOptions(),
               ...
               const std::string &uas_url_ = "/uas1",
               uint8_t target_system_ = 1, uint8_t target_component_ = 1);
  ...
  /**
   * @brief Add static transform. To publish all static transforms at once, we
   * stack them in a std::vector.
   *
  ...
  /**
   * @brief Create message header from time_boot_ms or time_usec stamps and
   * frame_id.
   *
   * Setting frame_id and stamp are pretty common, this little helper should
   * reduce LOC.
   *
   ...
   */
  template <typename T>
  inline std_msgs::msg::Header synchronized_header(const std::string &frame_id,
                                                   const T time_stamp) {
   ...
   *
   * @param capabilities can accept a multiple capability params either in enum
   * or int from
   */
  ...
  template <typename... Ts> bool has_capabilities(Ts... capabilities) {
    bool ret = true;
    std::initializer_list<bool> capabilities_list{
        has_capability<Ts>(capabilities)...};
   ...
   *
   * @param cb A void function that takes a single
   * mavlink::common::MAV_PROTOCOL_CAPABILITY(MAV_CAP) param
   */
...
private:
  ...
  // XXX(vooon): we have to use own executor because Node::create_sub_node()
  // doesn't work for us.
  using thread_ptr =
  ...
  std::unordered_map<mavlink::msgid_t, plugin::Plugin::Subscriptions>
  ...
  rclcpp::Subscription<mavros_msgs::msg::Mavlink>::SharedPtr
      source;                                                   // FCU -> UAS
  ...
  virtual plugin::Plugin::SharedPtr
  create_plugin_instance(const std::string &pl_name);
