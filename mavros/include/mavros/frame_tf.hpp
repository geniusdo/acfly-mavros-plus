 ...
 */
enum class StaticTF {
  //! will change orientation from being expressed WRT NED frame to WRT ENU
  //! frame
  NED_TO_ENU,
...
/**
 * @brief Orientation transform options when applying rotations to data, for
 * ECEF.
 */
...
/**
 * @brief Transform from attitude represented WRT aircraft frame to
 *		  attitude represented WRT base_link frame, treating aircraft
 *frame as in an absolute frame of reference (local NED).
 */
template <class T>
inline T transform_orientation_absolute_frame_aircraft_baselink(const T &in) {
  return detail::transform_orientation(
      in, StaticTF::ABSOLUTE_FRAME_AIRCRAFT_TO_BASELINK);
}
...
inline T transform_orientation_absolute_frame_baselink_aircraft(const T &in) {
  return detail::transform_orientation(
      in, StaticTF::ABSOLUTE_FRAME_BASELINK_TO_AIRCRAFT);
}
...
inline T transform_frame_ecef_enu(const T &in, const T &map_origin) {
  return detail::transform_static_frame(in, map_origin,
 ...
 */
template <class T>
inline T transform_frame_aircraft_ned(const T &in,
                                      const Eigen::Quaterniond &q) {
 ...
 */
template <class T>
inline T transform_frame_ned_aircraft(const T &in,
                                      const Eigen::Quaterniond &q) {
  return detail::transform_frame(in, q);
}
 ...
 */
template <class T>
inline T transform_frame_aircraft_enu(const T &in,
                                      const Eigen::Quaterniond &q) {
  return detail::transform_frame(in, q);
}
 ...
 */
template <class T>
inline T transform_frame_enu_aircraft(const T &in,
                                      const Eigen::Quaterniond &q) {
  return detail::transform_frame(in, q);
}
 ...
 */
template <class T>
inline T transform_frame_enu_baselink(const T &in,
                                      const Eigen::Quaterniond &q) {
  return detail::transform_frame(in, q);
}
 ...
 */
template <class T>
inline T transform_frame_baselink_enu(const T &in,
                                      const Eigen::Quaterniond &q) {
  return detail::transform_frame(in, q);
}
 ...
 * @brief Convert quaternion to euler angles
 */
inline void quaternion_to_rpy(const Eigen::Quaterniond &q, double &roll,
...
template <typename _Scalar,
          std::enable_if_t<std::is_floating_point<_Scalar>::value, bool> = true>
inline void quaternion_to_mavlink(const Eigen::Quaternion<_Scalar> &q,
...
inline void covariance_to_mavlink(const T &cov,
                                  std::array<float, SIZE> &covmsg) {
  std::copy(cov.cbegin(), cov.cend(), covmsg.begin());
}

/**
 * @brief Convert upper right triangular of a covariance matrix to MAVLink
 * float[n] format
 */
template <class T, std::size_t ARR_SIZE>
inline void covariance_urt_to_mavlink(const T &covmap,
                                      std::array<float, ARR_SIZE> &covmsg) {
  auto m = covmap;
  std::size_t COV_SIZE = m.rows() * (m.rows() + 1) / 2;
  rcpputils::assert_true(COV_SIZE == ARR_SIZE,
                         "frame_tf: covariance matrix URT size is different "
                         "from Mavlink msg covariance field size");
...
/**
 * @brief Convert MAVLink float[n] format (upper right triangular of a
 * covariance matrix) to Eigen::MatrixXd<n,n> full covariance matrix
 */
template <class T, std::size_t ARR_SIZE>
inline void
mavlink_urt_to_covariance_matrix(const std::array<float, ARR_SIZE> &covmsg,
                                 T &covmat) {
...
//     cog.outl(f"""//! @brief Helper to convert common ROS geometry_msgs::{tr}
//     to Eigen::{te}""") cog.outl(f"""inline Eigen::{te} to_eigen(const
//     geometry_msgs::msg::{tr} r)""") cog.outl(f"""{{""") cog.outl(f"""  return
//     Eigen::{te}({fl});""") cog.outl(f"""}}""")
//
...
//! @brief Helper to convert common ROS geometry_msgs::Vector3 to
//! Eigen::Vector3d
inline Eigen::Vector3d to_eigen(const geometry_msgs::msg::Vector3 r) {
  return Eigen::Vector3d(r.x, r.y, r.z);
}
//! @brief Helper to convert common ROS geometry_msgs::Quaternion to
//! Eigen::Quaterniond
