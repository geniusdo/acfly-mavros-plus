...
// local_names = [v for v in all_names if v.startswith(('LOCAL', 'BODY',
// 'MOCAP',
//     'VISION', 'ESTIM'))]
// other_names = ['MISSION']
//
...
public:
  ...
  static constexpr double encode_factor(const uint8_t frame) {
    switch (frame) {
    // [[[cog:
    // for names, factor in [(global_names, 10000000), (local_names, 10000),
    // (other_names, 1)]:
    //      for name in names:
  ...
  explicit MissionItem(const MISSION_ITEM &wpi) : mavros_msgs::msg::Waypoint() {
    // [[[cog:
...
public:
  MissionBase(plugin::UASPtr uas_, const std::string &name_,
              MTYPE mission_type_ = MTYPE::MISSION,
              const char *log_prefix_ = "WP",
              const std::chrono::nanoseconds bootup_time_ = 15s)
      : Plugin(uas_, name_), mission_type(mission_type_),
        log_prefix(log_prefix_), wp_state(WP::IDLE), wp_count(0),
        wp_start_id(0), wp_end_id(0), wp_cur_id(0), wp_cur_active(0),
        wp_set_active(0), wp_retries(RETRIES_COUNT), is_timedout(false),
        reschedule_pull(false), do_pull_after_gcs(false),
        enable_partial_push(false), use_mission_item_int(false),
        mission_item_int_support_confirmed(false), BOOTUP_TIME(bootup_time_),
        LIST_TIMEOUT(30s), WP_TIMEOUT(1s), RESCHEDULE_TIME(5s) {
    timeout_timer = node->create_wall_timer(
        WP_TIMEOUT, std::bind(&MissionBase::timeout_cb, this));
  ...
  Subscriptions get_subscriptions() override {
    ...
    // NOTE(vooon): those messages do not have mission_type and only needed for
    // waypoint plugin
...
protected:
   ...
   */
  template <class MsgT> bool filter_message(const MsgT &m) {
   ...
   */
  template <class MsgT> bool sequence_mismatch(const MsgT &m) {
    if (m.seq != wp_cur_id && m.seq != wp_cur_id + 1) {
      RCLCPP_WARN(get_logger(), "%s: Seq mismatch, dropping %s (%d != %zu)",
                  log_prefix, m.get_name().c_str(), m.seq, wp_cur_id);
      return true;
    }
   ...
   */
  virtual void handle_mission_item(const mavlink::mavlink_message_t *msg,
                                   MISSION_ITEM &wpi, MFilter filter);
   ...
   */
  virtual void handle_mission_item_int(const mavlink::mavlink_message_t *msg,
                                       MISSION_ITEM_INT &wpi, MFilter filter);
   ...
   */
  virtual void
  handle_mission_request(const mavlink::mavlink_message_t *msg,
   ...
   */
  virtual void
  handle_mission_request_int(const mavlink::mavlink_message_t *msg,
   ...
   */
  virtual void handle_mission_count(const mavlink::mavlink_message_t *msg,
   ...
   */
  virtual void handle_mission_ack(const mavlink::mavlink_message_t *msg,
   ...
   */
  virtual void
  handle_mission_current(const mavlink::mavlink_message_t *msg,
   ...
   */
  virtual void
  handle_mission_item_reached(const mavlink::mavlink_message_t *msg,
  ...
  void schedule_pull(const std::chrono::nanoseconds &dt) {
    ...
    schedule_timer = node->create_wall_timer(
        dt, std::bind(&MissionBase::scheduled_pull_cb, this));
  }

  //! @brief send a single waypoint to FCU
  template <class MsgT> void send_waypoint(size_t seq) {
    static_assert(std::is_same<MsgT, MISSION_ITEM>::value ||
                      std::is_same<MsgT, MISSION_ITEM_INT>::value,
                  "wrong type");
  ...
  bool wait_push_all() {
    ...
    return list_sending.wait_for(lock, LIST_TIMEOUT) ==
               std::cv_status::no_timeout &&
           !is_timedout;
  }
  ...
  template <class MsgT> void mission_send(MsgT &wpi) {
    static_assert(std::is_same<MsgT, MISSION_ITEM>::value ||
                      std::is_same<MsgT, MISSION_ITEM_INT>::value,
                  "wrong type");
  ...
  void mission_write_partial_list(const uint16_t start_index,
