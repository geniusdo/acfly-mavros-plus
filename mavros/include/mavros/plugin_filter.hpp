...
public:
  inline bool operator()(UASPtr uas, const mavlink::mavlink_message_t *cmsg,
...
//! SystemAndOk filter passes only messages with Framing::ok and matching target
//! system id
...
//! ComponentAndOk filter passes only messages with Framing::ok and matching
//! target
...
public:
  inline bool operator()(UASPtr uas, const mavlink::mavlink_message_t *cmsg,
                         const Framing framing) override {
    return framing == Framing::ok &&
           uas->is_my_target(cmsg->sysid, cmsg->compid);
  }
};
