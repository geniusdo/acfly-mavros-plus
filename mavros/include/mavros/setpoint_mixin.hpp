...
public:
  ...
  void set_position_target_local_ned(uint32_t time_boot_ms,
                                     ...
                                     uint16_t type_mask, Eigen::Vector3d p,
                                     Eigen::Vector3d v, Eigen::Vector3d af,
                                     float yaw, float yaw_rate) {
    static_assert(std::is_base_of<plugin::Plugin, D>::value,
                  "SetPositionTargetLocalNEDMixin should be used by "
                  "mavros::plugin::Plugin child");
    ...
    // [[[cog:
    // for f in ('time_boot_ms', 'coordinate_frame', 'type_mask', 'yaw',
    // 'yaw_rate'):
    //     cog.outl(f"sp.{f} = {f};")
...
public:
  ...
  void set_position_target_global_int(uint32_t time_boot_ms,
                                      uint8_t coordinate_frame,
                                      uint16_t type_mask, int32_t lat_int,
                                      int32_t lon_int, float alt,
                                      Eigen::Vector3d v, Eigen::Vector3d af,
                                      float yaw, float yaw_rate) {
    static_assert(std::is_base_of<plugin::Plugin, D>::value,
                  "SetPositionTargetGlobalIntMixin should be used by "
                  "mavros::plugin::Plugin child");
...
public:
  //! Message sepecification: @p
  //! https://mavlink.io/en/messages/common.html#SET_ATTITUDE_TARGET
  void set_attitude_target(uint32_t time_boot_ms, uint8_t type_mask,
                           Eigen::Quaterniond orientation,
                           Eigen::Vector3d body_rate, float thrust) {
    static_assert(std::is_base_of<plugin::Plugin, D>::value,
                  "SetAttitudeTargetMixin should be used by "
                  "mavros::plugin::Plugin child");
...
public:
  ...
  void tf2_start(const char *_thd_name,
                 void (D::*cbp)(const geometry_msgs::msg::TransformStamped &)) {
    ...
    auto timer_callback = [this, base, tf_transform_cb]() -> void {
      ...
      if (_uas->tf2_buffer.canTransform(_frame_id, _child_frame_id,
                                        tf2::TimePoint(),
        ...
        try {
          ...
          RCLCPP_ERROR(_uas->get_logger(), "%s: %s", tf_thd_name.c_str(),
                       ex.what());
        }
      }
    };

    timer_ = create_timer(base->node, base->uas->get_clock(),
                          rclcpp::Duration::from_seconds(1.0 / base->tf_rate),
                          timer_callback);
  }
