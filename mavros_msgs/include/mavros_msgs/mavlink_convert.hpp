...
inline bool convert(const Mavlink &rmsg, mavlink_message_t &mmsg) {
  if (rmsg.payload64.size() >
      sizeof(mmsg.payload64) / sizeof(mmsg.payload64[0])) {
    return false;
  }

  if (!rmsg.signature.empty() &&
      rmsg.signature.size() != sizeof(mmsg.signature)) {
    return false;
  }
 ...
 * @param[in]  framing_status  framing parse result (OK, BAD_CRC or
 * BAD_SIGNATURE)
 * @return true, this convertion can't fail
 */
inline bool convert(const mavlink_message_t &mmsg, Mavlink &rmsg,
  ...
  rmsg.payload64.assign(mmsg.payload64, mmsg.payload64 + payload64_len);

  // copy signature block only if message is signed
  if (mmsg.incompat_flags & MAVLINK_IFLAG_SIGNED) {
    rmsg.signature.assign(mmsg.signature,
