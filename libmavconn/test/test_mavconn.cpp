...
//
//  I think on Debian that was fixed while ago. But i can't use it in ros
//  buildfarm.
...
public:
  ...
  void recv_message(const mavlink_message_t *message,
...
TEST_F(UDP, send_message) {
  ...
  echo->connect([&](const mavlink_message_t *msg, const Framing framing
                    [[maybe_unused]]) { echo->send_message(msg); });

  // create client
  client = std::make_shared<MAVConnUDP>(44, 200, "0.0.0.0", 45003, "localhost",
                                        45002);
  client->connect(std::bind(&UDP::recv_message, this, std::placeholders::_1,
                            std::placeholders::_2));
...
TEST_F(TCP, bind_error) {
  ...
  ASSERT_THROW(conns[1] = std::make_shared<MAVConnTCPServer>(
                   42, 200, "localhost", 57600),
               DeviceError);
}
...
TEST_F(TCP, send_message) {
  ...
  echo_server->connect([&](const mavlink_message_t *msg,
                           const Framing framing [[maybe_unused]]) {
  ...
  client->connect(std::bind(&TCP::recv_message, this, std::placeholders::_1,
                            std::placeholders::_2));
...
TEST(SERIAL, open_error) {
  ...
  ASSERT_THROW(serial = std::make_shared<MAVConnSerial>(
                   42, 200, "/some/magic/not/exist/path", 57600),
               DeviceError);
}
...
TEST(URL, open_url_udp) {
  ...
  EXPECT_NO_THROW({
    udp = MAVConnInterface::open_url(
        "udp://localhost:45004@localhost:45005/?ids=2,241");
  ...
  EXPECT_THROW({ udp = MAVConnInterface::open_url("udp://localhost:45008"); },
               DeviceError);
}
