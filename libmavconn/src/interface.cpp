...
std::unordered_map<mavlink::msgid_t, const mavlink::mavlink_msg_entry_t *>
...
MAVConnInterface::MAVConnInterface(uint8_t system_id, uint8_t component_id)
    : sys_id(system_id),
      comp_id(component_id), m_parse_status{}, m_buffer{}, m_mavlink_status{},
...
void MAVConnInterface::parse_buffer(const char *pfx, uint8_t *buf,
  ...
  for (; bytes_received > 0; bytes_received--) {
    ...
    auto msg_received = static_cast<Framing>(mavlink::mavlink_frame_char_buffer(
        &m_buffer, &m_parse_status, c, &message, &m_mavlink_status));
...
void MAVConnInterface::log_recv(const char *pfx, mavlink_message_t &msg,
  ...
  CONSOLE_BRIDGE_logDebug(
      "%s%zu: recv: %s %4s Message-Id: %u [%u bytes] IDs: %u.%u Seq: %u", pfx,
      conn_id, proto_version_str, framing_str, msg.msgid, msg.len, msg.sysid,
      msg.compid, msg.seq);
}
...
void MAVConnInterface::log_send(const char *pfx, const mavlink_message_t *msg) {
  ...
  CONSOLE_BRIDGE_logDebug(
      "%s%zu: send: %s Message-Id: %u [%u bytes] IDs: %u.%u Seq: %u", pfx,
      conn_id, proto_version_str, msg->msgid, msg->len, msg->sysid, msg->compid,
      msg->seq);
}

void MAVConnInterface::log_send_obj(const char *pfx,
                                    const mavlink::Message &msg) {
  CONSOLE_BRIDGE_logDebug("%s%zu: send: %s", pfx, conn_id,
                          msg.to_yaml().c_str());
}
...
void MAVConnInterface::send_message_ignore_drop(const mavlink::Message &msg,
  ...
  try {
    ...
    CONSOLE_BRIDGE_logError(PFX "%zu: DROPPED Message %s: %s", conn_id,
                            msg.get_name().c_str(), e.what());
  }
}
 ...
 */
static void url_parse_host(const std::string &host, std::string &host_out,
 ...
 * Parse ?ids=sid,cid
 */
static void url_parse_query(const std::string &query, uint8_t &sysid,
                            uint8_t &compid) {
  ...
  CONSOLE_BRIDGE_logDebug(PFX "URL: found system/component id = [%u, %u]",
                          sysid, compid);
}

static MAVConnInterface::Ptr
url_parse_serial(const std::string &path, const std::string &query,
                 uint8_t system_id, uint8_t component_id, bool hwflow) {
  std::string file_path;
  int baudrate;

  // /dev/ttyACM0:57600
  url_parse_host(path, file_path, baudrate, MAVConnSerial::DEFAULT_DEVICE,
                 MAVConnSerial::DEFAULT_BAUDRATE);
  url_parse_query(query, system_id, component_id);

  return std::make_shared<MAVConnSerial>(system_id, component_id, file_path,
                                         baudrate, hwflow);
}

static MAVConnInterface::Ptr url_parse_udp(const std::string &hosts,
  ...
  if (is_udpb) {
    remote_host = permanent_broadcast
...
static MAVConnInterface::Ptr url_parse_tcp_client(const std::string &host,
                                                  const std::string &query,
                                                  uint8_t system_id,
...
static MAVConnInterface::Ptr url_parse_tcp_server(const std::string &host,
                                                  const std::string &query,
                                                  uint8_t system_id,
                                                  uint8_t component_id) {
  ...
  return std::make_shared<MAVConnTCPServer>(system_id, component_id, bind_host,
                                            bind_port);
}

MAVConnInterface::Ptr
MAVConnInterface::open_url_no_connect(std::string url, uint8_t system_id,
                                      uint8_t component_id) {
  ...
  CONSOLE_BRIDGE_logDebug(
      PFX "URL: %s: proto: %s, host: %s, path: %s, query: %s", url.c_str(),
  ...
  if (proto == "udp") {
    interface_ptr =
        url_parse_udp(host, query, system_id, component_id, false, false);
  } else if (proto == "udp-b") {
    interface_ptr =
        url_parse_udp(host, query, system_id, component_id, true, false);
  } else if (proto == "udp-pb") {
    interface_ptr =
        url_parse_udp(host, query, system_id, component_id, true, true);
  } else if (proto == "tcp") {
    ...
    interface_ptr =
        url_parse_serial(path, query, system_id, component_id, false);
  } else if (proto == "serial-hwfc") {
    interface_ptr =
        url_parse_serial(path, query, system_id, component_id, true);
  } else {
...
MAVConnInterface::Ptr
MAVConnInterface::open_url(std::string url, uint8_t system_id,
                           uint8_t component_id,
