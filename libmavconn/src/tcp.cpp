...
static bool resolve_address_tcp(io_service &io, size_t chan, std::string host,
                                uint16_t port, tcp::endpoint &ep) {
  bool result = false;
  ...
  auto fn = [&](const tcp::endpoint &q_ep) {
    ...
    ep.port(port);
    result = true;
    CONSOLE_BRIDGE_logDebug(PFXd "host %s resolved as %s", chan, host.c_str(),
  ...
  if (ec) {
    CONSOLE_BRIDGE_logWarn(PFXd "resolve error: %s", chan,
                           ec.message().c_str());
    result = false;
  }
...
MAVConnTCPClient::MAVConnTCPClient(uint8_t system_id, uint8_t component_id,
    ...
    : MAVConnInterface(system_id, component_id), io_service(),
      io_work(new io_service::work(io_service)), is_running(false),
      socket(io_service), is_destroying(false),
      tx_in_progress(false), tx_q{}, rx_buf{} {
  if (!resolve_address_tcp(io_service, conn_id, server_host, server_port,
                           server_ep)) {
    throw DeviceError("tcp: resolve", "Bind address resolve failed");
  }

  CONSOLE_BRIDGE_logInform(PFXd "Server address: %s", conn_id,
                           to_string_ss(server_ep).c_str());
...
MAVConnTCPClient::MAVConnTCPClient(uint8_t system_id, uint8_t component_id,
    ...
    : MAVConnInterface(system_id, component_id), is_running(false),
      socket(server_io), is_destroying(false),
      tx_in_progress(false), tx_q{}, rx_buf{} {
...
void MAVConnTCPClient::client_connected(size_t server_channel) {
  CONSOLE_BRIDGE_logInform(PFXd "Got client, id: %zu, address: %s",
                           server_channel, conn_id,
                           to_string_ss(server_ep).c_str());

  // start recv
  GET_IO_SERVICE(socket).post(
...
void MAVConnTCPClient::connect(const ReceivedCb &cb_handle_message,
  ...
  io_thread = std::thread([this]() {
    ...
    try {
      ...
      CONSOLE_BRIDGE_logError(PFXd "io_service execption: %s", conn_id,
                              ex.what());
    }
    is_running = false;
  });
...
void MAVConnTCPClient::close() {
  {
    ...
    if (ec) {
      CONSOLE_BRIDGE_logError(PFXd "shutdown: %s", conn_id,
                              ec.message().c_str());
    }
    socket.cancel();
    socket.close();
  ...
  // Stop io_service if the thread is not the io_thread (else exception
...
void MAVConnTCPClient::do_recv() {
  ...
  socket.async_receive(
      buffer(rx_buf), [sthis](error_code error, size_t bytes_transferred) {
        if (error) {
          CONSOLE_BRIDGE_logError(PFXd "receive: %s", sthis->conn_id,
                                  error.message().c_str());
          sthis->close();
          return;
        }
...
void MAVConnTCPClient::do_send(bool check_tx_state) {
  ...
  socket.async_send(
      ...
      [sthis, &buf_ref](error_code error, size_t bytes_transferred) {
        ...
        if (error) {
          CONSOLE_BRIDGE_logError(PFXd "send: %s", sthis->conn_id,
                                  error.message().c_str());
          sthis->close();
          return;
        }
...
MAVConnTCPServer::MAVConnTCPServer(uint8_t system_id, uint8_t component_id,
  ...
  CONSOLE_BRIDGE_logInform(PFXd "Bind address: %s", conn_id,
                           to_string_ss(bind_ep).c_str());
...
MAVConnInterface::IOStat MAVConnTCPServer::get_iostat() {
  ...
  for (auto &instp : client_list) {
    ...
    //         cog.outl("iostat.{p}_{f} +=
    //         inst_iostat.{p}_{f};".format(**locals()))
    // ]]]
...
void MAVConnTCPServer::do_accept() {
  ...
  auto sthis = shared_from_this();
  auto acceptor_client =
      std::make_shared<MAVConnTCPClient>(sys_id, comp_id, io_service);
  acceptor.async_accept(
      acceptor_client->socket, acceptor_client->server_ep,
      [sthis, acceptor_client](error_code error) {
        if (error) {
          CONSOLE_BRIDGE_logError(PFXd "accept: %s", sthis->conn_id,
                                  error.message().c_str());
          sthis->close();
          return;
        }
...
void MAVConnTCPServer::client_closed(
    std::weak_ptr<MAVConnTCPClient> weak_instp) {
  if (auto instp = weak_instp.lock()) {
    CONSOLE_BRIDGE_logInform(
        PFXd "Client connection closed, id: %p, address: %s", conn_id,
        instp.get(), to_string_ss(instp->server_ep).c_str());
