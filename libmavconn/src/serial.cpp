//
// libmavconn
// Copyright 2013,2014,2015,2016,2018,2021 Vladimir Ermakov, All rights
// reserved.
//
...
MAVConnSerial::MAVConnSerial(uint8_t system_id, uint8_t component_id,
    ...
    : MAVConnInterface(system_id, component_id), io_service(),
      serial_dev(io_service), tx_in_progress(false), tx_q{}, rx_buf{} {
  ...
  try {
    ...
    // Flow control setting in older versions of ASIO is broken, use workaround
    // (below) for now.
    serial_dev.set_option(SPB::flow_control(
...
void MAVConnSerial::send_bytes(const uint8_t *bytes, size_t length) {
  ...
  io_service.post(
      std::bind(&MAVConnSerial::do_write, shared_from_this(), true));
}
