...
MAVConnUDP::MAVConnUDP(uint8_t system_id, uint8_t component_id,
    ...
    : MAVConnInterface(system_id, component_id), io_service(),
      ...
      permanent_broadcast(false), remote_exists(false), socket(io_service),
      tx_in_progress(false), tx_q{}, rx_buf{} {
  using udps = asio::ip::udp::socket;

  if (!resolve_address_udp(io_service, conn_id, bind_host, bind_port,
                           bind_ep)) {
  ...
  if (remote_host != "") {
    if (remote_host != BROADCAST_REMOTE_HOST &&
        remote_host != PERMANENT_BROADCAST_REMOTE_HOST) {
    ...
    if (remote_exists) {
      CONSOLE_BRIDGE_logInform(PFXd "Remote address: %s", conn_id,
...
void MAVConnUDP::send_bytes(const uint8_t *bytes, size_t length) {
  ...
  if (!remote_exists) {
    CONSOLE_BRIDGE_logDebug(PFXd "send: Remote not known, message dropped.",
                            conn_id);
    return;
  }
...
void MAVConnUDP::do_recvfrom() {
  ...
  socket.async_receive_from(
      buffer(rx_buf), permanent_broadcast ? recv_ep : remote_ep,
      [sthis](error_code error, size_t bytes_transferred) {
        ...
        if (!sthis->permanent_broadcast &&
            sthis->remote_ep != sthis->last_remote_ep) {
          CONSOLE_BRIDGE_logInform(PFXd "Remote address: %s", sthis->conn_id,
  ...
  socket.async_send_to(
      ...
      [sthis, &buf_ref](error_code error, size_t bytes_transferred) {
        ...
        if (error == asio::error::network_unreachable) {
          CONSOLE_BRIDGE_logWarn(PFXd "sendto: %s, retrying", sthis->conn_id,
                                 error.message().c_str());
          // do not return, try to resend
        } else if (error) {
          CONSOLE_BRIDGE_logError(PFXd "sendto: %s", sthis->conn_id,
                                  error.message().c_str());
          sthis->close();
          return;
        }
