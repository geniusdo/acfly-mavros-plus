...
struct MsgBuffer {
  ...
  MsgBuffer() : data{}, len(0), pos(0) {}

  /**
   * @brief Buffer constructor from mavlink_message_t
   */
  explicit MsgBuffer(const mavlink::mavlink_message_t *msg) : pos(0) {
   ...
   */
  MsgBuffer(const mavlink::Message &obj, mavlink::mavlink_status_t *status,
    ...
    mavlink::mavlink_finalize_message_buffer(
        &msg, sysid, compid, status, mi.min_length, mi.length, mi.crc_extra);
